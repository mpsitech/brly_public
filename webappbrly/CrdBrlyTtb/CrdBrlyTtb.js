/**
  * \file CrdBrlyTtb.js
  * web client functionality for card CrdBrlyTtb
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppBrlyTtb", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppBrlyTtb", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppBrlyTtb", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppBrlyTtb", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppBrlyTtb", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlBrlyTtbHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlBrlyTtbList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlBrlyTtbRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppBrlyTtb", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppBrlyTtb", "shortMenu", short);
	setSi(srcdoc, "StatAppBrlyTtb", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppBrlyTtb", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppBrlyTtb", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagBrlyTtb", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagBrlyTtb", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrBrlyTtb", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrBrlyTtb", "MitCrdNewAvail") == "true");
	MitCrdTrlAvail = (retrieveSi(srcdoc, "StatShrBrlyTtb", "MitCrdTrlAvail") == "true");
	MitCrdIflAvail = (retrieveSi(srcdoc, "StatShrBrlyTtb", "MitCrdIflAvail") == "true");
	MitCrdIflActive = (retrieveSi(srcdoc, "StatShrBrlyTtb", "MitCrdIflActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppBrlyTtb", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdTtb"), retrieveCi(srcdoc, "ContInfBrlyTtb", "MtxCrdTtb"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagBrlyTtb", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdTrl"), retrieveTi(srcdoc, "TagBrlyTtb", "MitCrdTrl"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdIfl"), retrieveTi(srcdoc, "TagBrlyTtb", "MitCrdIfl"));
	setMitActive("MitCrdIfl", MitCrdIflActive);

	height -= setMitMspAvail("MspCrd1", MspCrd1Avail, 1);
	height -= setMitMspAvail("MitCrdNew", MitCrdNewAvail, 20);
	height -= setMitMspAvail("MitCrdTrl", MitCrdTrlAvail, 20);
	height -= setMitMspAvail("MitCrdIfl", MitCrdIflAvail, 20);
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgBrlyTtbTrlorig") || (sref == "DlgBrlyTtbImpflt")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgimpflt = retrieveSi(srcdoc, "StatShrBrlyTtb", "scrJrefDlgimpflt");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrBrlyTtb", "scrJrefDlgnew");
	var scrJrefDlgtrlorig = retrieveSi(srcdoc, "StatShrBrlyTtb", "scrJrefDlgtrlorig");

	if (scrJrefDlgimpflt != "") {
		if (scrJrefDlg != scrJrefDlgimpflt) showDlg("DlgBrlyTtbImpflt", scrJrefDlgimpflt);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgBrlyTtbNew", scrJrefDlgnew);
	} else if (scrJrefDlgtrlorig != "") {
		if (scrJrefDlg != scrJrefDlgtrlorig) showDlg("DlgBrlyTtbTrlorig", scrJrefDlgtrlorig);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfBrlyTtb", "MtxCrdTtb") + " - BeamRelay 0.2.2";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyTtbDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyTtbDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfBrlyTtb", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyTtbData", "ContInfBrlyTtb", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyTtbData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngBrlyTtbData", "StatAppBrlyTtb", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyTtbData", "StatShrBrlyTtb", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyTtbData", "TagBrlyTtb", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyTtbData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrBrlyTtb", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrBrlyTtb", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrBrlyTtb", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyTtbData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyTtb", "srefIxBrlyVReqitmode");

	if (srefIxBrlyVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxBrlyVReqitmode = "notify";
		setSi(srcdoc, "StatAppBrlyTtb", "srefIxBrlyVReqitmode", srefIxBrlyVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyTtb", "srefIxBrlyVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppBrlyTtb", "latency"));
