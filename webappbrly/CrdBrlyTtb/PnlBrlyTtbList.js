/**
  * \file PnlBrlyTtbList.js
  * web client functionality for panel PnlBrlyTtbList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyTtbList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyTtbList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyTtbList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyTtbList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyTtbList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyTtbList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyTtbList", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyTtbList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyTtbList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyTtbList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyTtbList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyTtbList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyTtbList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyTtbList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyTtbList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyTtbList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyTtbList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyTtbList", "TcoTitWidth"));
		var TcoFilWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyTtbList", "TcoFilWidth"));
		var TcoAliWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyTtbList", "TcoAliWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyTtbList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyTtbList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyTtbList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoFil"); doc.widths.push(TcoFilWidth); doc.fs.push("fil"); doc.tos.push(2); wtot += TcoFilWidth;
			} else if (i == 3) {
				doc.cols.push("TcoAli"); doc.widths.push(TcoAliWidth); doc.fs.push("ali2"); doc.tos.push(0); wtot += TcoAliWidth;
			} else if (i == 4) {
				doc.cols.push("TcoSta"); doc.widths.push(TcoStaWidth); doc.fs.push("sta"); doc.tos.push(0); wtot += TcoStaWidth;
			} else if (i == 5) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyTtbList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyTtbList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQTtbList
		rownode = getRownode(srcdoc, "ListBrlyQTtbList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyTtbList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyTtbList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyTtbList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyTtbList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyTtbList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyTtbList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyTtbList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyTtbList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyTtbListData", scrJref, "StgIacQryBrlyTtbList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyTtbList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyTtbList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyTtbList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyTtbListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyTtbList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyTtbList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyTtbListData", scrJref, "ContIacBrlyTtbList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "ContIacBrlyTtbList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "ContInfBrlyTtbList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "StatShrBrlyTtbList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "StgIacBrlyTtbList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "TagBrlyTtbList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "ListBrlyQTtbList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "StatAppQryBrlyTtbList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "StatShrQryBrlyTtbList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyTtbListData", "StgIacQryBrlyTtbList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyTtbListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyTtbListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyTtbListData") {
				mask = mergeDpchEngData(dom);
