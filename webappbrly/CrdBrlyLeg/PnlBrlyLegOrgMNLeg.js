/**
  * \file PnlBrlyLegOrgMNLeg.js
  * web client functionality for panel PnlBrlyLegOrgMNLeg
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLegOrgMNLeg", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLegOrgMNLeg", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("OrgMNLeg", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("OrgMNLeg_side").setAttribute("height", "30");
	doc.getElementById("OrgMNLeg_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("OrgMNLeg_side").src = "./PnlBrlyLegOrgMNLeg_aside.html";
	doc.getElementById("OrgMNLeg_cont").src = "./PnlBrlyLegOrgMNLeg_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLegOrgMNLeg", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLegOrgMNLeg", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("OrgMNLeg", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("OrgMNLeg_side").setAttribute("height", "255");
	doc.getElementById("OrgMNLeg_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("OrgMNLeg_side").src = "./PnlBrlyLegOrgMNLeg_bside.html";
	doc.getElementById("OrgMNLeg_cont").src = "./PnlBrlyLegOrgMNLeg_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("OrgMNLeg_side").contentDocument;
	contdoc = doc.getElementById("OrgMNLeg_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("OrgMNLeg_side").contentDocument;
	contdoc = doc.getElementById("OrgMNLeg_cont").contentDocument;
	hdrdoc = contdoc.getElementById("OrgMNLeg_hdr").contentDocument;
	tbldoc = contdoc.getElementById("OrgMNLeg_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("OrgMNLeg_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLegOrgMNLeg", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLegOrgMNLeg", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLegOrgMNLeg", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLegOrgMNLeg", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLegOrgMNLeg", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegOrgMNLeg", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegOrgMNLeg", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegOrgMNLeg", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegOrgMNLeg", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegOrgMNLeg", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegOrgMNLeg", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegOrgMNLeg", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegOrgMNLeg", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoMrefWidth"));
		var TcoDp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoDp0Width"));
		var TcoDp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoDp1Width"));
		var TcoOp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoOp0Width"));
		var TcoOp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoOp1Width"));
		var TcoXd0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoXd0Width"));
		var TcoXd1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoXd1Width"));
		var TcoXo0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoXo0Width"));
		var TcoXo1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegOrgMNLeg", "TcoXo1Width"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDp0"); doc.widths.push(TcoDp0Width); doc.fs.push("dp0"); wtot += TcoDp0Width;
			} else if (i == 3) {
				doc.cols.push("TcoDp1"); doc.widths.push(TcoDp1Width); doc.fs.push("dp1"); wtot += TcoDp1Width;
			} else if (i == 4) {
				doc.cols.push("TcoOp0"); doc.widths.push(TcoOp0Width); doc.fs.push("op0"); wtot += TcoOp0Width;
			} else if (i == 5) {
				doc.cols.push("TcoOp1"); doc.widths.push(TcoOp1Width); doc.fs.push("op1"); wtot += TcoOp1Width;
			} else if (i == 6) {
				doc.cols.push("TcoXd0"); doc.widths.push(TcoXd0Width); doc.fs.push("xd0"); wtot += TcoXd0Width;
			} else if (i == 7) {
				doc.cols.push("TcoXd1"); doc.widths.push(TcoXd1Width); doc.fs.push("xd1"); wtot += TcoXd1Width;
			} else if (i == 8) {
				doc.cols.push("TcoXo0"); doc.widths.push(TcoXo0Width); doc.fs.push("xo0"); wtot += TcoXo0Width;
			} else if (i == 9) {
				doc.cols.push("TcoXo1"); doc.widths.push(TcoXo1Width); doc.fs.push("xo1"); wtot += TcoXo1Width;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLegOrgMNLeg", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLegOrgMNLeg", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLegOrgMNLeg
		rownode = getRownode(srcdoc, "ListBrlyQLegOrgMNLeg", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLegOrgMNLeg", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLegOrgMNLeg", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLegOrgMNLeg", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("OrgMNLeg", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("OrgMNLeg_side").setAttribute("height", "" + (height+55));
	doc.getElementById("OrgMNLeg_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("OrgMNLeg_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("OrgMNLeg_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegOrgMNLeg", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegOrgMNLeg", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegOrgMNLeg", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLegOrgMNLeg", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLegOrgMNLeg", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLegOrgMNLegData", scrJref, "StgIacQryBrlyLegOrgMNLeg");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLegOrgMNLeg", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegOrgMNLeg", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLegOrgMNLeg", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLegOrgMNLeg", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegOrgMNLeg", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLegOrgMNLegDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "ContInfBrlyLegOrgMNLeg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "StatAppBrlyLegOrgMNLeg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "StatShrBrlyLegOrgMNLeg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "StgIacBrlyLegOrgMNLeg", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "TagBrlyLegOrgMNLeg", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "ListBrlyQLegOrgMNLeg", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "StatAppQryBrlyLegOrgMNLeg", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "StatShrQryBrlyLegOrgMNLeg", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLegOrgMNLegData", "StgIacQryBrlyLegOrgMNLeg", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLegOrgMNLegData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLegOrgMNLegData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("OrgMNLeg");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLegOrgMNLegData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
