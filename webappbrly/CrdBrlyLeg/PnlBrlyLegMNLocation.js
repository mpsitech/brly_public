/**
  * \file PnlBrlyLegMNLocation.js
  * web client functionality for panel PnlBrlyLegMNLocation
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLegMNLocation", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLegMNLocation", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("MNLocation", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNLocation_side").setAttribute("height", "30");
	doc.getElementById("MNLocation_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNLocation_side").src = "./PnlBrlyLegMNLocation_aside.html";
	doc.getElementById("MNLocation_cont").src = "./PnlBrlyLegMNLocation_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLegMNLocation", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLegMNLocation", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("MNLocation", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNLocation_side").setAttribute("height", "255");
	doc.getElementById("MNLocation_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNLocation_side").src = "./PnlBrlyLegMNLocation_bside.html";
	doc.getElementById("MNLocation_cont").src = "./PnlBrlyLegMNLocation_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNLocation_side").contentDocument;
	contdoc = doc.getElementById("MNLocation_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNLocation_side").contentDocument;
	contdoc = doc.getElementById("MNLocation_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNLocation_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNLocation_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNLocation_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLegMNLocation", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLegMNLocation", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLegMNLocation", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLegMNLocation", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLegMNLocation", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegMNLocation", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegMNLocation", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegMNLocation", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegMNLocation", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegMNLocation", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegMNLocation", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegMNLocation", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegMNLocation", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegMNLocation", "TcoMrefWidth"));
		var TcoLp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegMNLocation", "TcoLp0Width"));
		var TcoLp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegMNLocation", "TcoLp1Width"));
		var TcoLphWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegMNLocation", "TcoLphWidth"));
		var TcoLthWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegMNLocation", "TcoLthWidth"));
		var TcoXp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegMNLocation", "TcoXp0Width"));
		var TcoXp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegMNLocation", "TcoXp1Width"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoLp0"); doc.widths.push(TcoLp0Width); doc.fs.push("lp0"); wtot += TcoLp0Width;
			} else if (i == 3) {
				doc.cols.push("TcoLp1"); doc.widths.push(TcoLp1Width); doc.fs.push("lp1"); wtot += TcoLp1Width;
			} else if (i == 4) {
				doc.cols.push("TcoLph"); doc.widths.push(TcoLphWidth); doc.fs.push("lph"); wtot += TcoLphWidth;
			} else if (i == 5) {
				doc.cols.push("TcoLth"); doc.widths.push(TcoLthWidth); doc.fs.push("lth"); wtot += TcoLthWidth;
			} else if (i == 6) {
				doc.cols.push("TcoXp0"); doc.widths.push(TcoXp0Width); doc.fs.push("xp0"); wtot += TcoXp0Width;
			} else if (i == 7) {
				doc.cols.push("TcoXp1"); doc.widths.push(TcoXp1Width); doc.fs.push("xp1"); wtot += TcoXp1Width;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLegMNLocation", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLegMNLocation", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLegMNLocation
		rownode = getRownode(srcdoc, "ListBrlyQLegMNLocation", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLegMNLocation", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLegMNLocation", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLegMNLocation", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNLocation", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNLocation_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNLocation_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNLocation_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNLocation_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegMNLocation", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegMNLocation", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegMNLocation", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLegMNLocation", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLegMNLocation", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLegMNLocationData", scrJref, "StgIacQryBrlyLegMNLocation");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLegMNLocation", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegMNLocation", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLegMNLocation", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLegMNLocation", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegMNLocation", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLegMNLocationDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "ContInfBrlyLegMNLocation", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "StatAppBrlyLegMNLocation", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "StatShrBrlyLegMNLocation", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "StgIacBrlyLegMNLocation", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "TagBrlyLegMNLocation", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "ListBrlyQLegMNLocation", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "StatAppQryBrlyLegMNLocation", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "StatShrQryBrlyLegMNLocation", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLegMNLocationData", "StgIacQryBrlyLegMNLocation", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLegMNLocationData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLegMNLocationData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNLocation");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLegMNLocationData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
