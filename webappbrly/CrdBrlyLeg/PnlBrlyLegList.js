/**
  * \file PnlBrlyLegList.js
  * web client functionality for panel PnlBrlyLegList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyLegList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyLegList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyLegList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyLegList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLegList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLegList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyLegList", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLegList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLegList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegList", "ncol"));

		var TcoBloWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegList", "TcoBloWidth"));
		var TcoEloWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegList", "TcoEloWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegList", "TcoTypWidth"));
		var TcoAltWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegList", "TcoAltWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyLegList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoBlo"); doc.widths.push(TcoBloWidth); doc.fs.push("blo"); doc.tos.push(1); wtot += TcoBloWidth;
			} else if (i == 2) {
				doc.cols.push("TcoElo"); doc.widths.push(TcoEloWidth); doc.fs.push("elo"); doc.tos.push(2); wtot += TcoEloWidth;
			} else if (i == 3) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyLegList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLegList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLegList
		rownode = getRownode(srcdoc, "ListBrlyQLegList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLegList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLegList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLegList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLegList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLegList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLegListData", scrJref, "StgIacQryBrlyLegList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLegList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLegListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyLegList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyLegList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLegListData", scrJref, "ContIacBrlyLegList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "ContIacBrlyLegList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "ContInfBrlyLegList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "StatShrBrlyLegList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "StgIacBrlyLegList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "TagBrlyLegList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "ListBrlyQLegList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "StatAppQryBrlyLegList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "StatShrQryBrlyLegList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLegListData", "StgIacQryBrlyLegList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyLegListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLegListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLegListData") {
				mask = mergeDpchEngData(dom);
