/**
  * \file PnlBrlyLeg1NFlight.js
  * web client functionality for panel PnlBrlyLeg1NFlight
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLeg1NFlight", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLeg1NFlight", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("1NFlight", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NFlight_side").setAttribute("height", "30");
	doc.getElementById("1NFlight_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NFlight_side").src = "./PnlBrlyLeg1NFlight_aside.html";
	doc.getElementById("1NFlight_cont").src = "./PnlBrlyLeg1NFlight_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLeg1NFlight", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLeg1NFlight", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("1NFlight", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NFlight_side").setAttribute("height", "255");
	doc.getElementById("1NFlight_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NFlight_side").src = "./PnlBrlyLeg1NFlight_bside.html";
	doc.getElementById("1NFlight_cont").src = "./PnlBrlyLeg1NFlight_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NFlight_side").contentDocument;
	contdoc = doc.getElementById("1NFlight_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NFlight_side").contentDocument;
	contdoc = doc.getElementById("1NFlight_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NFlight_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NFlight_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NFlight_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLeg1NFlight", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLeg1NFlight", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLeg1NFlight", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLeg1NFlight", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLeg1NFlight", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLeg1NFlight", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLeg1NFlight", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLeg1NFlight", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLeg1NFlight", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLeg1NFlight", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLeg1NFlight", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLeg1NFlight", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLeg1NFlight", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLeg1NFlight", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLeg1NFlight", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLeg1NFlight", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLeg1NFlight
		rownode = getRownode(srcdoc, "ListBrlyQLeg1NFlight", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLeg1NFlight", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLeg1NFlight", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLeg1NFlight", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NFlight", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NFlight_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NFlight_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NFlight_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NFlight_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLeg1NFlight", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLeg1NFlight", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLeg1NFlight", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLeg1NFlight", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLeg1NFlight", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLeg1NFlightData", scrJref, "StgIacQryBrlyLeg1NFlight");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLeg1NFlight", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLeg1NFlight", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLeg1NFlight", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLeg1NFlight", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLeg1NFlight", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLeg1NFlightDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "ContInfBrlyLeg1NFlight", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "StatAppBrlyLeg1NFlight", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "StatShrBrlyLeg1NFlight", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "StgIacBrlyLeg1NFlight", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "TagBrlyLeg1NFlight", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "ListBrlyQLeg1NFlight", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "StatAppQryBrlyLeg1NFlight", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "StatShrQryBrlyLeg1NFlight", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLeg1NFlightData", "StgIacQryBrlyLeg1NFlight", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLeg1NFlightData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLeg1NFlightData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NFlight");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLeg1NFlightData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
