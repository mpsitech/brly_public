/**
  * \file PnlBrlyLegCor1NConnection.js
  * web client functionality for panel PnlBrlyLegCor1NConnection
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLegCor1NConnection", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLegCor1NConnection", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("Cor1NConnection", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Cor1NConnection_side").setAttribute("height", "30");
	doc.getElementById("Cor1NConnection_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Cor1NConnection_side").src = "./PnlBrlyLegCor1NConnection_aside.html";
	doc.getElementById("Cor1NConnection_cont").src = "./PnlBrlyLegCor1NConnection_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLegCor1NConnection", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLegCor1NConnection", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("Cor1NConnection", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Cor1NConnection_side").setAttribute("height", "255");
	doc.getElementById("Cor1NConnection_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Cor1NConnection_side").src = "./PnlBrlyLegCor1NConnection_bside.html";
	doc.getElementById("Cor1NConnection_cont").src = "./PnlBrlyLegCor1NConnection_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Cor1NConnection_side").contentDocument;
	contdoc = doc.getElementById("Cor1NConnection_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Cor1NConnection_side").contentDocument;
	contdoc = doc.getElementById("Cor1NConnection_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Cor1NConnection_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Cor1NConnection_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Cor1NConnection_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLegCor1NConnection", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLegCor1NConnection", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegCor1NConnection", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegCor1NConnection", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegCor1NConnection", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegCor1NConnection", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegCor1NConnection", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegCor1NConnection", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegCor1NConnection", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegCor1NConnection", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLegCor1NConnection", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLegCor1NConnection", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLegCor1NConnection
		rownode = getRownode(srcdoc, "ListBrlyQLegCor1NConnection", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLegCor1NConnection", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLegCor1NConnection", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Cor1NConnection", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Cor1NConnection_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Cor1NConnection_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Cor1NConnection_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Cor1NConnection_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegCor1NConnection", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegCor1NConnection", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLegCor1NConnection", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLegCor1NConnection", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLegCor1NConnection", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLegCor1NConnectionData", scrJref, "StgIacQryBrlyLegCor1NConnection");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLegCor1NConnection", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLegCor1NConnection", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLegCor1NConnection", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLegCor1NConnection", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLegCor1NConnection", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLegCor1NConnectionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "ContInfBrlyLegCor1NConnection", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "StatAppBrlyLegCor1NConnection", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "StatShrBrlyLegCor1NConnection", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "StgIacBrlyLegCor1NConnection", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "TagBrlyLegCor1NConnection", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "ListBrlyQLegCor1NConnection", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "StatAppQryBrlyLegCor1NConnection", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "StatShrQryBrlyLegCor1NConnection", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLegCor1NConnectionData", "StgIacQryBrlyLegCor1NConnection", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLegCor1NConnectionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLegCor1NConnectionData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Cor1NConnection");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLegCor1NConnectionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
