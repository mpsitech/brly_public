/**
  * \file PnlBrlyLocEnd1NLeg.js
  * web client functionality for panel PnlBrlyLocEnd1NLeg
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLocEnd1NLeg", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLocEnd1NLeg", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("End1NLeg", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("End1NLeg_side").setAttribute("height", "30");
	doc.getElementById("End1NLeg_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("End1NLeg_side").src = "./PnlBrlyLocEnd1NLeg_aside.html";
	doc.getElementById("End1NLeg_cont").src = "./PnlBrlyLocEnd1NLeg_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLocEnd1NLeg", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLocEnd1NLeg", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("End1NLeg", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("End1NLeg_side").setAttribute("height", "255");
	doc.getElementById("End1NLeg_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("End1NLeg_side").src = "./PnlBrlyLocEnd1NLeg_bside.html";
	doc.getElementById("End1NLeg_cont").src = "./PnlBrlyLocEnd1NLeg_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("End1NLeg_side").contentDocument;
	contdoc = doc.getElementById("End1NLeg_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("End1NLeg_side").contentDocument;
	contdoc = doc.getElementById("End1NLeg_cont").contentDocument;
	hdrdoc = contdoc.getElementById("End1NLeg_hdr").contentDocument;
	tbldoc = contdoc.getElementById("End1NLeg_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("End1NLeg_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocEnd1NLeg", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLocEnd1NLeg", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLocEnd1NLeg", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocEnd1NLeg", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLocEnd1NLeg", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEnd1NLeg", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEnd1NLeg", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEnd1NLeg", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEnd1NLeg", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEnd1NLeg", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEnd1NLeg", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEnd1NLeg", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEnd1NLeg", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocEnd1NLeg", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocEnd1NLeg", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLocEnd1NLeg", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLocEnd1NLeg
		rownode = getRownode(srcdoc, "ListBrlyQLocEnd1NLeg", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLocEnd1NLeg", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLocEnd1NLeg", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLocEnd1NLeg", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("End1NLeg", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("End1NLeg_side").setAttribute("height", "" + (height+55));
	doc.getElementById("End1NLeg_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("End1NLeg_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("End1NLeg_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEnd1NLeg", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEnd1NLeg", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEnd1NLeg", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLocEnd1NLeg", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLocEnd1NLeg", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocEnd1NLegData", scrJref, "StgIacQryBrlyLocEnd1NLeg");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocEnd1NLeg", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEnd1NLeg", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLocEnd1NLeg", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocEnd1NLeg", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEnd1NLeg", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLocEnd1NLegDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "ContInfBrlyLocEnd1NLeg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "StatAppBrlyLocEnd1NLeg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "StatShrBrlyLocEnd1NLeg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "StgIacBrlyLocEnd1NLeg", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "TagBrlyLocEnd1NLeg", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "ListBrlyQLocEnd1NLeg", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "StatAppQryBrlyLocEnd1NLeg", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "StatShrQryBrlyLocEnd1NLeg", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocEnd1NLegData", "StgIacQryBrlyLocEnd1NLeg", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLocEnd1NLegData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLocEnd1NLegData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("End1NLeg");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLocEnd1NLegData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
