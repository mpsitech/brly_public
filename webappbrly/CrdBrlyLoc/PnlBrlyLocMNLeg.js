/**
  * \file PnlBrlyLocMNLeg.js
  * web client functionality for panel PnlBrlyLocMNLeg
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLocMNLeg", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLocMNLeg", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("MNLeg", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNLeg_side").setAttribute("height", "30");
	doc.getElementById("MNLeg_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNLeg_side").src = "./PnlBrlyLocMNLeg_aside.html";
	doc.getElementById("MNLeg_cont").src = "./PnlBrlyLocMNLeg_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLocMNLeg", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLocMNLeg", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("MNLeg", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNLeg_side").setAttribute("height", "255");
	doc.getElementById("MNLeg_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNLeg_side").src = "./PnlBrlyLocMNLeg_bside.html";
	doc.getElementById("MNLeg_cont").src = "./PnlBrlyLocMNLeg_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNLeg_side").contentDocument;
	contdoc = doc.getElementById("MNLeg_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNLeg_side").contentDocument;
	contdoc = doc.getElementById("MNLeg_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNLeg_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNLeg_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNLeg_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocMNLeg", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLocMNLeg", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLocMNLeg", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocMNLeg", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLocMNLeg", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocMNLeg", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNLeg", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNLeg", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocMNLeg", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocMNLeg", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocMNLeg", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNLeg", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNLeg", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNLeg", "TcoMrefWidth"));
		var TcoLp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNLeg", "TcoLp0Width"));
		var TcoLp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNLeg", "TcoLp1Width"));
		var TcoLphWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNLeg", "TcoLphWidth"));
		var TcoLthWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNLeg", "TcoLthWidth"));
		var TcoXp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNLeg", "TcoXp0Width"));
		var TcoXp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNLeg", "TcoXp1Width"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocMNLeg", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLocMNLeg", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLocMNLeg
		rownode = getRownode(srcdoc, "ListBrlyQLocMNLeg", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLocMNLeg", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLocMNLeg", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLocMNLeg", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNLeg", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNLeg_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNLeg_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNLeg_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNLeg_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocMNLeg", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNLeg", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocMNLeg", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLocMNLeg", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLocMNLeg", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocMNLegData", scrJref, "StgIacQryBrlyLocMNLeg");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocMNLeg", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNLeg", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLocMNLeg", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocMNLeg", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocMNLeg", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLocMNLegDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "ContInfBrlyLocMNLeg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "StatAppBrlyLocMNLeg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "StatShrBrlyLocMNLeg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "StgIacBrlyLocMNLeg", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "TagBrlyLocMNLeg", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "ListBrlyQLocMNLeg", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "StatAppQryBrlyLocMNLeg", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "StatShrQryBrlyLocMNLeg", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNLegData", "StgIacQryBrlyLocMNLeg", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLocMNLegData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLocMNLegData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNLeg");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLocMNLegData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
