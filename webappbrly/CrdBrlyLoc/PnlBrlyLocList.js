/**
  * \file PnlBrlyLocList.js
  * web client functionality for panel PnlBrlyLocList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyLocList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyLocList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyLocList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyLocList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLocList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyLocList", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLocList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoSrfWidth"));
		var TcoIcaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoIcaWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoTitWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoTypWidth"));
		var TcoRegWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoRegWidth"));
		var TcoEqpWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoEqpWidth"));
		var TcoAltWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoAltWidth"));
		var TcoTheWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoTheWidth"));
		var TcoPhiWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocList", "TcoPhiWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyLocList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoIca"); doc.widths.push(TcoIcaWidth); doc.fs.push("ica"); doc.tos.push(2); wtot += TcoIcaWidth;
			} else if (i == 3) {
				doc.cols.push("TcoTit"); doc.widths.push(TcoTitWidth); doc.fs.push("tit"); doc.tos.push(3); wtot += TcoTitWidth;
			} else if (i == 4) {
				doc.cols.push("TcoTyp"); doc.widths.push(TcoTypWidth); doc.fs.push("typ"); doc.tos.push(4); wtot += TcoTypWidth;
			} else if (i == 5) {
				doc.cols.push("TcoReg"); doc.widths.push(TcoRegWidth); doc.fs.push("reg"); doc.tos.push(5); wtot += TcoRegWidth;
			} else if (i == 6) {
				doc.cols.push("TcoEqp"); doc.widths.push(TcoEqpWidth); doc.fs.push("eqp"); doc.tos.push(6); wtot += TcoEqpWidth;
			} else if (i == 7) {
				doc.cols.push("TcoAlt"); doc.widths.push(TcoAltWidth); doc.fs.push("alt"); doc.tos.push(0); wtot += TcoAltWidth;
			} else if (i == 8) {
				doc.cols.push("TcoThe"); doc.widths.push(TcoTheWidth); doc.fs.push("the"); doc.tos.push(0); wtot += TcoTheWidth;
			} else if (i == 9) {
				doc.cols.push("TcoPhi"); doc.widths.push(TcoPhiWidth); doc.fs.push("phi"); doc.tos.push(0); wtot += TcoPhiWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyLocList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLocList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLocList
		rownode = getRownode(srcdoc, "ListBrlyQLocList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLocList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLocList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLocList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLocList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLocList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocListData", scrJref, "StgIacQryBrlyLocList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLocList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLocListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyLocList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyLocList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocListData", scrJref, "ContIacBrlyLocList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "ContIacBrlyLocList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "ContInfBrlyLocList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "StatShrBrlyLocList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "StgIacBrlyLocList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "TagBrlyLocList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "ListBrlyQLocList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "StatAppQryBrlyLocList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "StatShrQryBrlyLocList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocListData", "StgIacQryBrlyLocList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyLocListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLocListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLocListData") {
				mask = mergeDpchEngData(dom);
