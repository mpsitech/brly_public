/**
  * \file PnlBrlyLocEqpMNConnection.js
  * web client functionality for panel PnlBrlyLocEqpMNConnection
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLocEqpMNConnection", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLocEqpMNConnection", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("EqpMNConnection_side").src = "./PnlBrlyLocEqpMNConnection_aside.html";
	doc.getElementById("EqpMNConnection_cont").src = "./PnlBrlyLocEqpMNConnection_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLocEqpMNConnection", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLocEqpMNConnection", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("EqpMNConnection_side").src = "./PnlBrlyLocEqpMNConnection_bside.html";
	doc.getElementById("EqpMNConnection_cont").src = "./PnlBrlyLocEqpMNConnection_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLocEqpMNConnection", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLocEqpMNConnection", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEqpMNConnection", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqpMNConnection", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqpMNConnection", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEqpMNConnection", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEqpMNConnection", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEqpMNConnection", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqpMNConnection", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqpMNConnection", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocEqpMNConnection", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLocEqpMNConnection", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLocEqpMNConnection
		rownode = getRownode(srcdoc, "ListBrlyQLocEqpMNConnection", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLocEqpMNConnection", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLocEqpMNConnection", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEqpMNConnection", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqpMNConnection", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEqpMNConnection", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLocEqpMNConnection", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLocEqpMNConnection", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocEqpMNConnectionData", scrJref, "StgIacQryBrlyLocEqpMNConnection");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocEqpMNConnection", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqpMNConnection", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLocEqpMNConnection", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocEqpMNConnection", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEqpMNConnection", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLocEqpMNConnectionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "ContInfBrlyLocEqpMNConnection", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "StatAppBrlyLocEqpMNConnection", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "StatShrBrlyLocEqpMNConnection", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "StgIacBrlyLocEqpMNConnection", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "TagBrlyLocEqpMNConnection", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "ListBrlyQLocEqpMNConnection", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "StatAppQryBrlyLocEqpMNConnection", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "StatShrQryBrlyLocEqpMNConnection", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqpMNConnectionData", "StgIacQryBrlyLocEqpMNConnection", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLocEqpMNConnectionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLocEqpMNConnectionData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLocEqpMNConnectionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
