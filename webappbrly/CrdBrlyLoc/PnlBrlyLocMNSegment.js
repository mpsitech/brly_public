/**
  * \file PnlBrlyLocMNSegment.js
  * web client functionality for panel PnlBrlyLocMNSegment
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLocMNSegment", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLocMNSegment", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("MNSegment", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNSegment_side").setAttribute("height", "30");
	doc.getElementById("MNSegment_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNSegment_side").src = "./PnlBrlyLocMNSegment_aside.html";
	doc.getElementById("MNSegment_cont").src = "./PnlBrlyLocMNSegment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLocMNSegment", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLocMNSegment", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("MNSegment", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNSegment_side").setAttribute("height", "255");
	doc.getElementById("MNSegment_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNSegment_side").src = "./PnlBrlyLocMNSegment_bside.html";
	doc.getElementById("MNSegment_cont").src = "./PnlBrlyLocMNSegment_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNSegment_side").contentDocument;
	contdoc = doc.getElementById("MNSegment_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNSegment_side").contentDocument;
	contdoc = doc.getElementById("MNSegment_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNSegment_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNSegment_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNSegment_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocMNSegment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLocMNSegment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLocMNSegment", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocMNSegment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLocMNSegment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocMNSegment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNSegment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNSegment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocMNSegment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocMNSegment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocMNSegment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNSegment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNSegment", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNSegment", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNSegment", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNSegment", "TcoStoWidth"));
		var TcoSp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNSegment", "TcoSp0Width"));
		var TcoSp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNSegment", "TcoSp1Width"));
		var TcoSphWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNSegment", "TcoSphWidth"));
		var TcoSthWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocMNSegment", "TcoSthWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocMNSegment", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLocMNSegment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLocMNSegment
		rownode = getRownode(srcdoc, "ListBrlyQLocMNSegment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLocMNSegment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLocMNSegment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLocMNSegment", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNSegment", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNSegment_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNSegment_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNSegment_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNSegment_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocMNSegment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNSegment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocMNSegment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLocMNSegment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLocMNSegment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocMNSegmentData", scrJref, "StgIacQryBrlyLocMNSegment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocMNSegment", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocMNSegment", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLocMNSegment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocMNSegment", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocMNSegment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLocMNSegmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "ContInfBrlyLocMNSegment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "StatAppBrlyLocMNSegment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "StatShrBrlyLocMNSegment", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "StgIacBrlyLocMNSegment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "TagBrlyLocMNSegment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "ListBrlyQLocMNSegment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "StatAppQryBrlyLocMNSegment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "StatShrQryBrlyLocMNSegment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocMNSegmentData", "StgIacQryBrlyLocMNSegment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLocMNSegmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLocMNSegmentData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNSegment");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLocMNSegmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
