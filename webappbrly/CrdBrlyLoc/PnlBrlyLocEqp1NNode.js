/**
  * \file PnlBrlyLocEqp1NNode.js
  * web client functionality for panel PnlBrlyLocEqp1NNode
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLocEqp1NNode", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLocEqp1NNode", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("Eqp1NNode_side").src = "./PnlBrlyLocEqp1NNode_aside.html";
	doc.getElementById("Eqp1NNode_cont").src = "./PnlBrlyLocEqp1NNode_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLocEqp1NNode", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLocEqp1NNode", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("Eqp1NNode_side").src = "./PnlBrlyLocEqp1NNode_bside.html";
	doc.getElementById("Eqp1NNode_cont").src = "./PnlBrlyLocEqp1NNode_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocEqp1NNode", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLocEqp1NNode", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLocEqp1NNode", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocEqp1NNode", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLocEqp1NNode", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEqp1NNode", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqp1NNode", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqp1NNode", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEqp1NNode", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEqp1NNode", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEqp1NNode", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqp1NNode", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqp1NNode", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocEqp1NNode", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocEqp1NNode", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLocEqp1NNode", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLocEqp1NNode
		rownode = getRownode(srcdoc, "ListBrlyQLocEqp1NNode", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLocEqp1NNode", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLocEqp1NNode", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLocEqp1NNode", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEqp1NNode", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqp1NNode", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocEqp1NNode", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLocEqp1NNode", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLocEqp1NNode", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocEqp1NNodeData", scrJref, "StgIacQryBrlyLocEqp1NNode");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocEqp1NNode", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocEqp1NNode", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLocEqp1NNode", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocEqp1NNode", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocEqp1NNode", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLocEqp1NNodeDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "ContInfBrlyLocEqp1NNode", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "StatAppBrlyLocEqp1NNode", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "StgIacBrlyLocEqp1NNode", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "TagBrlyLocEqp1NNode", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "ListBrlyQLocEqp1NNode", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "StatAppQryBrlyLocEqp1NNode", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "StatShrQryBrlyLocEqp1NNode", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocEqp1NNodeData", "StgIacQryBrlyLocEqp1NNode", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLocEqp1NNodeData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLocEqp1NNodeData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLocEqp1NNodeData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
