/**
  * \file PnlBrlyLocProxOrgMNLocation.js
  * web client functionality for panel PnlBrlyLocProxOrgMNLocation
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLocProxOrgMNLocation", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLocProxOrgMNLocation", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("ProxOrgMNLocation", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ProxOrgMNLocation_side").setAttribute("height", "30");
	doc.getElementById("ProxOrgMNLocation_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ProxOrgMNLocation_side").src = "./PnlBrlyLocProxOrgMNLocation_aside.html";
	doc.getElementById("ProxOrgMNLocation_cont").src = "./PnlBrlyLocProxOrgMNLocation_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLocProxOrgMNLocation", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLocProxOrgMNLocation", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("ProxOrgMNLocation", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ProxOrgMNLocation_side").setAttribute("height", "255");
	doc.getElementById("ProxOrgMNLocation_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ProxOrgMNLocation_side").src = "./PnlBrlyLocProxOrgMNLocation_bside.html";
	doc.getElementById("ProxOrgMNLocation_cont").src = "./PnlBrlyLocProxOrgMNLocation_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ProxOrgMNLocation_side").contentDocument;
	contdoc = doc.getElementById("ProxOrgMNLocation_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ProxOrgMNLocation_side").contentDocument;
	contdoc = doc.getElementById("ProxOrgMNLocation_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ProxOrgMNLocation_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ProxOrgMNLocation_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ProxOrgMNLocation_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLocProxOrgMNLocation", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLocProxOrgMNLocation", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocProxOrgMNLocation", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocProxOrgMNLocation", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocProxOrgMNLocation", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocProxOrgMNLocation", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocProxOrgMNLocation", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocProxOrgMNLocation", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocProxOrgMNLocation", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocProxOrgMNLocation", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocProxOrgMNLocation", "TcoMrefWidth"));
		var TcoDphWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocProxOrgMNLocation", "TcoDphWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDph"); doc.widths.push(TcoDphWidth); doc.fs.push("dph"); wtot += TcoDphWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLocProxOrgMNLocation", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLocProxOrgMNLocation
		rownode = getRownode(srcdoc, "ListBrlyQLocProxOrgMNLocation", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLocProxOrgMNLocation", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLocProxOrgMNLocation", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ProxOrgMNLocation", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ProxOrgMNLocation_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ProxOrgMNLocation_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ProxOrgMNLocation_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ProxOrgMNLocation_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocProxOrgMNLocation", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocProxOrgMNLocation", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocProxOrgMNLocation", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLocProxOrgMNLocation", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLocProxOrgMNLocation", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocProxOrgMNLocationData", scrJref, "StgIacQryBrlyLocProxOrgMNLocation");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocProxOrgMNLocation", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocProxOrgMNLocation", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLocProxOrgMNLocation", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocProxOrgMNLocation", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocProxOrgMNLocation", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLocProxOrgMNLocationDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "ContInfBrlyLocProxOrgMNLocation", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "StatAppBrlyLocProxOrgMNLocation", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "StatShrBrlyLocProxOrgMNLocation", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "StgIacBrlyLocProxOrgMNLocation", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "TagBrlyLocProxOrgMNLocation", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "ListBrlyQLocProxOrgMNLocation", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "StatAppQryBrlyLocProxOrgMNLocation", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "StatShrQryBrlyLocProxOrgMNLocation", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocProxOrgMNLocationData", "StgIacQryBrlyLocProxOrgMNLocation", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLocProxOrgMNLocationData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLocProxOrgMNLocationData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ProxOrgMNLocation");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLocProxOrgMNLocationData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
