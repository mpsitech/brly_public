/**
  * \file PnlBrlyLocVisOrgMNLocation.js
  * web client functionality for panel PnlBrlyLocVisOrgMNLocation
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyLocVisOrgMNLocation", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyLocVisOrgMNLocation", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("VisOrgMNLocation", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("VisOrgMNLocation_side").setAttribute("height", "30");
	doc.getElementById("VisOrgMNLocation_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("VisOrgMNLocation_side").src = "./PnlBrlyLocVisOrgMNLocation_aside.html";
	doc.getElementById("VisOrgMNLocation_cont").src = "./PnlBrlyLocVisOrgMNLocation_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyLocVisOrgMNLocation", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyLocVisOrgMNLocation", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("VisOrgMNLocation", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("VisOrgMNLocation_side").setAttribute("height", "255");
	doc.getElementById("VisOrgMNLocation_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("VisOrgMNLocation_side").src = "./PnlBrlyLocVisOrgMNLocation_bside.html";
	doc.getElementById("VisOrgMNLocation_cont").src = "./PnlBrlyLocVisOrgMNLocation_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("VisOrgMNLocation_side").contentDocument;
	contdoc = doc.getElementById("VisOrgMNLocation_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("VisOrgMNLocation_side").contentDocument;
	contdoc = doc.getElementById("VisOrgMNLocation_cont").contentDocument;
	hdrdoc = contdoc.getElementById("VisOrgMNLocation_hdr").contentDocument;
	tbldoc = contdoc.getElementById("VisOrgMNLocation_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("VisOrgMNLocation_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocVisOrgMNLocation", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyLocVisOrgMNLocation", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyLocVisOrgMNLocation", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyLocVisOrgMNLocation", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyLocVisOrgMNLocation", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocVisOrgMNLocation", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocVisOrgMNLocation", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocVisOrgMNLocation", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocVisOrgMNLocation", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocVisOrgMNLocation", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocVisOrgMNLocation", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocVisOrgMNLocation", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocVisOrgMNLocation", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyLocVisOrgMNLocation", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyLocVisOrgMNLocation", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyLocVisOrgMNLocation", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQLocVisOrgMNLocation
		rownode = getRownode(srcdoc, "ListBrlyQLocVisOrgMNLocation", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyLocVisOrgMNLocation", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyLocVisOrgMNLocation", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyLocVisOrgMNLocation", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("VisOrgMNLocation", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("VisOrgMNLocation_side").setAttribute("height", "" + (height+55));
	doc.getElementById("VisOrgMNLocation_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("VisOrgMNLocation_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("VisOrgMNLocation_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocVisOrgMNLocation", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocVisOrgMNLocation", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyLocVisOrgMNLocation", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyLocVisOrgMNLocation", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyLocVisOrgMNLocation", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyLocVisOrgMNLocationData", scrJref, "StgIacQryBrlyLocVisOrgMNLocation");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocVisOrgMNLocation", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyLocVisOrgMNLocation", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyLocVisOrgMNLocation", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyLocVisOrgMNLocation", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyLocVisOrgMNLocation", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyLocVisOrgMNLocationDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "ContInfBrlyLocVisOrgMNLocation", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "StatAppBrlyLocVisOrgMNLocation", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "StatShrBrlyLocVisOrgMNLocation", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "StgIacBrlyLocVisOrgMNLocation", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "TagBrlyLocVisOrgMNLocation", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "ListBrlyQLocVisOrgMNLocation", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "StatAppQryBrlyLocVisOrgMNLocation", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "StatShrQryBrlyLocVisOrgMNLocation", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyLocVisOrgMNLocationData", "StgIacQryBrlyLocVisOrgMNLocation", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyLocVisOrgMNLocationData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyLocVisOrgMNLocationData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("VisOrgMNLocation");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyLocVisOrgMNLocationData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
