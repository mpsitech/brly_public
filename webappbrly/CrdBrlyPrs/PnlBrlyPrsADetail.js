/**
  * \file PnlBrlyPrsADetail.js
  * web client functionality for panel PnlBrlyPrsADetail
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyPrsADetail", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyPrsADetail", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("ADetail_side").src = "./PnlBrlyPrsADetail_aside.html";
	doc.getElementById("ADetail_cont").src = "./PnlBrlyPrsADetail_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyPrsADetail", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyPrsADetail", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("ADetail_side").src = "./PnlBrlyPrsADetail_bside.html";
	doc.getElementById("ADetail_cont").src = "./PnlBrlyPrsADetail_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyPrsADetail", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyPrsADetail", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyPrsADetail", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyPrsADetail", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyPrsADetail", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPrsADetail", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPrsADetail", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPrsADetail", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPrsADetail", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPrsADetail", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPrsADetail", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPrsADetail", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPrsADetail", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyPrsADetail", "TcoTypWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyPrsADetail", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyPrsADetail", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyPrsADetail", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQPrsADetail
		rownode = getRownode(srcdoc, "ListBrlyQPrsADetail", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyPrsADetail", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyPrsADetail", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyPrsADetail", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPrsADetail", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPrsADetail", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPrsADetail", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyPrsADetail", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyPrsADetail", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyPrsADetailData", scrJref, "StgIacQryBrlyPrsADetail");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyPrsADetail", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPrsADetail", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyPrsADetail", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyPrsADetail", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPrsADetail", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyPrsADetailDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "ContInfBrlyPrsADetail", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "StatAppBrlyPrsADetail", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "StatShrBrlyPrsADetail", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "StgIacBrlyPrsADetail", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "TagBrlyPrsADetail", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "ListBrlyQPrsADetail", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "StatAppQryBrlyPrsADetail", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "StatShrQryBrlyPrsADetail", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyPrsADetailData", "StgIacQryBrlyPrsADetail", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyPrsADetailData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyPrsADetailData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyPrsADetailData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
