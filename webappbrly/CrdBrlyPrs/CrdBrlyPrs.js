/**
  * \file CrdBrlyPrs.js
  * web client functionality for card CrdBrlyPrs
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppBrlyPrs", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppBrlyPrs", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppBrlyPrs", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppBrlyPrs", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppBrlyPrs", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlBrlyPrsHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlBrlyPrsList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlBrlyPrsRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppBrlyPrs", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppBrlyPrs", "shortMenu", short);
	setSi(srcdoc, "StatAppBrlyPrs", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppBrlyPrs", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppBrlyPrs", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagBrlyPrs", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagBrlyPrs", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdPrs"), retrieveCi(srcdoc, "ContInfBrlyPrs", "MtxCrdPrs"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfBrlyPrs", "MtxCrdPrs") + " - BeamRelay 0.2.2";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyPrsDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyPrsDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfBrlyPrs", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyPrsData", "ContInfBrlyPrs", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyPrsData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngBrlyPrsData", "StatAppBrlyPrs", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyPrsData", "StatShrBrlyPrs", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyPrsData", "TagBrlyPrs", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyPrsData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrBrlyPrs", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrBrlyPrs", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrBrlyPrs", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyPrsData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyPrs", "srefIxBrlyVReqitmode");

	if (srefIxBrlyVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxBrlyVReqitmode = "notify";
		setSi(srcdoc, "StatAppBrlyPrs", "srefIxBrlyVReqitmode", srefIxBrlyVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyPrs", "srefIxBrlyVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppBrlyPrs", "latency"));
