/**
  * \file CrdBrlyUsr.js
  * web client functionality for card CrdBrlyUsr
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppBrlyUsr", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppBrlyUsr", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppBrlyUsr", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppBrlyUsr", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppBrlyUsr", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlBrlyUsrHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlBrlyUsrList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlBrlyUsrRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppBrlyUsr", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppBrlyUsr", "shortMenu", short);
	setSi(srcdoc, "StatAppBrlyUsr", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppBrlyUsr", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppBrlyUsr", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagBrlyUsr", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagBrlyUsr", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdUsr"), retrieveCi(srcdoc, "ContInfBrlyUsr", "MtxCrdUsr"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfBrlyUsr", "MtxCrdUsr") + " - BeamRelay 0.2.2";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyUsrDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyUsrDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfBrlyUsr", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyUsrData", "ContInfBrlyUsr", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyUsrData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngBrlyUsrData", "StatAppBrlyUsr", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyUsrData", "StatShrBrlyUsr", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyUsrData", "TagBrlyUsr", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyUsrData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrBrlyUsr", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrBrlyUsr", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrBrlyUsr", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyUsrData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyUsr", "srefIxBrlyVReqitmode");

	if (srefIxBrlyVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxBrlyVReqitmode = "notify";
		setSi(srcdoc, "StatAppBrlyUsr", "srefIxBrlyVReqitmode", srefIxBrlyVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyUsr", "srefIxBrlyVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppBrlyUsr", "latency"));
