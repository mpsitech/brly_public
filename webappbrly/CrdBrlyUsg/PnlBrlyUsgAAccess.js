/**
  * \file PnlBrlyUsgAAccess.js
  * web client functionality for panel PnlBrlyUsgAAccess
...
function refreshBD(bNotD, chgcol) {
	...
	if (chgcol) {
		...
		var TcoCrdWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyUsgAAccess", "TcoCrdWidth"));
		var TcoUacWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyUsgAAccess", "TcoUacWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyUsgAAccess", doc.cols[i]));
