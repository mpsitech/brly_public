/**
  * \file PnlBrlyConList.js
  * web client functionality for panel PnlBrlyConList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyConList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyConList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyConList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyConList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyConList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyConList", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyConList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConList", "ncol"));

		var TcoFltWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConList", "TcoFltWidth"));
		var TcoCorWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConList", "TcoCorWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConList", "TcoStoWidth"));
		var TcoModWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConList", "TcoModWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyConList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoFlt"); doc.widths.push(TcoFltWidth); doc.fs.push("flt"); doc.tos.push(1); wtot += TcoFltWidth;
			} else if (i == 2) {
				...
				doc.cols.push("TcoMod"); doc.widths.push(TcoModWidth); doc.fs.push("mod"); doc.tos.push(0); wtot += TcoModWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyConList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyConList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQConList
		rownode = getRownode(srcdoc, "ListBrlyQConList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyConList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyConList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyConList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyConList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyConList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyConListData", scrJref, "StgIacQryBrlyConList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyConList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyConListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyConList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyConList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyConListData", scrJref, "ContIacBrlyConList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "ContIacBrlyConList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "ContInfBrlyConList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "StatShrBrlyConList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "StgIacBrlyConList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "TagBrlyConList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "ListBrlyQConList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "StatAppQryBrlyConList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "StatShrQryBrlyConList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyConListData", "StgIacQryBrlyConList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyConListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyConListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyConListData") {
				mask = mergeDpchEngData(dom);
