/**
  * \file PnlBrlyCon1NRelay.js
  * web client functionality for panel PnlBrlyCon1NRelay
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyCon1NRelay", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyCon1NRelay", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("1NRelay", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NRelay_side").setAttribute("height", "30");
	doc.getElementById("1NRelay_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NRelay_side").src = "./PnlBrlyCon1NRelay_aside.html";
	doc.getElementById("1NRelay_cont").src = "./PnlBrlyCon1NRelay_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyCon1NRelay", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyCon1NRelay", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("1NRelay", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NRelay_side").setAttribute("height", "255");
	doc.getElementById("1NRelay_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NRelay_side").src = "./PnlBrlyCon1NRelay_bside.html";
	doc.getElementById("1NRelay_cont").src = "./PnlBrlyCon1NRelay_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NRelay_side").contentDocument;
	contdoc = doc.getElementById("1NRelay_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NRelay_side").contentDocument;
	contdoc = doc.getElementById("1NRelay_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NRelay_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NRelay_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NRelay_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyCon1NRelay", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyCon1NRelay", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyCon1NRelay", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyCon1NRelay", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyCon1NRelay", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyCon1NRelay", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyCon1NRelay", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyCon1NRelay", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyCon1NRelay", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyCon1NRelay", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyCon1NRelay", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyCon1NRelay", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyCon1NRelay", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyCon1NRelay", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyCon1NRelay", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyCon1NRelay", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQCon1NRelay
		rownode = getRownode(srcdoc, "ListBrlyQCon1NRelay", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyCon1NRelay", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyCon1NRelay", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyCon1NRelay", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NRelay", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NRelay_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NRelay_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NRelay_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NRelay_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyCon1NRelay", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyCon1NRelay", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyCon1NRelay", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyCon1NRelay", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyCon1NRelay", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyCon1NRelayData", scrJref, "StgIacQryBrlyCon1NRelay");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyCon1NRelay", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyCon1NRelay", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyCon1NRelay", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyCon1NRelay", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyCon1NRelay", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyCon1NRelayDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "ContInfBrlyCon1NRelay", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "StatAppBrlyCon1NRelay", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "StatShrBrlyCon1NRelay", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "StgIacBrlyCon1NRelay", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "TagBrlyCon1NRelay", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "ListBrlyQCon1NRelay", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "StatAppQryBrlyCon1NRelay", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "StatShrQryBrlyCon1NRelay", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyCon1NRelayData", "StgIacQryBrlyCon1NRelay", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyCon1NRelayData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyCon1NRelayData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NRelay");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyCon1NRelayData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
