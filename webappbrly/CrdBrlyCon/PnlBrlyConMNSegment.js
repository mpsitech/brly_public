/**
  * \file PnlBrlyConMNSegment.js
  * web client functionality for panel PnlBrlyConMNSegment
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyConMNSegment", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyConMNSegment", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNSegment_side").src = "./PnlBrlyConMNSegment_aside.html";
	doc.getElementById("MNSegment_cont").src = "./PnlBrlyConMNSegment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyConMNSegment", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyConMNSegment", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNSegment_side").src = "./PnlBrlyConMNSegment_bside.html";
	doc.getElementById("MNSegment_cont").src = "./PnlBrlyConMNSegment_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConMNSegment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyConMNSegment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyConMNSegment", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConMNSegment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyConMNSegment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConMNSegment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNSegment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNSegment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConMNSegment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConMNSegment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConMNSegment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNSegment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNSegment", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNSegment", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNSegment", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNSegment", "TcoStoWidth"));
		var TcoLp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNSegment", "TcoLp0Width"));
		var TcoLp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNSegment", "TcoLp1Width"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyConMNSegment", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyConMNSegment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQConMNSegment
		rownode = getRownode(srcdoc, "ListBrlyQConMNSegment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyConMNSegment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyConMNSegment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyConMNSegment", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConMNSegment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNSegment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConMNSegment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyConMNSegment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyConMNSegment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyConMNSegmentData", scrJref, "StgIacQryBrlyConMNSegment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyConMNSegment", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNSegment", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyConMNSegment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyConMNSegment", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConMNSegment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyConMNSegmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "ContInfBrlyConMNSegment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "StatAppBrlyConMNSegment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "StatShrBrlyConMNSegment", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "StgIacBrlyConMNSegment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "TagBrlyConMNSegment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "ListBrlyQConMNSegment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "StatAppQryBrlyConMNSegment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "StatShrQryBrlyConMNSegment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyConMNSegmentData", "StgIacQryBrlyConMNSegment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyConMNSegmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyConMNSegmentData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyConMNSegmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
