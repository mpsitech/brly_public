/**
  * \file PnlBrlyConConMNEquipment.js
  * web client functionality for panel PnlBrlyConConMNEquipment
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyConConMNEquipment", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyConConMNEquipment", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("ConMNEquipment", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ConMNEquipment_side").setAttribute("height", "30");
	doc.getElementById("ConMNEquipment_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ConMNEquipment_side").src = "./PnlBrlyConConMNEquipment_aside.html";
	doc.getElementById("ConMNEquipment_cont").src = "./PnlBrlyConConMNEquipment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyConConMNEquipment", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyConConMNEquipment", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("ConMNEquipment", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ConMNEquipment_side").setAttribute("height", "255");
	doc.getElementById("ConMNEquipment_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ConMNEquipment_side").src = "./PnlBrlyConConMNEquipment_bside.html";
	doc.getElementById("ConMNEquipment_cont").src = "./PnlBrlyConConMNEquipment_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ConMNEquipment_side").contentDocument;
	contdoc = doc.getElementById("ConMNEquipment_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ConMNEquipment_side").contentDocument;
	contdoc = doc.getElementById("ConMNEquipment_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ConMNEquipment_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ConMNEquipment_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ConMNEquipment_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConConMNEquipment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyConConMNEquipment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyConConMNEquipment", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConConMNEquipment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyConConMNEquipment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConConMNEquipment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConConMNEquipment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConConMNEquipment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConConMNEquipment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConConMNEquipment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConConMNEquipment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConConMNEquipment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConConMNEquipment", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConConMNEquipment", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyConConMNEquipment", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyConConMNEquipment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQConConMNEquipment
		rownode = getRownode(srcdoc, "ListBrlyQConConMNEquipment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyConConMNEquipment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyConConMNEquipment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyConConMNEquipment", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ConMNEquipment", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ConMNEquipment_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ConMNEquipment_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ConMNEquipment_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ConMNEquipment_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConConMNEquipment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConConMNEquipment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConConMNEquipment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyConConMNEquipment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyConConMNEquipment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyConConMNEquipmentData", scrJref, "StgIacQryBrlyConConMNEquipment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyConConMNEquipment", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConConMNEquipment", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyConConMNEquipment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyConConMNEquipment", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConConMNEquipment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyConConMNEquipmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "ContInfBrlyConConMNEquipment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "StatAppBrlyConConMNEquipment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "StatShrBrlyConConMNEquipment", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "StgIacBrlyConConMNEquipment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "TagBrlyConConMNEquipment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "ListBrlyQConConMNEquipment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "StatAppQryBrlyConConMNEquipment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "StatShrQryBrlyConConMNEquipment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyConConMNEquipmentData", "StgIacQryBrlyConConMNEquipment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyConConMNEquipmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyConConMNEquipmentData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ConMNEquipment");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyConConMNEquipmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
