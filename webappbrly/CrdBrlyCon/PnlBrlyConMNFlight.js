/**
  * \file PnlBrlyConMNFlight.js
  * web client functionality for panel PnlBrlyConMNFlight
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyConMNFlight", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyConMNFlight", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNFlight_side").src = "./PnlBrlyConMNFlight_aside.html";
	doc.getElementById("MNFlight_cont").src = "./PnlBrlyConMNFlight_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyConMNFlight", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyConMNFlight", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNFlight_side").src = "./PnlBrlyConMNFlight_bside.html";
	doc.getElementById("MNFlight_cont").src = "./PnlBrlyConMNFlight_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConMNFlight", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyConMNFlight", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyConMNFlight", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConMNFlight", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyConMNFlight", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConMNFlight", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNFlight", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNFlight", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConMNFlight", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConMNFlight", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConMNFlight", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNFlight", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNFlight", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNFlight", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNFlight", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNFlight", "TcoStoWidth"));
		var TcoLp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNFlight", "TcoLp0Width"));
		var TcoLp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyConMNFlight", "TcoLp1Width"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyConMNFlight", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyConMNFlight", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQConMNFlight
		rownode = getRownode(srcdoc, "ListBrlyQConMNFlight", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyConMNFlight", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyConMNFlight", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyConMNFlight", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConMNFlight", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNFlight", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConMNFlight", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyConMNFlight", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyConMNFlight", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyConMNFlightData", scrJref, "StgIacQryBrlyConMNFlight");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyConMNFlight", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConMNFlight", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyConMNFlight", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyConMNFlight", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConMNFlight", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyConMNFlightDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "ContInfBrlyConMNFlight", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "StatAppBrlyConMNFlight", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "StatShrBrlyConMNFlight", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "StgIacBrlyConMNFlight", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "TagBrlyConMNFlight", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "ListBrlyQConMNFlight", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "StatAppQryBrlyConMNFlight", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "StatShrQryBrlyConMNFlight", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyConMNFlightData", "StgIacQryBrlyConMNFlight", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyConMNFlightData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyConMNFlightData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyConMNFlightData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
