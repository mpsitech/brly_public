/**
  * \file PnlBrlyConRef1NSegment.js
  * web client functionality for panel PnlBrlyConRef1NSegment
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyConRef1NSegment", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyConRef1NSegment", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NSegment_side").src = "./PnlBrlyConRef1NSegment_aside.html";
	doc.getElementById("Ref1NSegment_cont").src = "./PnlBrlyConRef1NSegment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyConRef1NSegment", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyConRef1NSegment", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NSegment_side").src = "./PnlBrlyConRef1NSegment_bside.html";
	doc.getElementById("Ref1NSegment_cont").src = "./PnlBrlyConRef1NSegment_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConRef1NSegment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyConRef1NSegment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyConRef1NSegment", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyConRef1NSegment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyConRef1NSegment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConRef1NSegment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConRef1NSegment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConRef1NSegment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConRef1NSegment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConRef1NSegment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConRef1NSegment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConRef1NSegment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConRef1NSegment", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyConRef1NSegment", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyConRef1NSegment", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyConRef1NSegment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQConRef1NSegment
		rownode = getRownode(srcdoc, "ListBrlyQConRef1NSegment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyConRef1NSegment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyConRef1NSegment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyConRef1NSegment", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConRef1NSegment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConRef1NSegment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyConRef1NSegment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyConRef1NSegment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyConRef1NSegment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyConRef1NSegmentData", scrJref, "StgIacQryBrlyConRef1NSegment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyConRef1NSegment", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyConRef1NSegment", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyConRef1NSegment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyConRef1NSegment", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyConRef1NSegment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyConRef1NSegmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "ContInfBrlyConRef1NSegment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "StatAppBrlyConRef1NSegment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "StatShrBrlyConRef1NSegment", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "StgIacBrlyConRef1NSegment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "TagBrlyConRef1NSegment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "ListBrlyQConRef1NSegment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "StatAppQryBrlyConRef1NSegment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "StatShrQryBrlyConRef1NSegment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyConRef1NSegmentData", "StgIacQryBrlyConRef1NSegment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyConRef1NSegmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyConRef1NSegmentData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyConRef1NSegmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
