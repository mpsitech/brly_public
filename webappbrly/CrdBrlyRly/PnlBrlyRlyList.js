/**
  * \file PnlBrlyRlyList.js
  * web client functionality for panel PnlBrlyRlyList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyRlyList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyRlyList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyRlyList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyRlyList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRlyList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyRlyList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyRlyList", "srefIxBrlyVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfBrlyRlyList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagBrlyRlyList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRlyList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyRlyList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRlyList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfBrlyRlyList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyRlyList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRlyList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRlyList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRlyList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRlyList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRlyList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRlyList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagBrlyRlyList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfBrlyRlyList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRlyList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRlyList", "ncol"));

		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRlyList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRlyList", "TcoStoWidth"));
		var TcoConWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRlyList", "TcoConWidth"));
		var TcoDirWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRlyList", "TcoDirWidth"));
		var TcoCtdWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRlyList", "TcoCtdWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyRlyList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoCon"); doc.widths.push(TcoConWidth); doc.fs.push("con"); doc.tos.push(3); wtot += TcoConWidth;
			} else if (i == 4) {
				doc.cols.push("TcoDir"); doc.widths.push(TcoDirWidth); doc.fs.push("dir"); doc.tos.push(4); wtot += TcoDirWidth;
			} else if (i == 5) {
				doc.cols.push("TcoCtd"); doc.widths.push(TcoCtdWidth); doc.fs.push("ctd"); doc.tos.push(0); wtot += TcoCtdWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyRlyList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyRlyList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQRlyList
		rownode = getRownode(srcdoc, "ListBrlyQRlyList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyRlyList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyRlyList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyRlyList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRlyList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRlyList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRlyList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyRlyList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyRlyList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyRlyListData", scrJref, "StgIacQryBrlyRlyList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRlyList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyRlyList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRlyList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyRlyListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyRlyList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyRlyList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyRlyListData", scrJref, "ContIacBrlyRlyList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "ContIacBrlyRlyList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "ContInfBrlyRlyList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "StatShrBrlyRlyList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "StgIacBrlyRlyList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "TagBrlyRlyList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "ListBrlyQRlyList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "StatAppQryBrlyRlyList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "StatShrQryBrlyRlyList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyRlyListData", "StgIacQryBrlyRlyList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyRlyListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyRlyListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyRlyListData") {
				mask = mergeDpchEngData(dom);
