/**
  * \file PnlBrlyRly1NNode.js
  * web client functionality for panel PnlBrlyRly1NNode
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyRly1NNode", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyRly1NNode", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NNode_side").src = "./PnlBrlyRly1NNode_aside.html";
	doc.getElementById("1NNode_cont").src = "./PnlBrlyRly1NNode_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyRly1NNode", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyRly1NNode", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NNode_side").src = "./PnlBrlyRly1NNode_bside.html";
	doc.getElementById("1NNode_cont").src = "./PnlBrlyRly1NNode_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRly1NNode", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyRly1NNode", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyRly1NNode", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRly1NNode", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyRly1NNode", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRly1NNode", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRly1NNode", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRly1NNode", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRly1NNode", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRly1NNode", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRly1NNode", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRly1NNode", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRly1NNode", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRly1NNode", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyRly1NNode", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyRly1NNode", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQRly1NNode
		rownode = getRownode(srcdoc, "ListBrlyQRly1NNode", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyRly1NNode", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyRly1NNode", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyRly1NNode", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRly1NNode", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRly1NNode", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRly1NNode", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyRly1NNode", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyRly1NNode", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyRly1NNodeData", scrJref, "StgIacQryBrlyRly1NNode");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyRly1NNode", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRly1NNode", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyRly1NNode", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyRly1NNode", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRly1NNode", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyRly1NNodeDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "ContInfBrlyRly1NNode", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "StatAppBrlyRly1NNode", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "StgIacBrlyRly1NNode", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "TagBrlyRly1NNode", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "ListBrlyQRly1NNode", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "StatAppQryBrlyRly1NNode", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "StatShrQryBrlyRly1NNode", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyRly1NNodeData", "StgIacQryBrlyRly1NNode", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyRly1NNodeData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyRly1NNodeData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyRly1NNodeData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
