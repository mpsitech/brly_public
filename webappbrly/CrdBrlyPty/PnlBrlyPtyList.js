/**
  * \file PnlBrlyPtyList.js
  * web client functionality for panel PnlBrlyPtyList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyPtyList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyPtyList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyPtyList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyPtyList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyPtyList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyPtyList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyPtyList", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyPtyList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyPtyList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPtyList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPtyList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPtyList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPtyList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPtyList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPtyList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPtyList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPtyList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyPtyList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyPtyList", "TcoTitWidth"));
		var TcoEtyWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyPtyList", "TcoEtyWidth"));
		var TcoCapWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyPtyList", "TcoCapWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyPtyList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoEty"); doc.widths.push(TcoEtyWidth); doc.fs.push("ety2"); doc.tos.push(0); wtot += TcoEtyWidth;
			} else if (i == 4) {
				doc.cols.push("TcoCap"); doc.widths.push(TcoCapWidth); doc.fs.push("cap"); doc.tos.push(0); wtot += TcoCapWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyPtyList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyPtyList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQPtyList
		rownode = getRownode(srcdoc, "ListBrlyQPtyList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyPtyList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyPtyList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyPtyList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPtyList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPtyList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPtyList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyPtyList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyPtyList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyPtyListData", scrJref, "StgIacQryBrlyPtyList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPtyList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyPtyList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPtyList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyPtyListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyPtyList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyPtyList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyPtyListData", scrJref, "ContIacBrlyPtyList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "ContIacBrlyPtyList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "ContInfBrlyPtyList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "StatShrBrlyPtyList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "StgIacBrlyPtyList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "TagBrlyPtyList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "ListBrlyQPtyList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "StatAppQryBrlyPtyList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "StatShrQryBrlyPtyList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyPtyListData", "StgIacQryBrlyPtyList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyPtyListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyPtyListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyPtyListData") {
				mask = mergeDpchEngData(dom);
