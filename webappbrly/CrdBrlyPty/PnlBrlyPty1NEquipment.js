/**
  * \file PnlBrlyPty1NEquipment.js
  * web client functionality for panel PnlBrlyPty1NEquipment
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyPty1NEquipment", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyPty1NEquipment", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NEquipment_side").src = "./PnlBrlyPty1NEquipment_aside.html";
	doc.getElementById("1NEquipment_cont").src = "./PnlBrlyPty1NEquipment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyPty1NEquipment", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyPty1NEquipment", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NEquipment_side").src = "./PnlBrlyPty1NEquipment_bside.html";
	doc.getElementById("1NEquipment_cont").src = "./PnlBrlyPty1NEquipment_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyPty1NEquipment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyPty1NEquipment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyPty1NEquipment", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyPty1NEquipment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyPty1NEquipment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPty1NEquipment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPty1NEquipment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPty1NEquipment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPty1NEquipment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPty1NEquipment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPty1NEquipment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPty1NEquipment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPty1NEquipment", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyPty1NEquipment", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyPty1NEquipment", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyPty1NEquipment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQPty1NEquipment
		rownode = getRownode(srcdoc, "ListBrlyQPty1NEquipment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyPty1NEquipment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyPty1NEquipment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyPty1NEquipment", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPty1NEquipment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPty1NEquipment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyPty1NEquipment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyPty1NEquipment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyPty1NEquipment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyPty1NEquipmentData", scrJref, "StgIacQryBrlyPty1NEquipment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyPty1NEquipment", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyPty1NEquipment", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyPty1NEquipment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyPty1NEquipment", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyPty1NEquipment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyPty1NEquipmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "ContInfBrlyPty1NEquipment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "StatAppBrlyPty1NEquipment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "StgIacBrlyPty1NEquipment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "TagBrlyPty1NEquipment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "ListBrlyQPty1NEquipment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "StatAppQryBrlyPty1NEquipment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "StatShrQryBrlyPty1NEquipment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyPty1NEquipmentData", "StgIacQryBrlyPty1NEquipment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyPty1NEquipmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyPty1NEquipmentData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyPty1NEquipmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
