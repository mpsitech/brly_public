/**
  * \file PnlBrlyReg1NLocation.js
  * web client functionality for panel PnlBrlyReg1NLocation
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyReg1NLocation", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyReg1NLocation", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("1NLocation", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NLocation_side").setAttribute("height", "30");
	doc.getElementById("1NLocation_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NLocation_side").src = "./PnlBrlyReg1NLocation_aside.html";
	doc.getElementById("1NLocation_cont").src = "./PnlBrlyReg1NLocation_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyReg1NLocation", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyReg1NLocation", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("1NLocation", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NLocation_side").setAttribute("height", "255");
	doc.getElementById("1NLocation_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NLocation_side").src = "./PnlBrlyReg1NLocation_bside.html";
	doc.getElementById("1NLocation_cont").src = "./PnlBrlyReg1NLocation_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NLocation_side").contentDocument;
	contdoc = doc.getElementById("1NLocation_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NLocation_side").contentDocument;
	contdoc = doc.getElementById("1NLocation_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NLocation_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NLocation_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NLocation_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyReg1NLocation", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyReg1NLocation", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyReg1NLocation", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyReg1NLocation", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyReg1NLocation", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyReg1NLocation", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyReg1NLocation", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyReg1NLocation", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyReg1NLocation", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyReg1NLocation", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyReg1NLocation", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyReg1NLocation", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyReg1NLocation", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyReg1NLocation", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyReg1NLocation", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyReg1NLocation", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQReg1NLocation
		rownode = getRownode(srcdoc, "ListBrlyQReg1NLocation", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyReg1NLocation", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyReg1NLocation", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyReg1NLocation", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NLocation", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NLocation_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NLocation_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NLocation_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NLocation_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyReg1NLocation", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyReg1NLocation", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyReg1NLocation", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyReg1NLocation", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyReg1NLocation", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyReg1NLocationData", scrJref, "StgIacQryBrlyReg1NLocation");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyReg1NLocation", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyReg1NLocation", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyReg1NLocation", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyReg1NLocation", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyReg1NLocation", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyReg1NLocationDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "ContInfBrlyReg1NLocation", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "StatAppBrlyReg1NLocation", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "StatShrBrlyReg1NLocation", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "StgIacBrlyReg1NLocation", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "TagBrlyReg1NLocation", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "ListBrlyQReg1NLocation", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "StatAppQryBrlyReg1NLocation", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "StatShrQryBrlyReg1NLocation", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyReg1NLocationData", "StgIacQryBrlyReg1NLocation", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyReg1NLocationData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyReg1NLocationData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NLocation");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyReg1NLocationData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
