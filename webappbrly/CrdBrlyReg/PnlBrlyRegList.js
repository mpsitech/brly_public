/**
  * \file PnlBrlyRegList.js
  * web client functionality for panel PnlBrlyRegList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyRegList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyRegList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyRegList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyRegList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRegList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyRegList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyRegList", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRegList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyRegList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRegList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRegList", "TcoTitWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRegList", "TcoSupWidth"));
		var TcoDstWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRegList", "TcoDstWidth"));
		var TcoTofWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRegList", "TcoTofWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyRegList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDst"); doc.widths.push(TcoDstWidth); doc.fs.push("dst"); doc.tos.push(0); wtot += TcoDstWidth;
			} else if (i == 5) {
				doc.cols.push("TcoTof"); doc.widths.push(TcoTofWidth); doc.fs.push("tof"); doc.tos.push(0); wtot += TcoTofWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyRegList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyRegList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQRegList
		rownode = getRownode(srcdoc, "ListBrlyQRegList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyRegList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyRegList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyRegList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyRegList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyRegList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyRegListData", scrJref, "StgIacQryBrlyRegList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyRegList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyRegListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyRegList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyRegList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyRegListData", scrJref, "ContIacBrlyRegList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "ContIacBrlyRegList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "ContInfBrlyRegList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "StatShrBrlyRegList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "StgIacBrlyRegList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "TagBrlyRegList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "ListBrlyQRegList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "StatAppQryBrlyRegList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "StatShrQryBrlyRegList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyRegListData", "StgIacQryBrlyRegList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyRegListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyRegListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyRegListData") {
				mask = mergeDpchEngData(dom);
