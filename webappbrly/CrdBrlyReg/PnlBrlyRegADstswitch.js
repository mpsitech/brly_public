/**
  * \file PnlBrlyRegADstswitch.js
  * web client functionality for panel PnlBrlyRegADstswitch
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyRegADstswitch", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyRegADstswitch", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("ADstswitch", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ADstswitch_side").setAttribute("height", "30");
	doc.getElementById("ADstswitch_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ADstswitch_side").src = "./PnlBrlyRegADstswitch_aside.html";
	doc.getElementById("ADstswitch_cont").src = "./PnlBrlyRegADstswitch_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyRegADstswitch", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyRegADstswitch", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("ADstswitch", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ADstswitch_side").setAttribute("height", "255");
	doc.getElementById("ADstswitch_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ADstswitch_side").src = "./PnlBrlyRegADstswitch_bside.html";
	doc.getElementById("ADstswitch_cont").src = "./PnlBrlyRegADstswitch_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ADstswitch_side").contentDocument;
	contdoc = doc.getElementById("ADstswitch_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ADstswitch_side").contentDocument;
	contdoc = doc.getElementById("ADstswitch_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ADstswitch_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ADstswitch_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ADstswitch_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRegADstswitch", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyRegADstswitch", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyRegADstswitch", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRegADstswitch", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyRegADstswitch", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegADstswitch", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegADstswitch", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegADstswitch", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegADstswitch", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegADstswitch", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegADstswitch", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegADstswitch", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegADstswitch", "ncol"));

		var TcoStdWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRegADstswitch", "TcoStdWidth"));
		var TcoSttWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRegADstswitch", "TcoSttWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoStd"); doc.widths.push(TcoStdWidth); doc.fs.push("std"); wtot += TcoStdWidth;
			} else if (i == 2) {
				doc.cols.push("TcoStt"); doc.widths.push(TcoSttWidth); doc.fs.push("stt"); wtot += TcoSttWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyRegADstswitch", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyRegADstswitch", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQRegADstswitch
		rownode = getRownode(srcdoc, "ListBrlyQRegADstswitch", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyRegADstswitch", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyRegADstswitch", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyRegADstswitch", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ADstswitch", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ADstswitch_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ADstswitch_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ADstswitch_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ADstswitch_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegADstswitch", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegADstswitch", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegADstswitch", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyRegADstswitch", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyRegADstswitch", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyRegADstswitchData", scrJref, "StgIacQryBrlyRegADstswitch");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyRegADstswitch", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegADstswitch", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyRegADstswitch", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyRegADstswitch", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegADstswitch", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyRegADstswitchDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "ContInfBrlyRegADstswitch", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "StatAppBrlyRegADstswitch", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "StatShrBrlyRegADstswitch", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "StgIacBrlyRegADstswitch", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "TagBrlyRegADstswitch", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "ListBrlyQRegADstswitch", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "StatAppQryBrlyRegADstswitch", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "StatShrQryBrlyRegADstswitch", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyRegADstswitchData", "StgIacQryBrlyRegADstswitch", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyRegADstswitchData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyRegADstswitchData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ADstswitch");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyRegADstswitchData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
