/**
  * \file CrdBrlyReg.js
  * web client functionality for card CrdBrlyReg
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppBrlyReg", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppBrlyReg", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppBrlyReg", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppBrlyReg", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppBrlyReg", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlBrlyRegHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlBrlyRegList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlBrlyRegRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppBrlyReg", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppBrlyReg", "shortMenu", short);
	setSi(srcdoc, "StatAppBrlyReg", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppBrlyReg", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppBrlyReg", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagBrlyReg", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagBrlyReg", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdReg"), retrieveCi(srcdoc, "ContInfBrlyReg", "MtxCrdReg"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfBrlyReg", "MtxCrdReg") + " - BeamRelay 0.2.2";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyRegDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyRegDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfBrlyReg", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyRegData", "ContInfBrlyReg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyRegData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngBrlyRegData", "StatAppBrlyReg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyRegData", "StatShrBrlyReg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyRegData", "TagBrlyReg", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyRegData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrBrlyReg", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrBrlyReg", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrBrlyReg", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyRegData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyReg", "srefIxBrlyVReqitmode");

	if (srefIxBrlyVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxBrlyVReqitmode = "notify";
		setSi(srcdoc, "StatAppBrlyReg", "srefIxBrlyVReqitmode", srefIxBrlyVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyReg", "srefIxBrlyVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppBrlyReg", "latency"));
