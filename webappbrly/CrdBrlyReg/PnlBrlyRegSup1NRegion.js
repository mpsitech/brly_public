/**
  * \file PnlBrlyRegSup1NRegion.js
  * web client functionality for panel PnlBrlyRegSup1NRegion
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyRegSup1NRegion", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyRegSup1NRegion", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("Sup1NRegion", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Sup1NRegion_side").setAttribute("height", "30");
	doc.getElementById("Sup1NRegion_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Sup1NRegion_side").src = "./PnlBrlyRegSup1NRegion_aside.html";
	doc.getElementById("Sup1NRegion_cont").src = "./PnlBrlyRegSup1NRegion_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyRegSup1NRegion", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyRegSup1NRegion", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("Sup1NRegion", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Sup1NRegion_side").setAttribute("height", "255");
	doc.getElementById("Sup1NRegion_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Sup1NRegion_side").src = "./PnlBrlyRegSup1NRegion_bside.html";
	doc.getElementById("Sup1NRegion_cont").src = "./PnlBrlyRegSup1NRegion_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NRegion_side").contentDocument;
	contdoc = doc.getElementById("Sup1NRegion_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Sup1NRegion_side").contentDocument;
	contdoc = doc.getElementById("Sup1NRegion_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NRegion_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NRegion_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NRegion_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyRegSup1NRegion", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyRegSup1NRegion", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegSup1NRegion", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegSup1NRegion", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegSup1NRegion", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegSup1NRegion", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegSup1NRegion", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegSup1NRegion", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegSup1NRegion", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegSup1NRegion", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyRegSup1NRegion", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyRegSup1NRegion", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQRegSup1NRegion
		rownode = getRownode(srcdoc, "ListBrlyQRegSup1NRegion", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyRegSup1NRegion", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyRegSup1NRegion", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NRegion", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NRegion_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NRegion_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NRegion_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NRegion_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegSup1NRegion", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegSup1NRegion", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyRegSup1NRegion", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyRegSup1NRegion", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyRegSup1NRegion", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyRegSup1NRegionData", scrJref, "StgIacQryBrlyRegSup1NRegion");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyRegSup1NRegion", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyRegSup1NRegion", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyRegSup1NRegion", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyRegSup1NRegion", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyRegSup1NRegion", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyRegSup1NRegionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "ContInfBrlyRegSup1NRegion", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "StatAppBrlyRegSup1NRegion", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "StatShrBrlyRegSup1NRegion", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "StgIacBrlyRegSup1NRegion", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "TagBrlyRegSup1NRegion", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "ListBrlyQRegSup1NRegion", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "StatAppQryBrlyRegSup1NRegion", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "StatShrQryBrlyRegSup1NRegion", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyRegSup1NRegionData", "StgIacQryBrlyRegSup1NRegion", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyRegSup1NRegionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyRegSup1NRegionData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NRegion");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyRegSup1NRegionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
