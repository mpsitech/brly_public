/**
  * \file PnlBrlySegMNConnection.js
  * web client functionality for panel PnlBrlySegMNConnection
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlySegMNConnection", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlySegMNConnection", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("MNConnection", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNConnection_side").setAttribute("height", "30");
	doc.getElementById("MNConnection_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNConnection_side").src = "./PnlBrlySegMNConnection_aside.html";
	doc.getElementById("MNConnection_cont").src = "./PnlBrlySegMNConnection_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlySegMNConnection", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlySegMNConnection", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("MNConnection", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNConnection_side").setAttribute("height", "255");
	doc.getElementById("MNConnection_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNConnection_side").src = "./PnlBrlySegMNConnection_bside.html";
	doc.getElementById("MNConnection_cont").src = "./PnlBrlySegMNConnection_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNConnection_side").contentDocument;
	contdoc = doc.getElementById("MNConnection_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNConnection_side").contentDocument;
	contdoc = doc.getElementById("MNConnection_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNConnection_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNConnection_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNConnection_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegMNConnection", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlySegMNConnection", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlySegMNConnection", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegMNConnection", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlySegMNConnection", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegMNConnection", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNConnection", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNConnection", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegMNConnection", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegMNConnection", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegMNConnection", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNConnection", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNConnection", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNConnection", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNConnection", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNConnection", "TcoStoWidth"));
		var TcoLp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNConnection", "TcoLp0Width"));
		var TcoLp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNConnection", "TcoLp1Width"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlySegMNConnection", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlySegMNConnection", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQSegMNConnection
		rownode = getRownode(srcdoc, "ListBrlyQSegMNConnection", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlySegMNConnection", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlySegMNConnection", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlySegMNConnection", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNConnection", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNConnection_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNConnection_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNConnection_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNConnection_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegMNConnection", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNConnection", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegMNConnection", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlySegMNConnection", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlySegMNConnection", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlySegMNConnectionData", scrJref, "StgIacQryBrlySegMNConnection");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlySegMNConnection", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNConnection", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlySegMNConnection", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlySegMNConnection", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegMNConnection", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlySegMNConnectionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "ContInfBrlySegMNConnection", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "StatAppBrlySegMNConnection", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "StatShrBrlySegMNConnection", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "StgIacBrlySegMNConnection", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "TagBrlySegMNConnection", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "ListBrlyQSegMNConnection", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "StatAppQryBrlySegMNConnection", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "StatShrQryBrlySegMNConnection", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlySegMNConnectionData", "StgIacQryBrlySegMNConnection", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlySegMNConnectionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlySegMNConnectionData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNConnection");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlySegMNConnectionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
