/**
  * \file PnlBrlySegMNLocation.js
  * web client functionality for panel PnlBrlySegMNLocation
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlySegMNLocation", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlySegMNLocation", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNLocation_side").src = "./PnlBrlySegMNLocation_aside.html";
	doc.getElementById("MNLocation_cont").src = "./PnlBrlySegMNLocation_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlySegMNLocation", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlySegMNLocation", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNLocation_side").src = "./PnlBrlySegMNLocation_bside.html";
	doc.getElementById("MNLocation_cont").src = "./PnlBrlySegMNLocation_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegMNLocation", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlySegMNLocation", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlySegMNLocation", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegMNLocation", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlySegMNLocation", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegMNLocation", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNLocation", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNLocation", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegMNLocation", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegMNLocation", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegMNLocation", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNLocation", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNLocation", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNLocation", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNLocation", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNLocation", "TcoStoWidth"));
		var TcoSp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNLocation", "TcoSp0Width"));
		var TcoSp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNLocation", "TcoSp1Width"));
		var TcoSphWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNLocation", "TcoSphWidth"));
		var TcoSthWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegMNLocation", "TcoSthWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoSp0"); doc.widths.push(TcoSp0Width); doc.fs.push("sp0"); wtot += TcoSp0Width;
			} else if (i == 5) {
				doc.cols.push("TcoSp1"); doc.widths.push(TcoSp1Width); doc.fs.push("sp1"); wtot += TcoSp1Width;
			} else if (i == 6) {
				doc.cols.push("TcoSph"); doc.widths.push(TcoSphWidth); doc.fs.push("sph"); wtot += TcoSphWidth;
			} else if (i == 7) {
				doc.cols.push("TcoSth"); doc.widths.push(TcoSthWidth); doc.fs.push("sth"); wtot += TcoSthWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlySegMNLocation", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlySegMNLocation", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQSegMNLocation
		rownode = getRownode(srcdoc, "ListBrlyQSegMNLocation", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlySegMNLocation", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlySegMNLocation", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlySegMNLocation", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegMNLocation", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNLocation", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegMNLocation", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlySegMNLocation", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlySegMNLocation", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlySegMNLocationData", scrJref, "StgIacQryBrlySegMNLocation");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlySegMNLocation", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegMNLocation", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlySegMNLocation", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlySegMNLocation", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegMNLocation", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlySegMNLocationDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "ContInfBrlySegMNLocation", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "StatAppBrlySegMNLocation", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "StatShrBrlySegMNLocation", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "StgIacBrlySegMNLocation", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "TagBrlySegMNLocation", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "ListBrlyQSegMNLocation", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "StatAppQryBrlySegMNLocation", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "StatShrQryBrlySegMNLocation", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlySegMNLocationData", "StgIacQryBrlySegMNLocation", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlySegMNLocationData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlySegMNLocationData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlySegMNLocationData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
