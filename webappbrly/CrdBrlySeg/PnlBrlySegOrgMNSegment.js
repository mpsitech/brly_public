/**
  * \file PnlBrlySegOrgMNSegment.js
  * web client functionality for panel PnlBrlySegOrgMNSegment
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlySegOrgMNSegment", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlySegOrgMNSegment", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("OrgMNSegment", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("OrgMNSegment_side").setAttribute("height", "30");
	doc.getElementById("OrgMNSegment_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("OrgMNSegment_side").src = "./PnlBrlySegOrgMNSegment_aside.html";
	doc.getElementById("OrgMNSegment_cont").src = "./PnlBrlySegOrgMNSegment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlySegOrgMNSegment", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlySegOrgMNSegment", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("OrgMNSegment", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("OrgMNSegment_side").setAttribute("height", "255");
	doc.getElementById("OrgMNSegment_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("OrgMNSegment_side").src = "./PnlBrlySegOrgMNSegment_bside.html";
	doc.getElementById("OrgMNSegment_cont").src = "./PnlBrlySegOrgMNSegment_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("OrgMNSegment_side").contentDocument;
	contdoc = doc.getElementById("OrgMNSegment_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("OrgMNSegment_side").contentDocument;
	contdoc = doc.getElementById("OrgMNSegment_cont").contentDocument;
	hdrdoc = contdoc.getElementById("OrgMNSegment_hdr").contentDocument;
	tbldoc = contdoc.getElementById("OrgMNSegment_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("OrgMNSegment_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegOrgMNSegment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlySegOrgMNSegment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlySegOrgMNSegment", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegOrgMNSegment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlySegOrgMNSegment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegOrgMNSegment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegOrgMNSegment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegOrgMNSegment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegOrgMNSegment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegOrgMNSegment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegOrgMNSegment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegOrgMNSegment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegOrgMNSegment", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegOrgMNSegment", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegOrgMNSegment", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegOrgMNSegment", "TcoStoWidth"));
		var TcoDp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegOrgMNSegment", "TcoDp0Width"));
		var TcoDp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegOrgMNSegment", "TcoDp1Width"));
		var TcoOp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegOrgMNSegment", "TcoOp0Width"));
		var TcoOp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegOrgMNSegment", "TcoOp1Width"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlySegOrgMNSegment", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlySegOrgMNSegment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQSegOrgMNSegment
		rownode = getRownode(srcdoc, "ListBrlyQSegOrgMNSegment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlySegOrgMNSegment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlySegOrgMNSegment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlySegOrgMNSegment", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("OrgMNSegment", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("OrgMNSegment_side").setAttribute("height", "" + (height+55));
	doc.getElementById("OrgMNSegment_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("OrgMNSegment_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("OrgMNSegment_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegOrgMNSegment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegOrgMNSegment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegOrgMNSegment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlySegOrgMNSegment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlySegOrgMNSegment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlySegOrgMNSegmentData", scrJref, "StgIacQryBrlySegOrgMNSegment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlySegOrgMNSegment", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegOrgMNSegment", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlySegOrgMNSegment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlySegOrgMNSegment", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegOrgMNSegment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlySegOrgMNSegmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "ContInfBrlySegOrgMNSegment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "StatAppBrlySegOrgMNSegment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "StatShrBrlySegOrgMNSegment", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "StgIacBrlySegOrgMNSegment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "TagBrlySegOrgMNSegment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "ListBrlyQSegOrgMNSegment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "StatAppQryBrlySegOrgMNSegment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "StatShrQryBrlySegOrgMNSegment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlySegOrgMNSegmentData", "StgIacQryBrlySegOrgMNSegment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlySegOrgMNSegmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlySegOrgMNSegmentData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("OrgMNSegment");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlySegOrgMNSegmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
