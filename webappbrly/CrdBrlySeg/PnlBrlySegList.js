/**
  * \file PnlBrlySegList.js
  * web client functionality for panel PnlBrlySegList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlySegList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlySegList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlySegList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlySegList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlySegList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlySegList", "srefIxBrlyVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfBrlySegList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagBrlySegList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlySegList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlySegList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfBrlySegList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryBrlySegList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagBrlySegList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfBrlySegList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegList", "ncol"));

		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoReuWidth"));
		var TcoAl0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoAl0Width"));
		var TcoAl1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoAl1Width"));
		var TcoTh0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoTh0Width"));
		var TcoTh1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoTh1Width"));
		var TcoPh0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoPh0Width"));
		var TcoPh1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoPh1Width"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlySegList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlySegList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoRet"); doc.widths.push(TcoRetWidth); doc.fs.push("ret"); doc.tos.push(1); wtot += TcoRetWidth;
			} else if (i == 2) {
				doc.cols.push("TcoReu"); doc.widths.push(TcoReuWidth); doc.fs.push("reu"); doc.tos.push(2); wtot += TcoReuWidth;
			} else if (i == 3) {
				doc.cols.push("TcoAl0"); doc.widths.push(TcoAl0Width); doc.fs.push("al0"); doc.tos.push(0); wtot += TcoAl0Width;
			} else if (i == 4) {
				doc.cols.push("TcoAl1"); doc.widths.push(TcoAl1Width); doc.fs.push("al1"); doc.tos.push(0); wtot += TcoAl1Width;
			} else if (i == 5) {
				doc.cols.push("TcoTh0"); doc.widths.push(TcoTh0Width); doc.fs.push("th0"); doc.tos.push(0); wtot += TcoTh0Width;
			} else if (i == 6) {
				doc.cols.push("TcoTh1"); doc.widths.push(TcoTh1Width); doc.fs.push("th1"); doc.tos.push(0); wtot += TcoTh1Width;
			} else if (i == 7) {
				doc.cols.push("TcoPh0"); doc.widths.push(TcoPh0Width); doc.fs.push("ph0"); doc.tos.push(0); wtot += TcoPh0Width;
			} else if (i == 8) {
				doc.cols.push("TcoPh1"); doc.widths.push(TcoPh1Width); doc.fs.push("ph1"); doc.tos.push(0); wtot += TcoPh1Width;
			} else if (i == 9) {
				doc.cols.push("TcoSta"); doc.widths.push(TcoStaWidth); doc.fs.push("sta"); doc.tos.push(3); wtot += TcoStaWidth;
			} else if (i == 10) {
				doc.cols.push("TcoSto"); doc.widths.push(TcoStoWidth); doc.fs.push("sto"); doc.tos.push(4); wtot += TcoStoWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlySegList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlySegList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQSegList
		rownode = getRownode(srcdoc, "ListBrlyQSegList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlySegList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlySegList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlySegList", "numFCsiQst"));
...
function refresh(chgcol) {
	...
	else if (srefIxBrlyVExpstate == "regd") refreshB(chgcol);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlySegList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlySegList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlySegList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlySegListData", scrJref, "StgIacQryBrlySegList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlySegList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlySegList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlySegList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlySegListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlySegList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlySegList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlySegListData", scrJref, "ContIacBrlySegList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "ContIacBrlySegList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "ContInfBrlySegList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "StatShrBrlySegList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "StgIacBrlySegList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "TagBrlySegList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "ListBrlyQSegList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "StatAppQryBrlySegList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "StatShrQryBrlySegList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlySegListData", "StgIacQryBrlySegList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlySegListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlySegListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlySegListData") {
				mask = mergeDpchEngData(dom);
