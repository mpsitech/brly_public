/**
  * \file PnlBrlyOpr1NEquipment.js
  * web client functionality for panel PnlBrlyOpr1NEquipment
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyOpr1NEquipment", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyOpr1NEquipment", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("1NEquipment", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NEquipment_side").setAttribute("height", "30");
	doc.getElementById("1NEquipment_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NEquipment_side").src = "./PnlBrlyOpr1NEquipment_aside.html";
	doc.getElementById("1NEquipment_cont").src = "./PnlBrlyOpr1NEquipment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyOpr1NEquipment", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyOpr1NEquipment", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("1NEquipment", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NEquipment_side").setAttribute("height", "255");
	doc.getElementById("1NEquipment_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NEquipment_side").src = "./PnlBrlyOpr1NEquipment_bside.html";
	doc.getElementById("1NEquipment_cont").src = "./PnlBrlyOpr1NEquipment_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NEquipment_side").contentDocument;
	contdoc = doc.getElementById("1NEquipment_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NEquipment_side").contentDocument;
	contdoc = doc.getElementById("1NEquipment_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NEquipment_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NEquipment_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NEquipment_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyOpr1NEquipment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyOpr1NEquipment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyOpr1NEquipment", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyOpr1NEquipment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyOpr1NEquipment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyOpr1NEquipment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyOpr1NEquipment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyOpr1NEquipment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyOpr1NEquipment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyOpr1NEquipment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyOpr1NEquipment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyOpr1NEquipment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyOpr1NEquipment", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyOpr1NEquipment", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyOpr1NEquipment", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyOpr1NEquipment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQOpr1NEquipment
		rownode = getRownode(srcdoc, "ListBrlyQOpr1NEquipment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyOpr1NEquipment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyOpr1NEquipment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyOpr1NEquipment", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NEquipment", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NEquipment_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NEquipment_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NEquipment_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NEquipment_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyOpr1NEquipment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyOpr1NEquipment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyOpr1NEquipment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyOpr1NEquipment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyOpr1NEquipment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyOpr1NEquipmentData", scrJref, "StgIacQryBrlyOpr1NEquipment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyOpr1NEquipment", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyOpr1NEquipment", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyOpr1NEquipment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyOpr1NEquipment", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyOpr1NEquipment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyOpr1NEquipmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "ContInfBrlyOpr1NEquipment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "StatAppBrlyOpr1NEquipment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "StatShrBrlyOpr1NEquipment", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "StgIacBrlyOpr1NEquipment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "TagBrlyOpr1NEquipment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "ListBrlyQOpr1NEquipment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "StatAppQryBrlyOpr1NEquipment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "StatShrQryBrlyOpr1NEquipment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyOpr1NEquipmentData", "StgIacQryBrlyOpr1NEquipment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyOpr1NEquipmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyOpr1NEquipmentData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NEquipment");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyOpr1NEquipmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
