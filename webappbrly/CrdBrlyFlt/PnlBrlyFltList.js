/**
  * \file PnlBrlyFltList.js
  * web client functionality for panel PnlBrlyFltList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyFltList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyFltList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyFltList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyFltList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFltList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyFltList", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFltList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltList", "TcoSrfWidth"));
		var TcoEqpWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltList", "TcoEqpWidth"));
		var TcoLegWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltList", "TcoLegWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyFltList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoEqp"); doc.widths.push(TcoEqpWidth); doc.fs.push("eqp"); doc.tos.push(2); wtot += TcoEqpWidth;
			} else if (i == 3) {
				doc.cols.push("TcoLeg"); doc.widths.push(TcoLegWidth); doc.fs.push("leg"); doc.tos.push(3); wtot += TcoLegWidth;
			} else if (i == 4) {
				doc.cols.push("TcoSta"); doc.widths.push(TcoStaWidth); doc.fs.push("sta"); doc.tos.push(4); wtot += TcoStaWidth;
			} else if (i == 5) {
				doc.cols.push("TcoSto"); doc.widths.push(TcoStoWidth); doc.fs.push("sto"); doc.tos.push(5); wtot += TcoStoWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyFltList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFltList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFltList
		rownode = getRownode(srcdoc, "ListBrlyQFltList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFltList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFltList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFltList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFltList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFltList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFltListData", scrJref, "StgIacQryBrlyFltList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFltList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyFltList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyFltList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFltListData", scrJref, "ContIacBrlyFltList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "ContIacBrlyFltList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "ContInfBrlyFltList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "StatShrBrlyFltList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "StgIacBrlyFltList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "TagBrlyFltList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "ListBrlyQFltList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "StatAppQryBrlyFltList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "StatShrQryBrlyFltList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltListData", "StgIacQryBrlyFltList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyFltListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFltListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFltListData") {
				mask = mergeDpchEngData(dom);
