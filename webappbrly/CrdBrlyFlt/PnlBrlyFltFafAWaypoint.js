/**
  * \file PnlBrlyFltFafAWaypoint.js
  * web client functionality for panel PnlBrlyFltFafAWaypoint
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyFltFafAWaypoint", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyFltFafAWaypoint", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("FafAWaypoint", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("FafAWaypoint_side").setAttribute("height", "30");
	doc.getElementById("FafAWaypoint_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("FafAWaypoint_side").src = "./PnlBrlyFltFafAWaypoint_aside.html";
	doc.getElementById("FafAWaypoint_cont").src = "./PnlBrlyFltFafAWaypoint_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyFltFafAWaypoint", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyFltFafAWaypoint", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("FafAWaypoint", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("FafAWaypoint_side").setAttribute("height", "255");
	doc.getElementById("FafAWaypoint_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("FafAWaypoint_side").src = "./PnlBrlyFltFafAWaypoint_bside.html";
	doc.getElementById("FafAWaypoint_cont").src = "./PnlBrlyFltFafAWaypoint_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("FafAWaypoint_side").contentDocument;
	contdoc = doc.getElementById("FafAWaypoint_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("FafAWaypoint_side").contentDocument;
	contdoc = doc.getElementById("FafAWaypoint_cont").contentDocument;
	hdrdoc = contdoc.getElementById("FafAWaypoint_hdr").contentDocument;
	tbldoc = contdoc.getElementById("FafAWaypoint_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("FafAWaypoint_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltFafAWaypoint", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFltFafAWaypoint", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyFltFafAWaypoint", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltFafAWaypoint", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFltFafAWaypoint", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltFafAWaypoint", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltFafAWaypoint", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltFafAWaypoint", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltFafAWaypoint", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltFafAWaypoint", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltFafAWaypoint", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltFafAWaypoint", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltFafAWaypoint", "ncol"));

		var TcoTmsWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltFafAWaypoint", "TcoTmsWidth"));
		var TcoLatWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltFafAWaypoint", "TcoLatWidth"));
		var TcoLonWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltFafAWaypoint", "TcoLonWidth"));
		var TcoGspWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltFafAWaypoint", "TcoGspWidth"));
		var TcoAltWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltFafAWaypoint", "TcoAltWidth"));
		var TcoAstWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltFafAWaypoint", "TcoAstWidth"));
		var TcoUtyWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltFafAWaypoint", "TcoUtyWidth"));
		var TcoAchWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltFafAWaypoint", "TcoAchWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoTms"); doc.widths.push(TcoTmsWidth); doc.fs.push("tms"); wtot += TcoTmsWidth;
			} else if (i == 2) {
				doc.cols.push("TcoLat"); doc.widths.push(TcoLatWidth); doc.fs.push("lat"); wtot += TcoLatWidth;
			} else if (i == 3) {
				doc.cols.push("TcoLon"); doc.widths.push(TcoLonWidth); doc.fs.push("lon"); wtot += TcoLonWidth;
			} else if (i == 4) {
				doc.cols.push("TcoGsp"); doc.widths.push(TcoGspWidth); doc.fs.push("gsp"); wtot += TcoGspWidth;
			} else if (i == 5) {
				doc.cols.push("TcoAlt"); doc.widths.push(TcoAltWidth); doc.fs.push("alt"); wtot += TcoAltWidth;
			} else if (i == 6) {
				doc.cols.push("TcoAst"); doc.widths.push(TcoAstWidth); doc.fs.push("ast"); wtot += TcoAstWidth;
			} else if (i == 7) {
				doc.cols.push("TcoUty"); doc.widths.push(TcoUtyWidth); doc.fs.push("uty"); wtot += TcoUtyWidth;
			} else if (i == 8) {
				doc.cols.push("TcoAch"); doc.widths.push(TcoAchWidth); doc.fs.push("ach"); wtot += TcoAchWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFltFafAWaypoint", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFltFafAWaypoint", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFltFafAWaypoint
		rownode = getRownode(srcdoc, "ListBrlyQFltFafAWaypoint", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFltFafAWaypoint", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFltFafAWaypoint", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFltFafAWaypoint", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("FafAWaypoint", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("FafAWaypoint_side").setAttribute("height", "" + (height+55));
	doc.getElementById("FafAWaypoint_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("FafAWaypoint_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("FafAWaypoint_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltFafAWaypoint", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltFafAWaypoint", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltFafAWaypoint", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFltFafAWaypoint", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFltFafAWaypoint", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFltFafAWaypointData", scrJref, "StgIacQryBrlyFltFafAWaypoint");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltFafAWaypoint", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltFafAWaypoint", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFltFafAWaypoint", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltFafAWaypoint", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltFafAWaypoint", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltFafAWaypointDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "ContInfBrlyFltFafAWaypoint", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "StatAppBrlyFltFafAWaypoint", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "StatShrBrlyFltFafAWaypoint", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "StgIacBrlyFltFafAWaypoint", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "TagBrlyFltFafAWaypoint", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "ListBrlyQFltFafAWaypoint", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "StatAppQryBrlyFltFafAWaypoint", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "StatShrQryBrlyFltFafAWaypoint", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltFafAWaypointData", "StgIacQryBrlyFltFafAWaypoint", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyFltFafAWaypointData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFltFafAWaypointData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("FafAWaypoint");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFltFafAWaypointData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
