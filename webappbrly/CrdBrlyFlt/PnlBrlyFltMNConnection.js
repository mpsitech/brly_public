/**
  * \file PnlBrlyFltMNConnection.js
  * web client functionality for panel PnlBrlyFltMNConnection
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyFltMNConnection", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyFltMNConnection", "srefIxBrlyVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNConnection_side").src = "./PnlBrlyFltMNConnection_aside.html";
	doc.getElementById("MNConnection_cont").src = "./PnlBrlyFltMNConnection_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyFltMNConnection", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyFltMNConnection", "srefIxBrlyVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNConnection_side").src = "./PnlBrlyFltMNConnection_bside.html";
	doc.getElementById("MNConnection_cont").src = "./PnlBrlyFltMNConnection_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltMNConnection", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFltMNConnection", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyFltMNConnection", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltMNConnection", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFltMNConnection", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltMNConnection", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltMNConnection", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltMNConnection", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltMNConnection", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltMNConnection", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltMNConnection", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltMNConnection", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltMNConnection", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltMNConnection", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltMNConnection", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltMNConnection", "TcoStoWidth"));
		var TcoLp0Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltMNConnection", "TcoLp0Width"));
		var TcoLp1Width = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltMNConnection", "TcoLp1Width"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFltMNConnection", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFltMNConnection", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFltMNConnection
		rownode = getRownode(srcdoc, "ListBrlyQFltMNConnection", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFltMNConnection", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFltMNConnection", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFltMNConnection", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltMNConnection", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltMNConnection", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltMNConnection", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFltMNConnection", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFltMNConnection", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFltMNConnectionData", scrJref, "StgIacQryBrlyFltMNConnection");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltMNConnection", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltMNConnection", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFltMNConnection", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltMNConnection", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltMNConnection", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltMNConnectionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "ContInfBrlyFltMNConnection", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "StatAppBrlyFltMNConnection", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "StatShrBrlyFltMNConnection", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "StgIacBrlyFltMNConnection", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "TagBrlyFltMNConnection", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "ListBrlyQFltMNConnection", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "StatAppQryBrlyFltMNConnection", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "StatShrQryBrlyFltMNConnection", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltMNConnectionData", "StgIacQryBrlyFltMNConnection", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyFltMNConnectionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFltMNConnectionData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFltMNConnectionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
