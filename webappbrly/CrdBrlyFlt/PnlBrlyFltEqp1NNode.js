/**
  * \file PnlBrlyFltEqp1NNode.js
  * web client functionality for panel PnlBrlyFltEqp1NNode
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyFltEqp1NNode", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyFltEqp1NNode", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("Eqp1NNode", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Eqp1NNode_side").setAttribute("height", "30");
	doc.getElementById("Eqp1NNode_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Eqp1NNode_side").src = "./PnlBrlyFltEqp1NNode_aside.html";
	doc.getElementById("Eqp1NNode_cont").src = "./PnlBrlyFltEqp1NNode_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyFltEqp1NNode", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyFltEqp1NNode", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("Eqp1NNode", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Eqp1NNode_side").setAttribute("height", "255");
	doc.getElementById("Eqp1NNode_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Eqp1NNode_side").src = "./PnlBrlyFltEqp1NNode_bside.html";
	doc.getElementById("Eqp1NNode_cont").src = "./PnlBrlyFltEqp1NNode_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Eqp1NNode_side").contentDocument;
	contdoc = doc.getElementById("Eqp1NNode_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Eqp1NNode_side").contentDocument;
	contdoc = doc.getElementById("Eqp1NNode_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Eqp1NNode_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Eqp1NNode_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Eqp1NNode_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltEqp1NNode", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFltEqp1NNode", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyFltEqp1NNode", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltEqp1NNode", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFltEqp1NNode", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltEqp1NNode", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqp1NNode", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqp1NNode", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltEqp1NNode", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltEqp1NNode", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltEqp1NNode", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqp1NNode", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqp1NNode", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltEqp1NNode", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFltEqp1NNode", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFltEqp1NNode", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFltEqp1NNode
		rownode = getRownode(srcdoc, "ListBrlyQFltEqp1NNode", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFltEqp1NNode", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFltEqp1NNode", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFltEqp1NNode", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Eqp1NNode", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Eqp1NNode_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Eqp1NNode_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Eqp1NNode_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Eqp1NNode_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltEqp1NNode", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqp1NNode", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltEqp1NNode", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFltEqp1NNode", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFltEqp1NNode", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFltEqp1NNodeData", scrJref, "StgIacQryBrlyFltEqp1NNode");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltEqp1NNode", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqp1NNode", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFltEqp1NNode", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltEqp1NNode", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltEqp1NNode", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltEqp1NNodeDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "ContInfBrlyFltEqp1NNode", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "StatAppBrlyFltEqp1NNode", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "StgIacBrlyFltEqp1NNode", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "TagBrlyFltEqp1NNode", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "ListBrlyQFltEqp1NNode", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "StatAppQryBrlyFltEqp1NNode", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "StatShrQryBrlyFltEqp1NNode", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqp1NNodeData", "StgIacQryBrlyFltEqp1NNode", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyFltEqp1NNodeData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFltEqp1NNodeData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Eqp1NNode");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFltEqp1NNodeData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
