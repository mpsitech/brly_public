/**
  * \file PnlBrlyFlt1NConnection.js
  * web client functionality for panel PnlBrlyFlt1NConnection
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyFlt1NConnection", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyFlt1NConnection", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("1NConnection", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NConnection_side").setAttribute("height", "30");
	doc.getElementById("1NConnection_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NConnection_side").src = "./PnlBrlyFlt1NConnection_aside.html";
	doc.getElementById("1NConnection_cont").src = "./PnlBrlyFlt1NConnection_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyFlt1NConnection", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyFlt1NConnection", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("1NConnection", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NConnection_side").setAttribute("height", "255");
	doc.getElementById("1NConnection_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NConnection_side").src = "./PnlBrlyFlt1NConnection_bside.html";
	doc.getElementById("1NConnection_cont").src = "./PnlBrlyFlt1NConnection_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NConnection_side").contentDocument;
	contdoc = doc.getElementById("1NConnection_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NConnection_side").contentDocument;
	contdoc = doc.getElementById("1NConnection_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NConnection_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NConnection_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NConnection_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFlt1NConnection", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFlt1NConnection", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyFlt1NConnection", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFlt1NConnection", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFlt1NConnection", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFlt1NConnection", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFlt1NConnection", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFlt1NConnection", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFlt1NConnection", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFlt1NConnection", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFlt1NConnection", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFlt1NConnection", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFlt1NConnection", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFlt1NConnection", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFlt1NConnection", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFlt1NConnection", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFlt1NConnection
		rownode = getRownode(srcdoc, "ListBrlyQFlt1NConnection", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFlt1NConnection", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFlt1NConnection", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFlt1NConnection", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NConnection", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NConnection_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NConnection_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NConnection_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NConnection_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFlt1NConnection", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFlt1NConnection", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFlt1NConnection", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFlt1NConnection", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFlt1NConnection", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFlt1NConnectionData", scrJref, "StgIacQryBrlyFlt1NConnection");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyFlt1NConnection", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFlt1NConnection", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFlt1NConnection", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyFlt1NConnection", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFlt1NConnection", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFlt1NConnectionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "ContInfBrlyFlt1NConnection", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "StatAppBrlyFlt1NConnection", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "StatShrBrlyFlt1NConnection", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "StgIacBrlyFlt1NConnection", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "TagBrlyFlt1NConnection", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "ListBrlyQFlt1NConnection", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "StatAppQryBrlyFlt1NConnection", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "StatShrQryBrlyFlt1NConnection", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFlt1NConnectionData", "StgIacQryBrlyFlt1NConnection", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyFlt1NConnectionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFlt1NConnectionData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NConnection");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFlt1NConnectionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
