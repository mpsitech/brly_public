/**
  * \file PnlBrlyFltOrgMNFlight.js
  * web client functionality for panel PnlBrlyFltOrgMNFlight
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyFltOrgMNFlight", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyFltOrgMNFlight", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("OrgMNFlight", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("OrgMNFlight_side").setAttribute("height", "30");
	doc.getElementById("OrgMNFlight_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("OrgMNFlight_side").src = "./PnlBrlyFltOrgMNFlight_aside.html";
	doc.getElementById("OrgMNFlight_cont").src = "./PnlBrlyFltOrgMNFlight_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyFltOrgMNFlight", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyFltOrgMNFlight", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("OrgMNFlight", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("OrgMNFlight_side").setAttribute("height", "255");
	doc.getElementById("OrgMNFlight_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("OrgMNFlight_side").src = "./PnlBrlyFltOrgMNFlight_bside.html";
	doc.getElementById("OrgMNFlight_cont").src = "./PnlBrlyFltOrgMNFlight_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("OrgMNFlight_side").contentDocument;
	contdoc = doc.getElementById("OrgMNFlight_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("OrgMNFlight_side").contentDocument;
	contdoc = doc.getElementById("OrgMNFlight_cont").contentDocument;
	hdrdoc = contdoc.getElementById("OrgMNFlight_hdr").contentDocument;
	tbldoc = contdoc.getElementById("OrgMNFlight_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("OrgMNFlight_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltOrgMNFlight", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFltOrgMNFlight", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyFltOrgMNFlight", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltOrgMNFlight", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFltOrgMNFlight", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltOrgMNFlight", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltOrgMNFlight", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltOrgMNFlight", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltOrgMNFlight", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltOrgMNFlight", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltOrgMNFlight", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltOrgMNFlight", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltOrgMNFlight", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltOrgMNFlight", "TcoMrefWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltOrgMNFlight", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltOrgMNFlight", "TcoStoWidth"));
		var TcoXsaWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltOrgMNFlight", "TcoXsaWidth"));
		var TcoXsoWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltOrgMNFlight", "TcoXsoWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFltOrgMNFlight", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFltOrgMNFlight", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFltOrgMNFlight
		rownode = getRownode(srcdoc, "ListBrlyQFltOrgMNFlight", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFltOrgMNFlight", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFltOrgMNFlight", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFltOrgMNFlight", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("OrgMNFlight", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("OrgMNFlight_side").setAttribute("height", "" + (height+55));
	doc.getElementById("OrgMNFlight_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("OrgMNFlight_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("OrgMNFlight_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltOrgMNFlight", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltOrgMNFlight", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltOrgMNFlight", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFltOrgMNFlight", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFltOrgMNFlight", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFltOrgMNFlightData", scrJref, "StgIacQryBrlyFltOrgMNFlight");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltOrgMNFlight", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltOrgMNFlight", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFltOrgMNFlight", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltOrgMNFlight", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltOrgMNFlight", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltOrgMNFlightDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "ContInfBrlyFltOrgMNFlight", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "StatAppBrlyFltOrgMNFlight", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "StatShrBrlyFltOrgMNFlight", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "StgIacBrlyFltOrgMNFlight", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "TagBrlyFltOrgMNFlight", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "ListBrlyQFltOrgMNFlight", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "StatAppQryBrlyFltOrgMNFlight", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "StatShrQryBrlyFltOrgMNFlight", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltOrgMNFlightData", "StgIacQryBrlyFltOrgMNFlight", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyFltOrgMNFlightData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFltOrgMNFlightData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("OrgMNFlight");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFltOrgMNFlightData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
