/**
  * \file PnlBrlyFltRef1NSegment.js
  * web client functionality for panel PnlBrlyFltRef1NSegment
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyFltRef1NSegment", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyFltRef1NSegment", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NSegment", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NSegment_side").setAttribute("height", "30");
	doc.getElementById("Ref1NSegment_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NSegment_side").src = "./PnlBrlyFltRef1NSegment_aside.html";
	doc.getElementById("Ref1NSegment_cont").src = "./PnlBrlyFltRef1NSegment_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyFltRef1NSegment", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyFltRef1NSegment", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NSegment", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NSegment_side").setAttribute("height", "255");
	doc.getElementById("Ref1NSegment_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NSegment_side").src = "./PnlBrlyFltRef1NSegment_bside.html";
	doc.getElementById("Ref1NSegment_cont").src = "./PnlBrlyFltRef1NSegment_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NSegment_side").contentDocument;
	contdoc = doc.getElementById("Ref1NSegment_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NSegment_side").contentDocument;
	contdoc = doc.getElementById("Ref1NSegment_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NSegment_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NSegment_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NSegment_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltRef1NSegment", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFltRef1NSegment", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyFltRef1NSegment", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltRef1NSegment", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFltRef1NSegment", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltRef1NSegment", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltRef1NSegment", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltRef1NSegment", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltRef1NSegment", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltRef1NSegment", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltRef1NSegment", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltRef1NSegment", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltRef1NSegment", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltRef1NSegment", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFltRef1NSegment", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFltRef1NSegment", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFltRef1NSegment
		rownode = getRownode(srcdoc, "ListBrlyQFltRef1NSegment", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFltRef1NSegment", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFltRef1NSegment", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFltRef1NSegment", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NSegment", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NSegment_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NSegment_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NSegment_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NSegment_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltRef1NSegment", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltRef1NSegment", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltRef1NSegment", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFltRef1NSegment", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFltRef1NSegment", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFltRef1NSegmentData", scrJref, "StgIacQryBrlyFltRef1NSegment");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltRef1NSegment", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltRef1NSegment", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFltRef1NSegment", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltRef1NSegment", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltRef1NSegment", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltRef1NSegmentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "ContInfBrlyFltRef1NSegment", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "StatAppBrlyFltRef1NSegment", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "StatShrBrlyFltRef1NSegment", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "StgIacBrlyFltRef1NSegment", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "TagBrlyFltRef1NSegment", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "ListBrlyQFltRef1NSegment", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "StatAppQryBrlyFltRef1NSegment", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "StatShrQryBrlyFltRef1NSegment", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltRef1NSegmentData", "StgIacQryBrlyFltRef1NSegment", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyFltRef1NSegmentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFltRef1NSegmentData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NSegment");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFltRef1NSegmentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
