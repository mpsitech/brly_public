/**
  * \file PnlBrlyFltEqpMNConnection.js
  * web client functionality for panel PnlBrlyFltEqpMNConnection
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppBrlyFltEqpMNConnection", "srefIxBrlyVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppBrlyFltEqpMNConnection", "srefIxBrlyVExpstate", "mind");

	getRecwnd().changeHeight("EqpMNConnection", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("EqpMNConnection_side").setAttribute("height", "30");
	doc.getElementById("EqpMNConnection_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("EqpMNConnection_side").src = "./PnlBrlyFltEqpMNConnection_aside.html";
	doc.getElementById("EqpMNConnection_cont").src = "./PnlBrlyFltEqpMNConnection_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppBrlyFltEqpMNConnection", "srefIxBrlyVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppBrlyFltEqpMNConnection", "srefIxBrlyVExpstate", "regd");

	getRecwnd().changeHeight("EqpMNConnection", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("EqpMNConnection_side").setAttribute("height", "255");
	doc.getElementById("EqpMNConnection_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("EqpMNConnection_side").src = "./PnlBrlyFltEqpMNConnection_bside.html";
	doc.getElementById("EqpMNConnection_cont").src = "./PnlBrlyFltEqpMNConnection_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("EqpMNConnection_side").contentDocument;
	contdoc = doc.getElementById("EqpMNConnection_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("EqpMNConnection_side").contentDocument;
	contdoc = doc.getElementById("EqpMNConnection_cont").contentDocument;
	hdrdoc = contdoc.getElementById("EqpMNConnection_hdr").contentDocument;
	tbldoc = contdoc.getElementById("EqpMNConnection_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("EqpMNConnection_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatAppBrlyFltEqpMNConnection", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFltEqpMNConnection", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltEqpMNConnection", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqpMNConnection", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqpMNConnection", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltEqpMNConnection", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltEqpMNConnection", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltEqpMNConnection", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqpMNConnection", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqpMNConnection", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFltEqpMNConnection", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFltEqpMNConnection", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFltEqpMNConnection
		rownode = getRownode(srcdoc, "ListBrlyQFltEqpMNConnection", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFltEqpMNConnection", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFltEqpMNConnection", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("EqpMNConnection", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("EqpMNConnection_side").setAttribute("height", "" + (height+55));
	doc.getElementById("EqpMNConnection_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("EqpMNConnection_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("EqpMNConnection_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltEqpMNConnection", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqpMNConnection", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFltEqpMNConnection", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFltEqpMNConnection", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFltEqpMNConnection", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFltEqpMNConnectionData", scrJref, "StgIacQryBrlyFltEqpMNConnection");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltEqpMNConnection", "srefIxBrlyVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFltEqpMNConnection", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFltEqpMNConnection", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppBrlyFltEqpMNConnection", "srefIxBrlyVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFltEqpMNConnection", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltEqpMNConnectionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "ContInfBrlyFltEqpMNConnection", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "StatAppBrlyFltEqpMNConnection", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "StatShrBrlyFltEqpMNConnection", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "StgIacBrlyFltEqpMNConnection", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "TagBrlyFltEqpMNConnection", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "ListBrlyQFltEqpMNConnection", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "StatAppQryBrlyFltEqpMNConnection", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "StatShrQryBrlyFltEqpMNConnection", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFltEqpMNConnectionData", "StgIacQryBrlyFltEqpMNConnection", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngBrlyFltEqpMNConnectionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFltEqpMNConnectionData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("EqpMNConnection");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFltEqpMNConnectionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
