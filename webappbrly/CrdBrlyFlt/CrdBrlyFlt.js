/**
  * \file CrdBrlyFlt.js
  * web client functionality for card CrdBrlyFlt
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppBrlyFlt", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppBrlyFlt", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppBrlyFlt", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppBrlyFlt", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppBrlyFlt", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlBrlyFltHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlBrlyFltList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlBrlyFltRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppBrlyFlt", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppBrlyFlt", "shortMenu", short);
	setSi(srcdoc, "StatAppBrlyFlt", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppBrlyFlt", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppBrlyFlt", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagBrlyFlt", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagBrlyFlt", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdFlt"), retrieveCi(srcdoc, "ContInfBrlyFlt", "MtxCrdFlt"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfBrlyFlt", "MtxCrdFlt") + " - BeamRelay 0.2.2";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFltDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfBrlyFlt", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFltData", "ContInfBrlyFlt", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFltData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngBrlyFltData", "StatAppBrlyFlt", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngBrlyFltData", "StatShrBrlyFlt", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFltData", "TagBrlyFlt", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFltData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrBrlyFlt", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrBrlyFlt", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrBrlyFlt", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFltData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyFlt", "srefIxBrlyVReqitmode");

	if (srefIxBrlyVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxBrlyVReqitmode = "notify";
		setSi(srcdoc, "StatAppBrlyFlt", "srefIxBrlyVReqitmode", srefIxBrlyVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxBrlyVReqitmode = retrieveSi(srcdoc, "StatAppBrlyFlt", "srefIxBrlyVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppBrlyFlt", "latency"));
