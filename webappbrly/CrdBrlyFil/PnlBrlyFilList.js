/**
  * \file PnlBrlyFilList.js
  * web client functionality for panel PnlBrlyFilList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlBrlyFilList_aside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyFilList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlBrlyFilList_bside.html";
	doc.getElementById("List_cont").src = "./PnlBrlyFilList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFilList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagBrlyFilList", "Trs"));
...
function init() {
	var srefIxBrlyVExpstate = retrieveSi(srcdoc, "StatShrBrlyFilList", "srefIxBrlyVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagBrlyFilList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryBrlyFilList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFilList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFilList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFilList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFilList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFilList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFilList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFilList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFilList", "ncol"));

		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFilList", "TcoFnmWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFilList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFilList", "TcoReuWidth"));
		var TcoCntWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFilList", "TcoCntWidth"));
		var TcoMimWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFilList", "TcoMimWidth"));
		var TcoSizWidth = parseInt(retrieveSi(srcdoc, "StgIacBrlyFilList", "TcoSizWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyFilList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagBrlyFilList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryBrlyFilList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListBrlyQFilList
		rownode = getRownode(srcdoc, "ListBrlyQFilList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagBrlyFilList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagBrlyFilList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfBrlyFilList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFilList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFilList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryBrlyFilList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryBrlyFilList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryBrlyFilList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFilListData", scrJref, "StgIacQryBrlyFilList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryBrlyFilList", "firstcol"));

	setSi(srcdoc, "StatAppQryBrlyFilList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryBrlyFilList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppBrlyFilListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacBrlyFilList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacBrlyFilList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppBrlyFilListData", scrJref, "ContIacBrlyFilList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "ContIacBrlyFilList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "ContInfBrlyFilList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "StatShrBrlyFilList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "StgIacBrlyFilList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "TagBrlyFilList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "ListBrlyQFilList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "StatAppQryBrlyFilList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "StatShrQryBrlyFilList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngBrlyFilListData", "StgIacQryBrlyFilList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngBrlyFilListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngBrlyFilListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngBrlyFilListData") {
				mask = mergeDpchEngData(dom);
