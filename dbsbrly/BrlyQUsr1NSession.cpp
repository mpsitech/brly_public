...
void TblBrlyQUsr1NSession::insertRec(
			BrlyQUsr1NSession* rec
		) {
};
...
MyTblBrlyQUsr1NSession::MyTblBrlyQUsr1NSession() : TblBrlyQUsr1NSession(), MyTable() {
	stmtInsertRec = NULL;
	stmtUpdateRec = NULL;
...
bool MyTblBrlyQUsr1NSession::loadRecBySQL(
	...
	if (!dbresult) throw DbsException("DbsException / MySQL: error storing result! (TblBrlyQUsr1NSession / loadRecBySQL)\n");
...
ubigint MyTblBrlyQUsr1NSession::loadRstBySQL(
	...
	dbresult = mysql_store_result(dbs);
	if (!dbresult) throw DbsException("DbsException / MySQL: error storing result! (TblBrlyQUsr1NSession / loadRstBySQL)\n");
...
void MyTblBrlyQUsr1NSession::insertRec(
			BrlyQUsr1NSession* rec
		) {
	...
	if (mysql_stmt_bind_param(stmtInsertRec, bind)) throw DbsException("DbsException / MySQL: error binding to statement! (TblBrlyQUsr1NSession / stmtInsertRec)\n");
	if (mysql_stmt_execute(stmtInsertRec)) throw DbsException("DbsException / MySQL: error executing statement! (TblBrlyQUsr1NSession / stmtInsertRec)\n");
...
void MyTblBrlyQUsr1NSession::updateRec(
	...
	if (mysql_stmt_bind_param(stmtUpdateRec, bind)) throw DbsException("DbsException / MySQL: error binding to statement! (TblBrlyQUsr1NSession / stmtUpdateRec)\n");
	if (mysql_stmt_execute(stmtUpdateRec)) throw DbsException("DbsException / MySQL: error executing statement! (TblBrlyQUsr1NSession / stmtUpdateRec)\n");
};
...
void MyTblBrlyQUsr1NSession::removeRecByQref(
	...
	if (mysql_stmt_bind_param(stmtRemoveRecByQref, &bind)) throw DbsException("DbsException / MySQL: error binding to statement! (TblBrlyQUsr1NSession / stmtRemoveRecByQref)\n");
	if (mysql_stmt_execute(stmtRemoveRecByQref)) throw DbsException("DbsException / MySQL: error executing statement! (TblBrlyQUsr1NSession / stmtRemoveRecByQref)\n");
};
...
void MyTblBrlyQUsr1NSession::removeRstByJref(
	...
	if (mysql_stmt_bind_param(stmtRemoveRstByJref, &bind)) throw DbsException("DbsException / MySQL: error binding to statement! (TblBrlyQUsr1NSession / stmtRemoveRstByJref)\n");
	if (mysql_stmt_execute(stmtRemoveRstByJref)) throw DbsException("DbsException / MySQL: error executing statement! (TblBrlyQUsr1NSession / stmtRemoveRstByJref)\n");
};
...
PgTblBrlyQUsr1NSession::PgTblBrlyQUsr1NSession() : TblBrlyQUsr1NSession(), PgTable() {
};
...
void PgTblBrlyQUsr1NSession::initStatements() {
	...
	res = PQprepare(dbs, "TblBrlyQUsr1NSession_insertRec", "INSERT INTO TblBrlyQUsr1NSession (jref, jnum, ref) VALUES ($1,$2,$3) RETURNING qref", 3, NULL);
	if (PQresultStatus(res) != PGRES_COMMAND_OK) initStatementsErr(res);
	res = PQprepare(dbs, "TblBrlyQUsr1NSession_updateRec", "UPDATE TblBrlyQUsr1NSession SET jref = $1, jnum = $2, ref = $3 WHERE qref = $4", 4, NULL);
	if (PQresultStatus(res) != PGRES_COMMAND_OK) initStatementsErr(res);
	res = PQprepare(dbs, "TblBrlyQUsr1NSession_removeRecByQref", "DELETE FROM TblBrlyQUsr1NSession WHERE qref = $1", 1, NULL);
	if (PQresultStatus(res) != PGRES_COMMAND_OK) initStatementsErr(res);
	res = PQprepare(dbs, "TblBrlyQUsr1NSession_removeRstByJref", "DELETE FROM TblBrlyQUsr1NSession WHERE jref = $1", 1, NULL);
	if (PQresultStatus(res) != PGRES_COMMAND_OK) initStatementsErr(res);

	res = PQprepare(dbs, "TblBrlyQUsr1NSession_loadRecByQref", "SELECT qref, jref, jnum, ref FROM TblBrlyQUsr1NSession WHERE qref = $1", 1, NULL);
	if (PQresultStatus(res) != PGRES_COMMAND_OK) initStatementsErr(res);
	res = PQprepare(dbs, "TblBrlyQUsr1NSession_loadRstByJref", "SELECT qref, jref, jnum, ref FROM TblBrlyQUsr1NSession WHERE jref = $1 ORDER BY jnum ASC", 1, NULL);
	if (PQresultStatus(res) != PGRES_COMMAND_OK) initStatementsErr(res);
};
...
bool PgTblBrlyQUsr1NSession::loadRecByStmt(
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) throw DbsException("DbsException / PgSQL: error executing statement! (TblBrlyQUsr1NSession / " + srefStmt + ")\n");
...
void PgTblBrlyQUsr1NSession::insertRec(
			BrlyQUsr1NSession* rec
		) {
	PGresult* res;
	char* ptr;
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK) throw DbsException("DbsException / PgSQL: error executing statement! (TblBrlyQUsr1NSession_insertRec)\n");
...
void PgTblBrlyQUsr1NSession::updateRec(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) throw DbsException("DbsException / PgSQL: error executing statement! (TblBrlyQUsr1NSession_updateRec)\n");
...
void PgTblBrlyQUsr1NSession::removeRecByQref(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK)
		throw DbsException("DbsException / PgSQL: error executing statement! (TblBrlyQUsr1NSession_removeRecByQref)\n");
...
void PgTblBrlyQUsr1NSession::removeRstByJref(
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK)
		throw DbsException("DbsException / PgSQL: error executing statement! (TblBrlyQUsr1NSession_removeRstByJref)\n");
