/**
  ...
  * API code for job QryBrlyFltFafAWaypoint (implementation)
...
QryBrlyFltFafAWaypoint::StatApp::StatApp(
			const uint firstcol
...
bool QryBrlyFltFafAWaypoint::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryBrlyFltFafAWaypoint");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryBrlyFltFafAWaypoint";
...
set<uint> QryBrlyFltFafAWaypoint::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyFltFafAWaypoint::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryBrlyFltFafAWaypoint::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryBrlyFltFafAWaypoint");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryBrlyFltFafAWaypoint";
