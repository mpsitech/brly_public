/**
  * \file CrdBrlyUsr.cpp
  * API code for job CrdBrlyUsr (implementation)
...
bool CrdBrlyUsr::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyUsr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyUsr";
...
bool CrdBrlyUsr::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyUsr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyUsr";
...
set<uint> CrdBrlyUsr::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdBrlyUsr::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdBrlyUsr::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlyUsr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlyUsr";
...
bool CrdBrlyUsr::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyUsr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyUsr";
...
CrdBrlyUsr::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYUSRDO, scrJref) {
...
void CrdBrlyUsr::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyUsrDo");
...
CrdBrlyUsr::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYUSRDATA) {
...
void CrdBrlyUsr::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyUsrData");
	else
		basefound = checkXPath(docctx, basexpath);
