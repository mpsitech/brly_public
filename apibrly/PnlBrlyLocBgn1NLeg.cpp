/**
  * \file PnlBrlyLocBgn1NLeg.cpp
  * API code for job PnlBrlyLocBgn1NLeg (implementation)
...
bool PnlBrlyLocBgn1NLeg::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyLocBgn1NLeg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyLocBgn1NLeg";
...
PnlBrlyLocBgn1NLeg::StatApp::StatApp(
...
bool PnlBrlyLocBgn1NLeg::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyLocBgn1NLeg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyLocBgn1NLeg";
...
set<uint> PnlBrlyLocBgn1NLeg::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyLocBgn1NLeg::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyLocBgn1NLeg::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlyLocBgn1NLeg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlyLocBgn1NLeg";
...
bool PnlBrlyLocBgn1NLeg::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyLocBgn1NLeg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyLocBgn1NLeg";
...
PnlBrlyLocBgn1NLeg::DpchAppData::DpchAppData(
			...
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLOCBGN1NLEGDATA, scrJref) {
...
void PnlBrlyLocBgn1NLeg::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyLocBgn1NLegData");
...
PnlBrlyLocBgn1NLeg::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLOCBGN1NLEGDO, scrJref) {
...
void PnlBrlyLocBgn1NLeg::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyLocBgn1NLegDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlBrlyLocBgn1NLeg::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYLOCBGN1NLEGDATA) {
...
void PnlBrlyLocBgn1NLeg::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyLocBgn1NLegData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryBrlyLocBgn1NLeg::StatApp();
		statshrqry = QryBrlyLocBgn1NLeg::StatShr();
