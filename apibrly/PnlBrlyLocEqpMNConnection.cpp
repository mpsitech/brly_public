/**
  ...
  * API code for job PnlBrlyLocEqpMNConnection (implementation)
...
bool PnlBrlyLocEqpMNConnection::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyLocEqpMNConnection");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyLocEqpMNConnection";
...
PnlBrlyLocEqpMNConnection::StatApp::StatApp(
...
bool PnlBrlyLocEqpMNConnection::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyLocEqpMNConnection");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyLocEqpMNConnection";
...
set<uint> PnlBrlyLocEqpMNConnection::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyLocEqpMNConnection::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyLocEqpMNConnection::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlyLocEqpMNConnection");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlyLocEqpMNConnection";
...
PnlBrlyLocEqpMNConnection::Tag::Tag(
			const string& Cpt
			, const string& Trs
...
bool PnlBrlyLocEqpMNConnection::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyLocEqpMNConnection");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyLocEqpMNConnection";
...
PnlBrlyLocEqpMNConnection::DpchAppData::DpchAppData(
			...
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLOCEQPMNCONNECTIONDATA, scrJref) {
...
void PnlBrlyLocEqpMNConnection::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyLocEqpMNConnectionData");
...
PnlBrlyLocEqpMNConnection::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLOCEQPMNCONNECTIONDO, scrJref) {
...
void PnlBrlyLocEqpMNConnection::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyLocEqpMNConnectionDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlBrlyLocEqpMNConnection::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYLOCEQPMNCONNECTIONDATA) {
...
void PnlBrlyLocEqpMNConnection::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyLocEqpMNConnectionData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryBrlyLocEqpMNConnection::StatApp();
		statshrqry = QryBrlyLocEqpMNConnection::StatShr();
