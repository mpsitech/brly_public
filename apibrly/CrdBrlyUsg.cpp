/**
  * \file CrdBrlyUsg.cpp
  * API code for job CrdBrlyUsg (implementation)
...
bool CrdBrlyUsg::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyUsg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyUsg";
...
bool CrdBrlyUsg::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyUsg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyUsg";
...
set<uint> CrdBrlyUsg::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdBrlyUsg::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdBrlyUsg::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlyUsg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlyUsg";
...
bool CrdBrlyUsg::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyUsg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyUsg";
...
CrdBrlyUsg::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYUSGDO, scrJref) {
...
void CrdBrlyUsg::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyUsgDo");
...
CrdBrlyUsg::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYUSGDATA) {
...
void CrdBrlyUsg::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyUsgData");
	else
		basefound = checkXPath(docctx, basexpath);
