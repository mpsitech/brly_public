/**
  ...
  * API code for job PnlBrlyFltHeadbar (implementation)
...
PnlBrlyFltHeadbar::StgInf::StgInf(
...
bool PnlBrlyFltHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfBrlyFltHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfBrlyFltHeadbar";
...
set<uint> PnlBrlyFltHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyFltHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyFltHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyFltHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyFltHeadbar";
...
PnlBrlyFltHeadbar::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYFLTHEADBARDATA) {
};
...
void PnlBrlyFltHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyFltHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
