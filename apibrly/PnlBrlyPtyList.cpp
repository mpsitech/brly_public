/**
  ...
  * API code for job PnlBrlyPtyList (implementation)
...
bool PnlBrlyPtyList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyPtyList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyPtyList";
...
bool PnlBrlyPtyList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlyPtyList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlyPtyList";
...
PnlBrlyPtyList::Tag::Tag(
	...
	mask = {CPT, TRS, TXTSHOWING1, TXTSHOWING2, TCOSRF, TCOTIT, TCOETY, TCOCAP};
};

bool PnlBrlyPtyList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyPtyList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyPtyList";
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoEty", TcoEty)) add(TCOETY);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoCap", TcoCap)) add(TCOCAP);
	};
...
PnlBrlyPtyList::DpchAppData::DpchAppData(
			...
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYPTYLISTDATA, scrJref) {
...
void PnlBrlyPtyList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyPtyListData");
...
PnlBrlyPtyList::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYPTYLISTDO, scrJref) {
...
void PnlBrlyPtyList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyPtyListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlBrlyPtyList::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYPTYLISTDATA) {
...
void PnlBrlyPtyList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyPtyListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryBrlyPtyList::StatApp();
		statshrqry = QryBrlyPtyList::StatShr();
