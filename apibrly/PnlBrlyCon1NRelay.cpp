/**
  ...
  * API code for job PnlBrlyCon1NRelay (implementation)
...
bool PnlBrlyCon1NRelay::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyCon1NRelay");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyCon1NRelay";
...
PnlBrlyCon1NRelay::StatApp::StatApp(
...
bool PnlBrlyCon1NRelay::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyCon1NRelay");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyCon1NRelay";
...
set<uint> PnlBrlyCon1NRelay::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyCon1NRelay::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyCon1NRelay::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlyCon1NRelay");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlyCon1NRelay";
...
bool PnlBrlyCon1NRelay::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyCon1NRelay");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyCon1NRelay";
...
PnlBrlyCon1NRelay::DpchAppData::DpchAppData(
			...
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYCON1NRELAYDATA, scrJref) {
...
void PnlBrlyCon1NRelay::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyCon1NRelayData");
...
PnlBrlyCon1NRelay::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYCON1NRELAYDO, scrJref) {
...
void PnlBrlyCon1NRelay::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyCon1NRelayDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlBrlyCon1NRelay::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYCON1NRELAYDATA) {
...
void PnlBrlyCon1NRelay::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyCon1NRelayData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryBrlyCon1NRelay::StatApp();
		statshrqry = QryBrlyCon1NRelay::StatShr();
