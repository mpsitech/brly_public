/**
  ...
  * API code for job PnlBrlySegHeadbar (implementation)
...
PnlBrlySegHeadbar::StgInf::StgInf(
...
bool PnlBrlySegHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfBrlySegHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfBrlySegHeadbar";
...
set<uint> PnlBrlySegHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlySegHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlySegHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlySegHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlySegHeadbar";
...
PnlBrlySegHeadbar::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYSEGHEADBARDATA) {
};
...
void PnlBrlySegHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlySegHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
