/**
  ...
  * API code for job PnlBrlyUsgAAccess (implementation)
...
bool PnlBrlyUsgAAccess::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyUsgAAccess";
...
PnlBrlyUsgAAccess::StatApp::StatApp(
...
bool PnlBrlyUsgAAccess::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyUsgAAccess";
...
set<uint> PnlBrlyUsgAAccess::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyUsgAAccess::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyUsgAAccess::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlyUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlyUsgAAccess";
...
bool PnlBrlyUsgAAccess::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyUsgAAccess";
...
PnlBrlyUsgAAccess::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryBrlyUsgAAccess::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYUSGAACCESSDATA, scrJref) {
...
void PnlBrlyUsgAAccess::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyUsgAAccessData");
...
PnlBrlyUsgAAccess::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYUSGAACCESSDO, scrJref) {
...
void PnlBrlyUsgAAccess::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyUsgAAccessDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlBrlyUsgAAccess::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYUSGAACCESSDATA) {
...
void PnlBrlyUsgAAccess::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyUsgAAccessData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryBrlyUsgAAccess::StatApp();
		statshrqry = QryBrlyUsgAAccess::StatShr();
