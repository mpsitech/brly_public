/**
  ...
  * API code for job PnlBrlyConHeadbar (implementation)
...
PnlBrlyConHeadbar::StgInf::StgInf(
...
bool PnlBrlyConHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfBrlyConHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfBrlyConHeadbar";
...
set<uint> PnlBrlyConHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyConHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyConHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyConHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyConHeadbar";
...
PnlBrlyConHeadbar::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYCONHEADBARDATA) {
};
...
void PnlBrlyConHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyConHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
