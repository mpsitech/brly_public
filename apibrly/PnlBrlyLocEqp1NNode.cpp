/**
  * \file PnlBrlyLocEqp1NNode.cpp
  * API code for job PnlBrlyLocEqp1NNode (implementation)
...
bool PnlBrlyLocEqp1NNode::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyLocEqp1NNode");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyLocEqp1NNode";
...
PnlBrlyLocEqp1NNode::StatApp::StatApp(
...
bool PnlBrlyLocEqp1NNode::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyLocEqp1NNode");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyLocEqp1NNode";
...
set<uint> PnlBrlyLocEqp1NNode::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyLocEqp1NNode::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyLocEqp1NNode::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyLocEqp1NNode");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyLocEqp1NNode";
...
PnlBrlyLocEqp1NNode::DpchAppData::DpchAppData(
			...
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLOCEQP1NNODEDATA, scrJref) {
...
void PnlBrlyLocEqp1NNode::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyLocEqp1NNodeData");
...
PnlBrlyLocEqp1NNode::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLOCEQP1NNODEDO, scrJref) {
...
void PnlBrlyLocEqp1NNode::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyLocEqp1NNodeDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlBrlyLocEqp1NNode::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYLOCEQP1NNODEDATA) {
...
void PnlBrlyLocEqp1NNode::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyLocEqp1NNodeData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryBrlyLocEqp1NNode::StatApp();
		statshrqry = QryBrlyLocEqp1NNode::StatShr();
