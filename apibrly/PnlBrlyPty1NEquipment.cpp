/**
  ...
  * API code for job PnlBrlyPty1NEquipment (implementation)
...
bool PnlBrlyPty1NEquipment::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyPty1NEquipment");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyPty1NEquipment";
...
PnlBrlyPty1NEquipment::StatApp::StatApp(
...
bool PnlBrlyPty1NEquipment::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyPty1NEquipment");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyPty1NEquipment";
...
set<uint> PnlBrlyPty1NEquipment::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyPty1NEquipment::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyPty1NEquipment::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyPty1NEquipment");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyPty1NEquipment";
...
PnlBrlyPty1NEquipment::DpchAppData::DpchAppData(
			...
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYPTY1NEQUIPMENTDATA, scrJref) {
...
void PnlBrlyPty1NEquipment::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyPty1NEquipmentData");
...
PnlBrlyPty1NEquipment::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYPTY1NEQUIPMENTDO, scrJref) {
...
void PnlBrlyPty1NEquipment::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyPty1NEquipmentDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlBrlyPty1NEquipment::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYPTY1NEQUIPMENTDATA) {
...
void PnlBrlyPty1NEquipment::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyPty1NEquipmentData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryBrlyPty1NEquipment::StatApp();
		statshrqry = QryBrlyPty1NEquipment::StatShr();
