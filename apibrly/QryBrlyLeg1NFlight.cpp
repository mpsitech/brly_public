/**
  ...
  * API code for job QryBrlyLeg1NFlight (implementation)
...
QryBrlyLeg1NFlight::StatApp::StatApp(
			const uint firstcol
...
bool QryBrlyLeg1NFlight::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryBrlyLeg1NFlight");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryBrlyLeg1NFlight";
...
set<uint> QryBrlyLeg1NFlight::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyLeg1NFlight::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryBrlyLeg1NFlight::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryBrlyLeg1NFlight");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryBrlyLeg1NFlight";
