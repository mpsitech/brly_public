/**
  * \file CrdBrlyOpr.cpp
  * API code for job CrdBrlyOpr (implementation)
...
bool CrdBrlyOpr::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlyOpr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlyOpr";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdOpr", MtxCrdOpr)) add(MTXCRDOPR);
	};
...
set<uint> CrdBrlyOpr::ContInf::comm(
	...
	if (MtxCrdOpr.compare(comp->MtxCrdOpr) == 0) insert(items, MTXCRDOPR);
...
bool CrdBrlyOpr::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlyOpr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlyOpr";
...
set<uint> CrdBrlyOpr::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdBrlyOpr::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdBrlyOpr::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlyOpr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlyOpr";
...
bool CrdBrlyOpr::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyOpr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyOpr";
...
CrdBrlyOpr::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYOPRDO, scrJref) {
...
void CrdBrlyOpr::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlyOprDo");
...
CrdBrlyOpr::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYOPRDATA) {
...
void CrdBrlyOpr::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyOprData");
	else
		basefound = checkXPath(docctx, basexpath);
