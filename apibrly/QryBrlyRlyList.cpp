/**
  ...
  * API code for job QryBrlyRlyList (implementation)
...
/******************************************************************************
 class QryBrlyRlyList::VecVOrd
 ******************************************************************************/

uint QryBrlyRlyList::VecVOrd::getIx(
			const string& sref
		) {
	...
	else if (s == "con") return CON;
	else if (s == "dir") return DIR;

	return(0);
};

string QryBrlyRlyList::VecVOrd::getSref(
			const uint ix
		) {
	if (ix == STA) return("sta");
	else if (ix == STO) return("sto");
	else if (ix == CON) return("con");
	else if (ix == DIR) return("dir");

	return("");
};

/******************************************************************************
 class QryBrlyRlyList::StatApp
 ******************************************************************************/

QryBrlyRlyList::StatApp::StatApp(
			const uint firstcol
...
bool QryBrlyRlyList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryBrlyRlyList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryBrlyRlyList";
...
set<uint> QryBrlyRlyList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyRlyList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryBrlyRlyList::StatShr
 ******************************************************************************/

QryBrlyRlyList::StatShr::StatShr(
			const uint ntot
...
bool QryBrlyRlyList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryBrlyRlyList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryBrlyRlyList";
...
set<uint> QryBrlyRlyList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyRlyList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryBrlyRlyList::StgIac
 ******************************************************************************/

QryBrlyRlyList::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryBrlyRlyList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryBrlyRlyList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryBrlyRlyList";
...
void QryBrlyRlyList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryBrlyRlyList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryBrlyRlyList";
...
set<uint> QryBrlyRlyList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyRlyList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
