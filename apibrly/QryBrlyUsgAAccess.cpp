/**
  ...
  * API code for job QryBrlyUsgAAccess (implementation)
...
QryBrlyUsgAAccess::StatApp::StatApp(
			const uint firstcol
...
bool QryBrlyUsgAAccess::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryBrlyUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryBrlyUsgAAccess";
...
set<uint> QryBrlyUsgAAccess::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyUsgAAccess::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryBrlyUsgAAccess::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryBrlyUsgAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryBrlyUsgAAccess";
