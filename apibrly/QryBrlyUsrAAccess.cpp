/**
  ...
  * API code for job QryBrlyUsrAAccess (implementation)
...
/******************************************************************************
 class QryBrlyUsrAAccess::StatApp
 ******************************************************************************/

QryBrlyUsrAAccess::StatApp::StatApp(
			const uint firstcol
...
bool QryBrlyUsrAAccess::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryBrlyUsrAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryBrlyUsrAAccess";
...
set<uint> QryBrlyUsrAAccess::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyUsrAAccess::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryBrlyUsrAAccess::StatShr
 ******************************************************************************/

QryBrlyUsrAAccess::StatShr::StatShr(
			const uint ntot
...
bool QryBrlyUsrAAccess::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryBrlyUsrAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryBrlyUsrAAccess";
...
set<uint> QryBrlyUsrAAccess::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyUsrAAccess::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryBrlyUsrAAccess::StgIac
 ******************************************************************************/

QryBrlyUsrAAccess::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryBrlyUsrAAccess::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryBrlyUsrAAccess");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryBrlyUsrAAccess";
...
void QryBrlyUsrAAccess::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryBrlyUsrAAccess";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryBrlyUsrAAccess";
...
set<uint> QryBrlyUsrAAccess::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyUsrAAccess::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
