/**
  * \file CrdBrlySeg.cpp
  * API code for job CrdBrlySeg (implementation)
...
bool CrdBrlySeg::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfBrlySeg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfBrlySeg";
...
set<uint> CrdBrlySeg::ContInf::comm(
	...
	if (MtxCrdSeg.compare(comp->MtxCrdSeg) == 0) insert(items, MTXCRDSEG);
...
bool CrdBrlySeg::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppBrlySeg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppBrlySeg";
...
set<uint> CrdBrlySeg::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdBrlySeg::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdBrlySeg::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrBrlySeg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrBrlySeg";
...
bool CrdBrlySeg::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlySeg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlySeg";
...
CrdBrlySeg::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYSEGDO, scrJref) {
...
void CrdBrlySeg::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppBrlySegDo");
...
CrdBrlySeg::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYSEGDATA) {
...
void CrdBrlySeg::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlySegData");
	else
		basefound = checkXPath(docctx, basexpath);
