/**
  ...
  * API code for job QryBrlyFltMNConnection (implementation)
...
QryBrlyFltMNConnection::StatApp::StatApp(
			const uint firstcol
...
bool QryBrlyFltMNConnection::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryBrlyFltMNConnection");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryBrlyFltMNConnection";
...
set<uint> QryBrlyFltMNConnection::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyFltMNConnection::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryBrlyFltMNConnection::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryBrlyFltMNConnection");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryBrlyFltMNConnection";
