/**
  ...
  * API code for job QryBrlyConList (implementation)
...
/******************************************************************************
 class QryBrlyConList::VecVOrd
 ******************************************************************************/

uint QryBrlyConList::VecVOrd::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);

	if (s == "flt") return FLT;
	else if (s == "cor") return COR;
	else if (s == "sta") return STA;

	return(0);
};

string QryBrlyConList::VecVOrd::getSref(
			const uint ix
		) {
	if (ix == FLT) return("flt");
	else if (ix == COR) return("cor");
...
/******************************************************************************
 class QryBrlyConList::StatApp
 ******************************************************************************/

QryBrlyConList::StatApp::StatApp(
			const uint firstcol
...
bool QryBrlyConList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryBrlyConList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryBrlyConList";
...
set<uint> QryBrlyConList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyConList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryBrlyConList::StatShr
 ******************************************************************************/

QryBrlyConList::StatShr::StatShr(
			const uint ntot
...
bool QryBrlyConList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryBrlyConList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryBrlyConList";
...
set<uint> QryBrlyConList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyConList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryBrlyConList::StgIac
 ******************************************************************************/

QryBrlyConList::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryBrlyConList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryBrlyConList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryBrlyConList";
...
void QryBrlyConList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryBrlyConList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryBrlyConList";
...
set<uint> QryBrlyConList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryBrlyConList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
