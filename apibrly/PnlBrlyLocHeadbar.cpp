/**
  * \file PnlBrlyLocHeadbar.cpp
  * API code for job PnlBrlyLocHeadbar (implementation)
...
PnlBrlyLocHeadbar::StgInf::StgInf(
...
bool PnlBrlyLocHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfBrlyLocHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfBrlyLocHeadbar";
...
set<uint> PnlBrlyLocHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlBrlyLocHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlBrlyLocHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagBrlyLocHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemBrlyLocHeadbar";
...
PnlBrlyLocHeadbar::DpchEngData::DpchEngData() : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYLOCHEADBARDATA) {
};
...
void PnlBrlyLocHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngBrlyLocHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
