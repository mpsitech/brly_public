...
void* BrlydJobprc::run(
	...
	while (true) {
		...
		while (req == NULL) {
			...
			Mutex::lock(&(xchg->mcJobprcs), "xchg->mcJobprcs", "BrlydJobprc", "run");
			Cond::wait(&(xchg->cJobprcs), &(xchg->mcJobprcs), "xchg->cJobprcs", "BrlydJobprc", "run");

			req = xchg->pullFirstReq();

			Mutex::unlock(&(xchg->mcJobprcs), "xchg->mcJobprcs", "BrlydJobprc", "run");
		};

		// re-signal condition to avoid using broadcast
		Cond::signal(&(xchg->cJobprcs), &(xchg->mcJobprcs), "xchg->cJobprcs", "xchg->mcJobprcs", "BrlydJobprc", "run");
		...
		if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
			...
			if (req->dpchapp) {
				//cout << "BrlydJobprc::run() REGULAR request: '" << VecBrlyVDpch::getSref(req->dpchapp->ixBrlyVDpch) << "'" << endl;
			...
			if (req->dpchret) {
				//cout << "BrlydJobprc::run() RET request: '" << VecBrlyVDpch::getSref(req->dpchret->ixBrlyVDpch) << "'" << endl;
...
void BrlydJobprc::cleanup(
	...
	Mutex::unlock(&(xchg->mcJobprcs), "xchg->mcJobprcs", "BrlydJobprc", "cleanup");
};
