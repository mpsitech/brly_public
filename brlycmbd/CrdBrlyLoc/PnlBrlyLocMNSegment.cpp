/**
  * \file PnlBrlyLocMNSegment.cpp
  * job handler for job PnlBrlyLocMNSegment (implementation)
...
#include "PnlBrlyLocMNSegment_blks.cpp"
#include "PnlBrlyLocMNSegment_evals.cpp"

/******************************************************************************
 class PnlBrlyLocMNSegment
 ******************************************************************************/

PnlBrlyLocMNSegment::PnlBrlyLocMNSegment(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYLOCMNSEGMENT, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyLocMNSegment(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyLocMNSegment::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyLocMNSegment::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyLocMNSegment::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyLocMNSegment::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCMNSEGMENTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCMNSEGMENTDO) {
...
void PnlBrlyLocMNSegment::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyLocMNSegment::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocMNSegment::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyLocMNSegment::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQLocMNSegment* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyLocMNSegment::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyLocMNSegment::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyLocMNSegment::StgIac::JNUM)) {
...
void PnlBrlyLocMNSegment::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocMNSegment::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocMNSegment::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyLocMNSegment::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyLocMNSegment::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
