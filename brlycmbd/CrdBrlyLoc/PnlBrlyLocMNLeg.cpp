/**
  ...
  * job handler for job PnlBrlyLocMNLeg (implementation)
...
#include "PnlBrlyLocMNLeg_blks.cpp"
#include "PnlBrlyLocMNLeg_evals.cpp"

/******************************************************************************
 class PnlBrlyLocMNLeg
 ******************************************************************************/

PnlBrlyLocMNLeg::PnlBrlyLocMNLeg(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYLOCMNLEG, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyLocMNLeg(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyLocMNLeg::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyLocMNLeg::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyLocMNLeg::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCMNLEGDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCMNLEGDO) {
...
void PnlBrlyLocMNLeg::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyLocMNLeg::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocMNLeg::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyLocMNLeg::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyLocMNLeg::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyLocMNLeg::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyLocMNLeg::StgIac::JNUM)) {
...
void PnlBrlyLocMNLeg::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyLocMNLeg::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocMNLeg::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocMNLeg::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyLocMNLeg::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyLocMNLeg::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
