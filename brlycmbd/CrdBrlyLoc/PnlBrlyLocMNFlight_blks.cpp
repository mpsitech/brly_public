/**
  * \file PnlBrlyLocMNFlight_blks.cpp
  * job handler for job PnlBrlyLocMNFlight (implementation of blocks)
...
void PnlBrlyLocMNFlight::ContInf::writeXML(
	...
	else itemtag = "ContitemInfBrlyLocMNFlight";
...
void PnlBrlyLocMNFlight::StatApp::writeXML(
	...
	else itemtag = "StatitemAppBrlyLocMNFlight";
...
void PnlBrlyLocMNFlight::StatShr::writeXML(
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrBrlyLocMNFlight";
...
void PnlBrlyLocMNFlight::Tag::writeXML(
	...
	else itemtag = "TagitemBrlyLocMNFlight";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixBrlyVLocale == VecBrlyVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Flights within visibility range");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoXsa", "XVisStart");
			writeStringAttr(wr, itemtag, "sref", "TcoXso", "XVisStop");
		} else if (ixBrlyVLocale == VecBrlyVLocale::DECH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Fl\\u00fcge im Bereich der Sichtbarkeit");
...
PnlBrlyLocMNFlight::DpchAppData::DpchAppData() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLOCMNFLIGHTDATA) {
};
...
void PnlBrlyLocMNFlight::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyLocMNFlightData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlyLocMNFlight::DpchAppDo::DpchAppDo() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLOCMNFLIGHTDO) {
	ixVDo = 0;
};
...
void PnlBrlyLocMNFlight::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyLocMNFlightDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlyLocMNFlight::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, QryBrlyLocMNFlight::StatShr* statshrqry
			, QryBrlyLocMNFlight::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYLOCMNFLIGHTDATA, jref) {
...
void PnlBrlyLocMNFlight::DpchEngData::merge(
			DpchEngBrly* dpcheng
		) {
...
void PnlBrlyLocMNFlight::DpchEngData::writeXML(
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngBrlyLocMNFlightData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		...
		if (has(STATAPPQRY)) QryBrlyLocMNFlight::StatApp::writeXML(wr);
