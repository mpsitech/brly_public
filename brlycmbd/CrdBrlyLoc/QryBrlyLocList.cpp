...
QryBrlyLocList::QryBrlyLocList(
	...
	xchg->addClstn(VecBrlyVCall::CALLBRLYLOCMOD, jref, Clstn::VecVJobmask::ALL, 0, Arg(), Clstn::VecVJactype::LOCK);
...
void QryBrlyLocList::refreshJnum() {
	...
	xchg->removeClstns(VecBrlyVCall::CALLBRLYLOCUPD_REFEQ, jref);
	if (preRefSel != 0) xchg->addRefClstn(VecBrlyVCall::CALLBRLYLOCUPD_REFEQ, jref, Clstn::VecVJobmask::ALL, 0, preRefSel, Clstn::VecVJactype::LOCK);
};
...
void QryBrlyLocList::rerun_orderSQL(
	...
	if (preIxOrd == VecVOrd::SRF) sqlstr += " ORDER BY TblBrlyMLocation.sref ASC";
	...
	else if (preIxOrd == VecVOrd::EQP) sqlstr += " ORDER BY TblBrlyMLocation.refBrlyMEquipment ASC";
};
...
bool QryBrlyLocList::handleCallBrlyStubChg(
			DbsBrly* dbsbrly
