/**
  ...
  * job handler for job PnlBrlyLocEnd1NLeg (implementation)
...
#include "PnlBrlyLocEnd1NLeg_blks.cpp"
#include "PnlBrlyLocEnd1NLeg_evals.cpp"

/******************************************************************************
 class PnlBrlyLocEnd1NLeg
 ******************************************************************************/

PnlBrlyLocEnd1NLeg::PnlBrlyLocEnd1NLeg(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYLOCEND1NLEG, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyLocEnd1NLeg(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyLocEnd1NLeg::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyLocEnd1NLeg::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyLocEnd1NLeg::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyLocEnd1NLeg::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCEND1NLEGDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCEND1NLEGDO) {
...
void PnlBrlyLocEnd1NLeg::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyLocEnd1NLeg::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocEnd1NLeg::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyLocEnd1NLeg::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQLocEnd1NLeg* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyLocEnd1NLeg::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyLocEnd1NLeg::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyLocEnd1NLeg::StgIac::JNUM)) {
...
void PnlBrlyLocEnd1NLeg::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyLocEnd1NLeg::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyLocEnd1NLeg::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyLocEnd1NLeg::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
