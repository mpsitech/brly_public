/**
  ...
  * job handler for job PnlBrlyLocProxOrgMNLocation (implementation)
...
#include "PnlBrlyLocProxOrgMNLocation_blks.cpp"
#include "PnlBrlyLocProxOrgMNLocation_evals.cpp"

/******************************************************************************
 class PnlBrlyLocProxOrgMNLocation
 ******************************************************************************/

PnlBrlyLocProxOrgMNLocation::PnlBrlyLocProxOrgMNLocation(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYLOCPROXORGMNLOCATION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyLocProxOrgMNLocation(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyLocProxOrgMNLocation::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyLocProxOrgMNLocation::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyLocProxOrgMNLocation::updatePreset(
			DbsBrly* dbsbrly
	...
	if (ixBrlyVPreset == VecBrlyVPreset::PREBRLYREFLOC) {
		recPlocRloc = BrlyProxRMLocationMLocation();
...
void PnlBrlyLocProxOrgMNLocation::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCPROXORGMNLOCATIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCPROXORGMNLOCATIONDO) {
...
void PnlBrlyLocProxOrgMNLocation::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyLocProxOrgMNLocation::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocProxOrgMNLocation::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyLocProxOrgMNLocation::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQLocProxOrgMNLocation* recSelNew = NULL;

	BrlyProxRMLocationMLocation* _recPlocRloc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyLocProxOrgMNLocation::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyLocProxOrgMNLocation::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyLocProxOrgMNLocation::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbsbrly->tblbrlyproxrmlocationmlocation->loadRecByRef(recSelNew->ref, &_recPlocRloc)) {
					recPlocRloc = *_recPlocRloc;
					delete _recPlocRloc;
				};
...
void PnlBrlyLocProxOrgMNLocation::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyLocProxOrgMNLocation::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocProxOrgMNLocation::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocProxOrgMNLocation::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyLocProxOrgMNLocation::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyLocProxOrgMNLocation::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
