/**
  ...
  * job handler for job PnlBrlyLocEqp1NNode (implementation)
...
#include "PnlBrlyLocEqp1NNode_blks.cpp"

/******************************************************************************
 class PnlBrlyLocEqp1NNode
 ******************************************************************************/

PnlBrlyLocEqp1NNode::PnlBrlyLocEqp1NNode(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYLOCEQP1NNODE, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyLocEqp1NNode(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyLocEqp1NNode::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyLocEqp1NNode::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyLocEqp1NNode::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyLocEqp1NNode::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCEQP1NNODEDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCEQP1NNODEDO) {
...
void PnlBrlyLocEqp1NNode::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyLocEqp1NNode::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocEqp1NNode::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyLocEqp1NNode::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQLocEqp1NNode* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyLocEqp1NNode::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyLocEqp1NNode::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyLocEqp1NNode::StgIac::JNUM)) {
...
void PnlBrlyLocEqp1NNode::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyLocEqp1NNode::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyLocEqp1NNode::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
