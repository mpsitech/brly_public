/**
  * \file PnlBrlyLocEqpMNConnection.cpp
  * job handler for job PnlBrlyLocEqpMNConnection (implementation)
...
#include "PnlBrlyLocEqpMNConnection_blks.cpp"
#include "PnlBrlyLocEqpMNConnection_evals.cpp"

/******************************************************************************
 class PnlBrlyLocEqpMNConnection
 ******************************************************************************/

PnlBrlyLocEqpMNConnection::PnlBrlyLocEqpMNConnection(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYLOCEQPMNCONNECTION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyLocEqpMNConnection(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyLocEqpMNConnection::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyLocEqpMNConnection::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyLocEqpMNConnection::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyLocEqpMNConnection::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCEQPMNCONNECTIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLOCEQPMNCONNECTIONDO) {
...
void PnlBrlyLocEqpMNConnection::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyLocEqpMNConnection::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocEqpMNConnection::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyLocEqpMNConnection::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQLocEqpMNConnection* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyLocEqpMNConnection::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyLocEqpMNConnection::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyLocEqpMNConnection::StgIac::JNUM)) {
...
void PnlBrlyLocEqpMNConnection::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyLocEqpMNConnection::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocEqpMNConnection::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLocEqpMNConnection::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyLocEqpMNConnection::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyLocEqpMNConnection::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
