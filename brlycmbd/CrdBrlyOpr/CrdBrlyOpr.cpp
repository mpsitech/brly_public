/**
  * \file CrdBrlyOpr.cpp
  * job handler for job CrdBrlyOpr (implementation)
...
#include "CrdBrlyOpr_blks.cpp"

/******************************************************************************
 class CrdBrlyOpr
 ******************************************************************************/

CrdBrlyOpr::CrdBrlyOpr(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
			, const ubigint ref
		) : JobBrly(xchg, VecBrlyVJob::CRDBRLYOPR, jrefSup, ixBrlyVLocale) {
	...
	pnlrec = new PnlBrlyOprRec(xchg, dbsbrly, jref, ixBrlyVLocale);
	pnlheadbar = new PnlBrlyOprHeadbar(xchg, dbsbrly, jref, ixBrlyVLocale);
	pnllist = new PnlBrlyOprList(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* CrdBrlyOpr::getNewDpchEng(
			set<uint> items
		) {
...
void CrdBrlyOpr::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdBrlyOpr/" + VecBrlyVLocale::getSref(ixBrlyVLocale);
	continf.MtxCrdOpr = StubBrly::getStubOprStd(dbsbrly, xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFOPR, jref), ixBrlyVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdBrlyOpr::changeRef(
			DbsBrly* dbsbrly
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsbrly, VecBrlyVCall::CALLBRLYLOG, jref, VecBrlyVPreset::VOID, 0, "CrdBrlyOpr", ref);
	xchg->addRefPreset(VecBrlyVPreset::PREBRLYREFOPR, jref, ref);

	refresh(dbsbrly, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));

	if (pnllist) pnllist->updatePreset(dbsbrly, VecBrlyVPreset::PREBRLYREFOPR, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbsbrly, VecBrlyVPreset::PREBRLYREFOPR, jrefTrig, notif);
};

void CrdBrlyOpr::updatePreset(
			DbsBrly* dbsbrly
...
void CrdBrlyOpr::leaveSgeIdle(
			DbsBrly* dbsbrly
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdBrlyOpr::enterSgeAlrbrlyabt(
			DbsBrly* dbsbrly
			, const bool reenter
		) {
...
void CrdBrlyOpr::leaveSgeAlrbrlyabt(
			DbsBrly* dbsbrly
		) {
	// IP leaveSgeAlrbrlyabt --- INSERT
};

uint CrdBrlyOpr::enterSgeClose(
			DbsBrly* dbsbrly
			, const bool reenter
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsbrly, VecBrlyVCall::CALLBRLYCRDCLOSE, jref, VecBrlyVCard::CRDBRLYOPR);
	return retval;
};

void CrdBrlyOpr::leaveSgeClose(
			DbsBrly* dbsbrly
		) {
	// IP leaveSgeClose --- INSERT
};

string CrdBrlyOpr::getSquawk(
			DbsBrly* dbsbrly
		) {
	string retval;
	// IP getSquawk --- BEGIN
...
void CrdBrlyOpr::handleRequest(
	...
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYOPRDO) {
...
void CrdBrlyOpr::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void CrdBrlyOpr::handleDpchAppDoClose(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	muteRefresh = true;
...
void CrdBrlyOpr::handleDpchAppDoMitAppAbtClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void CrdBrlyOpr::handleDpchAppBrlyAlert(
			DbsBrly* dbsbrly
...
bool CrdBrlyOpr::handleCallBrlyDlgClose(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdBrlyOpr::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdBrlyOpr::handleCallBrlyRefPreSet(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecBrlyVPreset::PREBRLYREFOPR) {
