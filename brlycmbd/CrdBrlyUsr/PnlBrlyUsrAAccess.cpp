/**
  ...
  * job handler for job PnlBrlyUsrAAccess (implementation)
...
#include "PnlBrlyUsrAAccess_blks.cpp"
#include "PnlBrlyUsrAAccess_evals.cpp"

/******************************************************************************
 class PnlBrlyUsrAAccess
 ******************************************************************************/

PnlBrlyUsrAAccess::PnlBrlyUsrAAccess(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYUSRAACCESS, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyUsrAAccess(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyUsrAAccess::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyUsrAAccess::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyUsrAAccess::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyUsrAAccess::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYUSRAACCESSDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYUSRAACCESSDO) {
...
void PnlBrlyUsrAAccess::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyUsrAAccess::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsrAAccess::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyUsrAAccess::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQUsrAAccess* recSelNew = NULL;

	BrlyAMUserAccess* _recUsrAacc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyUsrAAccess::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyUsrAAccess::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyUsrAAccess::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recUsrAacc = BrlyAMUserAccess();

			if (recSelNew) {
				if (dbsbrly->tblbrlyamuseraccess->loadRecByRef(recSelNew->ref, &_recUsrAacc)) {
...
void PnlBrlyUsrAAccess::handleDpchAppDoButNewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsrAAccess::handleDpchAppDoButDuplicateClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsrAAccess::handleDpchAppDoButDeleteClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsrAAccess::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyUsrAAccess::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyUsrAAccess::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
