/**
  * \file PnlBrlyUsrRec_blks.cpp
  * job handler for job PnlBrlyUsrRec (implementation of blocks)
...
void PnlBrlyUsrRec::ContInf::writeXML(
	...
	else itemtag = "ContitemInfBrlyUsrRec";
...
void PnlBrlyUsrRec::StatApp::writeXML(
	...
	else itemtag = "StatitemAppBrlyUsrRec";
...
void PnlBrlyUsrRec::StatShr::writeXML(
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrBrlyUsrRec";
...
set<uint> PnlBrlyUsrRec::StatShr::diff(
	...
	diffitems = {IXBRLYVEXPSTATE, JREFDETAIL, JREFAACCESS, JREF1NSESSION, JREFMNUSERGROUP, BUTREGULARIZEACTIVE};
	for (auto it=commitems.begin();it!=commitems.end();it++) diffitems.erase(*it);
...
void PnlBrlyUsrRec::Tag::writeXML(
	...
	else itemtag = "TagitemBrlyUsrRec";
...
PnlBrlyUsrRec::DpchAppDo::DpchAppDo() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYUSRRECDO) {
	ixVDo = 0;
};
...
void PnlBrlyUsrRec::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyUsrRecDo");
	else
...
PnlBrlyUsrRec::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYUSRRECDATA, jref) {
...
void PnlBrlyUsrRec::DpchEngData::merge(
			DpchEngBrly* dpcheng
		) {
...
void PnlBrlyUsrRec::DpchEngData::writeXML(
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngBrlyUsrRecData");
