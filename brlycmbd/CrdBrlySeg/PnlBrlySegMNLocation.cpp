/**
  ...
  * job handler for job PnlBrlySegMNLocation (implementation)
...
#include "PnlBrlySegMNLocation_blks.cpp"
#include "PnlBrlySegMNLocation_evals.cpp"

/******************************************************************************
 class PnlBrlySegMNLocation
 ******************************************************************************/

PnlBrlySegMNLocation::PnlBrlySegMNLocation(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYSEGMNLOCATION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlySegMNLocation(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlySegMNLocation::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlySegMNLocation::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlySegMNLocation::updatePreset(
			DbsBrly* dbsbrly
	...
	if (ixBrlyVPreset == VecBrlyVPreset::PREBRLYREFSEG) {
		recLocRseg = BrlyRMLocationMSegment();
...
void PnlBrlySegMNLocation::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYSEGMNLOCATIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYSEGMNLOCATIONDO) {
...
void PnlBrlySegMNLocation::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlySegMNLocation::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegMNLocation::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlySegMNLocation::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQSegMNLocation* recSelNew = NULL;

	BrlyRMLocationMSegment* _recLocRseg = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlySegMNLocation::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlySegMNLocation::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlySegMNLocation::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbsbrly->tblbrlyrmlocationmsegment->loadRecByRef(recSelNew->ref, &_recLocRseg)) {
					recLocRseg = *_recLocRseg;
					delete _recLocRseg;
				};
...
void PnlBrlySegMNLocation::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXUACLOC, jref)) {
			sref = "CrdBrlyLoc";
...
void PnlBrlySegMNLocation::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegMNLocation::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegMNLocation::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlySegMNLocation::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlySegMNLocation::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
