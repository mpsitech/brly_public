/**
  ...
  * job handler for job PnlBrlySegOrgMNSegment (implementation)
...
#include "PnlBrlySegOrgMNSegment_blks.cpp"
#include "PnlBrlySegOrgMNSegment_evals.cpp"

/******************************************************************************
 class PnlBrlySegOrgMNSegment
 ******************************************************************************/

PnlBrlySegOrgMNSegment::PnlBrlySegOrgMNSegment(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYSEGORGMNSEGMENT, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlySegOrgMNSegment(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlySegOrgMNSegment::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlySegOrgMNSegment::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlySegOrgMNSegment::updatePreset(
			DbsBrly* dbsbrly
	...
	if (ixBrlyVPreset == VecBrlyVPreset::PREBRLYREFSEG) {
		recSegRseg = BrlyRMSegmentMSegment();
...
void PnlBrlySegOrgMNSegment::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYSEGORGMNSEGMENTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYSEGORGMNSEGMENTDO) {
...
void PnlBrlySegOrgMNSegment::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlySegOrgMNSegment::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegOrgMNSegment::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlySegOrgMNSegment::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQSegOrgMNSegment* recSelNew = NULL;

	BrlyRMSegmentMSegment* _recSegRseg = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlySegOrgMNSegment::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlySegOrgMNSegment::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlySegOrgMNSegment::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbsbrly->tblbrlyrmsegmentmsegment->loadRecByRef(recSelNew->ref, &_recSegRseg)) {
					recSegRseg = *_recSegRseg;
					delete _recSegRseg;
				};
				if (dbsbrly->tblbrlymsegment->loadRecByRef(recSelNew->mref, &_recSeg)) {
					recSeg = *_recSeg;
					delete _recSeg;
				};
			};
...
void PnlBrlySegOrgMNSegment::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXUACSEG, jref)) if (ixPre == VecBrlyVPreset::PREBRLYREFFLT) {
			sref = "CrdBrlySeg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbsbrly, VecBrlyVCall::CALLBRLYCRDOPEN, jref, ixPre, refPre, sref, recSeg.ref, jrefNew);
		};
		if (jrefNew == 0) {
			if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXUACSEG, jref)) if (ixPre == VecBrlyVPreset::PREBRLYREFCON) {
				sref = "CrdBrlySeg";
				xchg->triggerIxRefSrefIntvalToRefCall(dbsbrly, VecBrlyVCall::CALLBRLYCRDOPEN, jref, ixPre, refPre, sref, recSeg.ref, jrefNew);
			};
		};
...
void PnlBrlySegOrgMNSegment::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegOrgMNSegment::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegOrgMNSegment::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlySegOrgMNSegment::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlySegOrgMNSegment::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
