...
void QryBrlySegMNConnection::rerun(
	...
	sqlstr += " FROM TblBrlyRMConnectionMSegment, TblBrlyMConnection";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYCONRSEGMOD_SEGEQ, jref, Clstn::VecVJobmask::ALL, 0, preRefSeg, Clstn::VecVJactype::LOCK);
};
...
void QryBrlySegMNConnection::handleCall(
	...
	} else if (call->ixVCall == VecBrlyVCall::CALLBRLYCONRSEGMOD_SEGEQ) {
		call->abort = handleCallBrlyConRsegMod_segEq(dbsbrly, call->jref, call->argInv.ref);
	};
};

bool QryBrlySegMNConnection::handleCallBrlyStubChg(
			DbsBrly* dbsbrly
