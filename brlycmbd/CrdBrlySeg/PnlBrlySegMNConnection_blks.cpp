/**
  * \file PnlBrlySegMNConnection_blks.cpp
  * job handler for job PnlBrlySegMNConnection (implementation of blocks)
...
void PnlBrlySegMNConnection::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfBrlySegMNConnection";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfBrlySegMNConnection";
...
void PnlBrlySegMNConnection::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixBrlyVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppBrlySegMNConnection";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppBrlySegMNConnection";
...
void PnlBrlySegMNConnection::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrBrlySegMNConnection";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrBrlySegMNConnection";
...
void PnlBrlySegMNConnection::Tag::writeXML(
			const uint ixBrlyVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagBrlySegMNConnection";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemBrlySegMNConnection";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixBrlyVLocale == VecBrlyVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Availability for connections");
			...
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Verf\\u00fcgbarkeit f\\u00fcr Verbindungen");
...
PnlBrlySegMNConnection::DpchAppData::DpchAppData() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYSEGMNCONNECTIONDATA) {
};
...
void PnlBrlySegMNConnection::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlySegMNConnectionData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlySegMNConnection::DpchAppDo::DpchAppDo() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYSEGMNCONNECTIONDO) {
	ixVDo = 0;
};
...
void PnlBrlySegMNConnection::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlySegMNConnectionDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlySegMNConnection::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYSEGMNCONNECTIONDATA, jref) {
...
void PnlBrlySegMNConnection::DpchEngData::merge(
			DpchEngBrly* dpcheng
		) {
...
void PnlBrlySegMNConnection::DpchEngData::writeXML(
			const uint ixBrlyVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngBrlySegMNConnectionData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		...
		if (has(STATAPPQRY)) QryBrlySegMNConnection::StatApp::writeXML(wr);
