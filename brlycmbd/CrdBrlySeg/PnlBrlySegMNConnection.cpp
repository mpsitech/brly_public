/**
  ...
  * job handler for job PnlBrlySegMNConnection (implementation)
...
#include "PnlBrlySegMNConnection_blks.cpp"
#include "PnlBrlySegMNConnection_evals.cpp"

/******************************************************************************
 class PnlBrlySegMNConnection
 ******************************************************************************/

PnlBrlySegMNConnection::PnlBrlySegMNConnection(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYSEGMNCONNECTION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlySegMNConnection(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlySegMNConnection::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlySegMNConnection::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlySegMNConnection::updatePreset(
			DbsBrly* dbsbrly
	...
	if (ixBrlyVPreset == VecBrlyVPreset::PREBRLYREFSEG) {
		recConRseg = BrlyRMConnectionMSegment();
...
void PnlBrlySegMNConnection::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYSEGMNCONNECTIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYSEGMNCONNECTIONDO) {
...
void PnlBrlySegMNConnection::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlySegMNConnection::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegMNConnection::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlySegMNConnection::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQSegMNConnection* recSelNew = NULL;

	BrlyRMConnectionMSegment* _recConRseg = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlySegMNConnection::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlySegMNConnection::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlySegMNConnection::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbsbrly->tblbrlyrmconnectionmsegment->loadRecByRef(recSelNew->ref, &_recConRseg)) {
					recConRseg = *_recConRseg;
					delete _recConRseg;
				};
...
void PnlBrlySegMNConnection::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlySegMNConnection::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegMNConnection::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlySegMNConnection::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlySegMNConnection::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlySegMNConnection::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
