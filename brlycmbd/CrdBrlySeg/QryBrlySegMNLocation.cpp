...
void QryBrlySegMNLocation::rerun(
	...
	sqlstr += " FROM TblBrlyRMLocationMSegment, TblBrlyMLocation";
	...
	xchg->addRefClstn(VecBrlyVCall::CALLBRLYLOCRSEGMOD_SEGEQ, jref, Clstn::VecVJobmask::ALL, 0, preRefSeg, Clstn::VecVJactype::LOCK);
};
...
void QryBrlySegMNLocation::handleCall(
	...
	} else if (call->ixVCall == VecBrlyVCall::CALLBRLYLOCRSEGMOD_SEGEQ) {
		call->abort = handleCallBrlyLocRsegMod_segEq(dbsbrly, call->jref, call->argInv.ref);
	};
};

bool QryBrlySegMNLocation::handleCallBrlyStubChg(
			DbsBrly* dbsbrly
