/**
  * \file PnlBrlyFilList.cpp
  * job handler for job PnlBrlyFilList (implementation)
...
#include "PnlBrlyFilList_blks.cpp"
#include "PnlBrlyFilList_evals.cpp"

/******************************************************************************
 class PnlBrlyFilList
 ******************************************************************************/

PnlBrlyFilList::PnlBrlyFilList(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYFILLIST, jrefSup, ixBrlyVLocale) {
	...
	QryBrlyFilList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecBrlyVPreset::PREBRLYREFSEL, jref, xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFFIL, jref));
	xchg->addIxPreset(VecBrlyVPreset::PREBRLYIXORD, jref, QryBrlyFilList::VecVOrd::FNM);

	qry = new QryBrlyFilList(xchg, dbsbrly, jref, ixBrlyVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixBrlyVExpstate = ((xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFFIL, jref)) ? VecBrlyVExpstate::MIND : VecBrlyVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyFilList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyFilList::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyFilList::regularize(
			DbsBrly* dbsbrly
			, const bool notif
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFilList::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYFILLISTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYFILLISTDO) {
...
void PnlBrlyFilList::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyFilList::handleDpchAppDataContiac(
			DbsBrly* dbsbrly
			, ContIac* _contiac
			, DpchEngBrly** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryBrlyFilList::VecVOrd::FNM) && (_contiac->numFTos <= QryBrlyFilList::VecVOrd::REU)) {
...
void PnlBrlyFilList::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFilList::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyFilList::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyFilList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyFilList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			refresh(dbsbrly, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyFilList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsbrly, VecBrlyVCall::CALLBRLYREFPRESET, jref, VecBrlyVPreset::PREBRLYREFFIL, refSelNew);
		};
	};
...
void PnlBrlyFilList::handleDpchAppDoButMinimizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFilList::handleDpchAppDoButRegularizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFilList::handleDpchAppDoButNewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFilList::handleDpchAppDoButDeleteClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFilList::handleDpchAppDoButFilterClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFilList::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyFilList::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyFilList::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
