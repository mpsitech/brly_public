/**
  * \file PnlBrlyPrsList.cpp
  * job handler for job PnlBrlyPrsList (implementation)
...
#include "PnlBrlyPrsList_blks.cpp"
#include "PnlBrlyPrsList_evals.cpp"

/******************************************************************************
 class PnlBrlyPrsList
 ******************************************************************************/

PnlBrlyPrsList::PnlBrlyPrsList(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYPRSLIST, jrefSup, ixBrlyVLocale) {
	...
	QryBrlyPrsList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecBrlyVPreset::PREBRLYREFSEL, jref, xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFPRS, jref));
	xchg->addIxPreset(VecBrlyVPreset::PREBRLYIXORD, jref, QryBrlyPrsList::VecVOrd::LNM);

	qry = new QryBrlyPrsList(xchg, dbsbrly, jref, ixBrlyVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixBrlyVExpstate = ((xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFPRS, jref)) ? VecBrlyVExpstate::MIND : VecBrlyVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyPrsList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyPrsList::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyPrsList::regularize(
			DbsBrly* dbsbrly
			, const bool notif
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyPrsList::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYPRSLISTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYPRSLISTDO) {
...
void PnlBrlyPrsList::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyPrsList::handleDpchAppDataContiac(
			DbsBrly* dbsbrly
			, ContIac* _contiac
			, DpchEngBrly** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryBrlyPrsList::VecVOrd::LNM) && (_contiac->numFTos <= QryBrlyPrsList::VecVOrd::OWN)) {
...
void PnlBrlyPrsList::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyPrsList::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyPrsList::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyPrsList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyPrsList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			refresh(dbsbrly, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyPrsList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsbrly, VecBrlyVCall::CALLBRLYREFPRESET, jref, VecBrlyVPreset::PREBRLYREFPRS, refSelNew);
		};
	};
...
void PnlBrlyPrsList::handleDpchAppDoButMinimizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyPrsList::handleDpchAppDoButRegularizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyPrsList::handleDpchAppDoButNewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyPrsList::handleDpchAppDoButDeleteClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyPrsList::handleDpchAppDoButFilterClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyPrsList::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyPrsList::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyPrsList::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
