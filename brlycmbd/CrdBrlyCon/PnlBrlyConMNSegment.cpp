/**
  ...
  * job handler for job PnlBrlyConMNSegment (implementation)
...
#include "PnlBrlyConMNSegment_blks.cpp"
#include "PnlBrlyConMNSegment_evals.cpp"

/******************************************************************************
 class PnlBrlyConMNSegment
 ******************************************************************************/

PnlBrlyConMNSegment::PnlBrlyConMNSegment(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYCONMNSEGMENT, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyConMNSegment(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyConMNSegment::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyConMNSegment::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyConMNSegment::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyConMNSegment::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYCONMNSEGMENTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYCONMNSEGMENTDO) {
...
void PnlBrlyConMNSegment::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyConMNSegment::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyConMNSegment::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyConMNSegment::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQConMNSegment* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyConMNSegment::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyConMNSegment::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyConMNSegment::StgIac::JNUM)) {
...
void PnlBrlyConMNSegment::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyConMNSegment::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyConMNSegment::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyConMNSegment::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyConMNSegment::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyConMNSegment::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
