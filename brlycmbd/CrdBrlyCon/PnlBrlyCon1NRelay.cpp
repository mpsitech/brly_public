/**
  * \file PnlBrlyCon1NRelay.cpp
  * job handler for job PnlBrlyCon1NRelay (implementation)
...
#include "PnlBrlyCon1NRelay_blks.cpp"
#include "PnlBrlyCon1NRelay_evals.cpp"

/******************************************************************************
 class PnlBrlyCon1NRelay
 ******************************************************************************/

PnlBrlyCon1NRelay::PnlBrlyCon1NRelay(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYCON1NRELAY, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyCon1NRelay(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyCon1NRelay::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyCon1NRelay::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyCon1NRelay::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyCon1NRelay::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYCON1NRELAYDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYCON1NRELAYDO) {
...
void PnlBrlyCon1NRelay::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyCon1NRelay::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyCon1NRelay::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyCon1NRelay::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQCon1NRelay* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyCon1NRelay::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyCon1NRelay::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyCon1NRelay::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recRly = BrlyMRelay();

			if (recSelNew) {
				if (dbsbrly->tblbrlymrelay->loadRecByRef(recSelNew->ref, &_recRly)) {
					recRly = *_recRly;
					delete _recRly;
				};
			};
...
void PnlBrlyCon1NRelay::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXUACRLY, jref)) if (refCon != 0) {
			sref = "CrdBrlyRly";
			xchg->triggerIxRefSrefIntvalToRefCall(dbsbrly, VecBrlyVCall::CALLBRLYCRDOPEN, jref, VecBrlyVPreset::PREBRLYREFCON, refCon, sref, recRly.ref, jrefNew);
		};
...
void PnlBrlyCon1NRelay::handleDpchAppDoButNewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXUACRLY, jref) & VecBrlyWUiaccess::EDIT) != 0) if (refCon != 0) {
			sref = "CrdBrlyRly";
...
void PnlBrlyCon1NRelay::handleDpchAppDoButDeleteClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyCon1NRelay::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyCon1NRelay::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyCon1NRelay::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
