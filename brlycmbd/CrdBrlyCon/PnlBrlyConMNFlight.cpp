/**
  ...
  * job handler for job PnlBrlyConMNFlight (implementation)
...
#include "PnlBrlyConMNFlight_blks.cpp"
#include "PnlBrlyConMNFlight_evals.cpp"

/******************************************************************************
 class PnlBrlyConMNFlight
 ******************************************************************************/

PnlBrlyConMNFlight::PnlBrlyConMNFlight(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYCONMNFLIGHT, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyConMNFlight(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyConMNFlight::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyConMNFlight::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyConMNFlight::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyConMNFlight::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYCONMNFLIGHTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYCONMNFLIGHTDO) {
...
void PnlBrlyConMNFlight::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyConMNFlight::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyConMNFlight::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyConMNFlight::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQConMNFlight* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyConMNFlight::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyConMNFlight::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyConMNFlight::StgIac::JNUM)) {
...
void PnlBrlyConMNFlight::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyConMNFlight::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyConMNFlight::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyConMNFlight::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyConMNFlight::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyConMNFlight::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
