/**
  * \file BrlycmbdAppsrv.cpp
  * application server for Brly combined daemon (implementation)
...
MHD_Daemon* BrlycmbdAppsrv::start(
			XchgBrlycmbd* xchg
		) {
//	return MHD_start_daemon(MHD_USE_SELECT_INTERNALLY, xchg->stgbrlycmbd.appsrvport, &MhdAccept, (void*) xchg, &MhdCallback, (void*) xchg, MHD_OPTION_NOTIFY_COMPLETED, &MhdComplete, (void*) xchg, MHD_OPTION_THREAD_POOL_SIZE, 16, MHD_OPTION_END);
	return MHD_start_daemon(MHD_USE_THREAD_PER_CONNECTION, xchg->stgbrlycmbd.appsrvport, &MhdAccept, (void*) xchg, &MhdCallback, (void*) xchg, MHD_OPTION_NOTIFY_COMPLETED, &MhdComplete, (void*) xchg, MHD_OPTION_END);
};

void BrlycmbdAppsrv::stop(
			MHD_Daemon* d
		) {
	MHD_stop_daemon(d);
};

int BrlycmbdAppsrv::MhdAccept(
			void* cls
			, const sockaddr* addr
			, socklen_t addrlen
		) {
...
int BrlycmbdAppsrv::MhdCallback(
			void* cls
	...
	if (*con_cls == NULL) {
		...
		if (strcmp(method, "GET") == 0) {
			...
			if (s.find("/web/") != string::npos) {
				...
				// ex. "/html/CrdBrlyNav/CrdBrlyNav.html" ; note that eventual parameters such as "?scrJref=abcd1234efgh5678" are not included in url
			...
			*con_cls = req;
			...
			if (req->ixVBasetype == ReqBrly::VecVBasetype::WEB) {
				...
				if (valid) {
					...
					if (dcol->dpchs.empty()) {
						if (dcol->hot) {
							...
							Mutex::lock(&(req->mcReady), "req->mcReady", "BrlycmbdAppsrv", "MhdCallback[1]");
							dcol->unlockAccess("BrlycmbdAppsrv", "MhdCallback[3]");
							Cond::timedwait(&(req->cReady), &(req->mcReady), 15000000, "req->cReady", "BrlycmbdAppsrv", "MhdCallback");
							// cout << "got one after timedwait" << endl;
							Mutex::unlock(&(req->mcReady), "req->mcReady", "BrlycmbdAppsrv", "MhdCallback[1]");

							dcol = xchg->getDcolByJref(req->jref);
							if (dcol) {
								// note: the destructor of dcol is called after dcol is removed from the list from which it can be found by getDcolByJref()
								dcol->req = NULL;
								dcol->unlockAccess("BrlycmbdAppsrv", "MhdCallback[4]");
						...
						dcol->hot = true;
						dcol->unlockAccess("BrlycmbdAppsrv", "MhdCallback[5]");
				...
				if (valid) {
					...
					dcol->unlockAccess("BrlycmbdAppsrv", "MhdCallback[6]");
				...
				if (valid) {
					...
					// wait for reply
					Mutex::lock(&(req->mcReady), "req->mcReady", "BrlycmbdAppsrv", "MhdCallback[2]");
					Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "BrlycmbdAppsrv", "MhdCallback[1]");
					Mutex::unlock(&(req->mcReady), "req->mcReady", "BrlycmbdAppsrv", "MhdCallback[2]");
				...
				*upload_data_size = 0;
				...
				if (req->ixVBasetype == ReqBrly::VecVBasetype::REGULAR) {
					...
					if (valid) {
						...
						// wait for reply
						Mutex::lock(&(req->mcReady), "req->mcReady", "BrlycmbdAppsrv", "MhdCallback[3]");
						if (req->ixVState != ReqBrly::VecVState::REPLY) Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "BrlycmbdAppsrv", "MhdCallback[2]");
						Mutex::unlock(&(req->mcReady), "req->mcReady", "BrlycmbdAppsrv", "MhdCallback[3]");
					...
					if (valid) {
						...
						Mutex::lock(&(req->mcReady), "req->mcReady", "BrlycmbdAppsrv", "MhdCallback[4]");
						Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "BrlycmbdAppsrv", "MhdCallback[3]");
						Mutex::unlock(&(req->mcReady), "req->mcReady", "BrlycmbdAppsrv", "MhdCallback[4]");
...
#ifdef _WIN32
int BrlycmbdAppsrv::MhdFilesend(
			void* con_cls
			, uint64_t pos
			, char* buf
			, int max
...
#else
ssize_t BrlycmbdAppsrv::MhdFilesend(
			void* con_cls
			, uint64_t pos
			, char* buf
			, size_t max
...
int BrlycmbdAppsrv::MhdPostrecv(
			void* con_cls
			, MHD_ValueKind kind
			, const char* key
			, const char* filename
...
void BrlycmbdAppsrv::MhdComplete(
			void* cls
			, MHD_Connection* connection
			, void** con_cls
...
bool BrlycmbdAppsrv::validateIp(
			XchgBrlycmbd* xchg
			, ReqBrly* req
		) {
	bool retval = true;
...
bool BrlycmbdAppsrv::validateJrefNotify(
			XchgBrlycmbd* xchg
			, ReqBrly* req
		) {
	bool retval = false;
	...
	if (dcol) {
		...
		dcol->unlockAccess("BrlycmbdAppsrv", "validateJrefNotify");
	};

	return retval;
};

uint BrlycmbdAppsrv::readDpchApp(
			XchgBrlycmbd* xchg
			, ReqBrly* req
		) {
	xmlDoc* doc = NULL;
...
void BrlycmbdAppsrv::writeDpchEng(
			XchgBrlycmbd* xchg
			, ReqBrly* req
		) {
	xmlBuffer* buf = NULL;
