/**
  * \file PnlBrlyUsgAAccess.cpp
  * job handler for job PnlBrlyUsgAAccess (implementation)
...
#include "PnlBrlyUsgAAccess_blks.cpp"
#include "PnlBrlyUsgAAccess_evals.cpp"

/******************************************************************************
 class PnlBrlyUsgAAccess
 ******************************************************************************/

PnlBrlyUsgAAccess::PnlBrlyUsgAAccess(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYUSGAACCESS, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyUsgAAccess(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyUsgAAccess::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyUsgAAccess::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyUsgAAccess::updatePreset(
			DbsBrly* dbsbrly
	...
	if (ixBrlyVPreset == VecBrlyVPreset::PREBRLYREFUSG) {
		recUsgAacc = BrlyAMUsergroupAccess();
...
void PnlBrlyUsgAAccess::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYUSGAACCESSDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYUSGAACCESSDO) {
...
void PnlBrlyUsgAAccess::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyUsgAAccess::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgAAccess::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyUsgAAccess::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQUsgAAccess* recSelNew = NULL;

	BrlyAMUsergroupAccess* _recUsgAacc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyUsgAAccess::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyUsgAAccess::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyUsgAAccess::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recUsgAacc = BrlyAMUsergroupAccess();

			if (recSelNew) {
				if (dbsbrly->tblbrlyamusergroupaccess->loadRecByRef(recSelNew->ref, &_recUsgAacc)) {
...
void PnlBrlyUsgAAccess::handleDpchAppDoButNewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgAAccess::handleDpchAppDoButDuplicateClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgAAccess::handleDpchAppDoButDeleteClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgAAccess::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyUsgAAccess::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyUsgAAccess::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
