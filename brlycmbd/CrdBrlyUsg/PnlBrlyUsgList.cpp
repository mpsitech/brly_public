/**
  * \file PnlBrlyUsgList.cpp
  * job handler for job PnlBrlyUsgList (implementation)
...
#include "PnlBrlyUsgList_blks.cpp"
#include "PnlBrlyUsgList_evals.cpp"

/******************************************************************************
 class PnlBrlyUsgList
 ******************************************************************************/

PnlBrlyUsgList::PnlBrlyUsgList(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYUSGLIST, jrefSup, ixBrlyVLocale) {
	...
	QryBrlyUsgList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecBrlyVPreset::PREBRLYREFSEL, jref, xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFUSG, jref));
	xchg->addIxPreset(VecBrlyVPreset::PREBRLYIXORD, jref, QryBrlyUsgList::VecVOrd::SRF);

	qry = new QryBrlyUsgList(xchg, dbsbrly, jref, ixBrlyVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixBrlyVExpstate = ((xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFUSG, jref)) ? VecBrlyVExpstate::MIND : VecBrlyVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyUsgList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyUsgList::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyUsgList::regularize(
			DbsBrly* dbsbrly
			, const bool notif
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgList::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYUSGLISTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYUSGLISTDO) {
...
void PnlBrlyUsgList::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyUsgList::handleDpchAppDataContiac(
			DbsBrly* dbsbrly
			, ContIac* _contiac
			, DpchEngBrly** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryBrlyUsgList::VecVOrd::SRF) && (_contiac->numFTos <= QryBrlyUsgList::VecVOrd::SRF)) {
...
void PnlBrlyUsgList::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgList::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyUsgList::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyUsgList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyUsgList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			refresh(dbsbrly, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyUsgList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsbrly, VecBrlyVCall::CALLBRLYREFPRESET, jref, VecBrlyVPreset::PREBRLYREFUSG, refSelNew);
		};
	};
...
void PnlBrlyUsgList::handleDpchAppDoButMinimizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgList::handleDpchAppDoButRegularizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgList::handleDpchAppDoButNewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgList::handleDpchAppDoButDeleteClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgList::handleDpchAppDoButFilterClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyUsgList::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyUsgList::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyUsgList::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
