/**
  * \file CrdBrlyUsg.cpp
  * job handler for job CrdBrlyUsg (implementation)
...
#include "CrdBrlyUsg_blks.cpp"

/******************************************************************************
 class CrdBrlyUsg
 ******************************************************************************/

CrdBrlyUsg::CrdBrlyUsg(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
			, const ubigint ref
		) : JobBrly(xchg, VecBrlyVJob::CRDBRLYUSG, jrefSup, ixBrlyVLocale) {
	...
	pnlrec = new PnlBrlyUsgRec(xchg, dbsbrly, jref, ixBrlyVLocale);
	pnlheadbar = new PnlBrlyUsgHeadbar(xchg, dbsbrly, jref, ixBrlyVLocale);
	pnllist = new PnlBrlyUsgList(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* CrdBrlyUsg::getNewDpchEng(
			set<uint> items
		) {
...
void CrdBrlyUsg::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdBrlyUsg/" + VecBrlyVLocale::getSref(ixBrlyVLocale);
	continf.MtxCrdUsg = StubBrly::getStubUsgStd(dbsbrly, xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFUSG, jref), ixBrlyVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdBrlyUsg::changeRef(
			DbsBrly* dbsbrly
	...
	uint ixBrlyVAccess;
	xchg->triggerIxRefToIxCall(dbsbrly, VecBrlyVCall::CALLBRLYACCESS, jref, VecBrlyVMaintable::TBLBRLYMUSERGROUP, ref, ixBrlyVAccess);

	if (ixBrlyVAccess != VecBrlyVAccess::NONE) {
		xchg->addIxPreset(VecBrlyVPreset::PREBRLYIXACC, jref, ixBrlyVAccess);

		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsbrly, VecBrlyVCall::CALLBRLYLOG, jref, VecBrlyVPreset::VOID, 0, "CrdBrlyUsg", ref);
		xchg->addRefPreset(VecBrlyVPreset::PREBRLYREFUSG, jref, ref);

		refresh(dbsbrly, moditems);
		if (notif) xchg->submitDpch(getNewDpchEng(moditems));

		if (pnllist) pnllist->updatePreset(dbsbrly, VecBrlyVPreset::PREBRLYREFUSG, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbsbrly, VecBrlyVPreset::PREBRLYREFUSG, jrefTrig, notif);
	};
};

void CrdBrlyUsg::updatePreset(
			DbsBrly* dbsbrly
...
void CrdBrlyUsg::leaveSgeIdle(
			DbsBrly* dbsbrly
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdBrlyUsg::enterSgeAlrbrlyabt(
			DbsBrly* dbsbrly
			, const bool reenter
		) {
...
void CrdBrlyUsg::leaveSgeAlrbrlyabt(
			DbsBrly* dbsbrly
		) {
	// IP leaveSgeAlrbrlyabt --- INSERT
};

uint CrdBrlyUsg::enterSgeClose(
			DbsBrly* dbsbrly
			, const bool reenter
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsbrly, VecBrlyVCall::CALLBRLYCRDCLOSE, jref, VecBrlyVCard::CRDBRLYUSG);
	return retval;
};

void CrdBrlyUsg::leaveSgeClose(
			DbsBrly* dbsbrly
		) {
	// IP leaveSgeClose --- INSERT
};

string CrdBrlyUsg::getSquawk(
			DbsBrly* dbsbrly
		) {
	string retval;
	// IP getSquawk --- BEGIN
...
void CrdBrlyUsg::handleRequest(
	...
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYUSGDO) {
...
void CrdBrlyUsg::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void CrdBrlyUsg::handleDpchAppDoClose(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	muteRefresh = true;
...
void CrdBrlyUsg::handleDpchAppDoMitAppAbtClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void CrdBrlyUsg::handleDpchAppBrlyAlert(
			DbsBrly* dbsbrly
...
bool CrdBrlyUsg::handleCallBrlyDlgClose(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdBrlyUsg::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdBrlyUsg::handleCallBrlyRefPreSet(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecBrlyVPreset::PREBRLYREFUSG) {
