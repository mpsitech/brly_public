/**
  * \file PnlBrlyRly1NNode.cpp
  * job handler for job PnlBrlyRly1NNode (implementation)
...
#include "PnlBrlyRly1NNode_blks.cpp"

/******************************************************************************
 class PnlBrlyRly1NNode
 ******************************************************************************/

PnlBrlyRly1NNode::PnlBrlyRly1NNode(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYRLY1NNODE, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyRly1NNode(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyRly1NNode::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyRly1NNode::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyRly1NNode::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYRLY1NNODEDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYRLY1NNODEDO) {
...
void PnlBrlyRly1NNode::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyRly1NNode::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRly1NNode::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyRly1NNode::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQRly1NNode* recSelNew = NULL;

	BrlyMNode* _recNde = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyRly1NNode::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyRly1NNode::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyRly1NNode::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recNde = BrlyMNode();

			if (recSelNew) {
				if (dbsbrly->tblbrlymnode->loadRecByRef(recSelNew->ref, &_recNde)) {
					recNde = *_recNde;
					delete _recNde;
				};
			};
...
void PnlBrlyRly1NNode::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyRly1NNode::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyRly1NNode::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
