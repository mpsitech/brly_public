/**
  ...
  * job handler for job PnlBrlyRlyList (implementation)
...
#include "PnlBrlyRlyList_blks.cpp"
#include "PnlBrlyRlyList_evals.cpp"

/******************************************************************************
 class PnlBrlyRlyList
 ******************************************************************************/

PnlBrlyRlyList::PnlBrlyRlyList(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYRLYLIST, jrefSup, ixBrlyVLocale) {
	...
	QryBrlyRlyList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecBrlyVPreset::PREBRLYREFSEL, jref, xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFRLY, jref));

	qry = new QryBrlyRlyList(xchg, dbsbrly, jref, ixBrlyVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixBrlyVExpstate = ((xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFRLY, jref)) ? VecBrlyVExpstate::MIND : VecBrlyVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyRlyList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyRlyList::refresh(
	...
	uint ixPre = xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXPRE, jref);
	if ((ixPre != 0) && (ixPre != VecBrlyVPreset::VOID)) {
		continf.TxtFor = VecBrlyVPreset::getTitle(ixPre, ixBrlyVLocale);

		if (ixPre == VecBrlyVPreset::PREBRLYREFCON) continf.TxtPre = StubBrly::getStubConStd(dbsbrly, xchg->getRefPreset(ixPre, jref), ixBrlyVLocale, Stub::VecVNonetype::FULL);
...
void PnlBrlyRlyList::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyRlyList::regularize(
			DbsBrly* dbsbrly
			, const bool notif
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRlyList::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYRLYLISTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYRLYLISTDO) {
...
void PnlBrlyRlyList::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyRlyList::handleDpchAppDataContiac(
			DbsBrly* dbsbrly
			, ContIac* _contiac
			, DpchEngBrly** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryBrlyRlyList::VecVOrd::STA) && (_contiac->numFTos <= QryBrlyRlyList::VecVOrd::DIR)) {
...
void PnlBrlyRlyList::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRlyList::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyRlyList::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyRlyList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyRlyList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			refresh(dbsbrly, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyRlyList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsbrly, VecBrlyVCall::CALLBRLYREFPRESET, jref, VecBrlyVPreset::PREBRLYREFRLY, refSelNew);
		};
	};
...
void PnlBrlyRlyList::handleDpchAppDoButMinimizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRlyList::handleDpchAppDoButRegularizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRlyList::handleDpchAppDoButNewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRlyList::handleDpchAppDoButDeleteClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRlyList::handleDpchAppDoButFilterClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRlyList::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyRlyList::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyRlyList::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
