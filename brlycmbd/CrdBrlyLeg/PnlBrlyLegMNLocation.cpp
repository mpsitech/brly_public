/**
  ...
  * job handler for job PnlBrlyLegMNLocation (implementation)
...
#include "PnlBrlyLegMNLocation_blks.cpp"
#include "PnlBrlyLegMNLocation_evals.cpp"

/******************************************************************************
 class PnlBrlyLegMNLocation
 ******************************************************************************/

PnlBrlyLegMNLocation::PnlBrlyLegMNLocation(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYLEGMNLOCATION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyLegMNLocation(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyLegMNLocation::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyLegMNLocation::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyLegMNLocation::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyLegMNLocation::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLEGMNLOCATIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYLEGMNLOCATIONDO) {
...
void PnlBrlyLegMNLocation::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyLegMNLocation::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLegMNLocation::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyLegMNLocation::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQLegMNLocation* recSelNew = NULL;

	BrlyRMLegMLocation* _recLegRloc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyLegMNLocation::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyLegMNLocation::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyLegMNLocation::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbsbrly->tblbrlyrmlegmlocation->loadRecByRef(recSelNew->ref, &_recLegRloc)) {
					recLegRloc = *_recLegRloc;
					delete _recLegRloc;
				};
...
void PnlBrlyLegMNLocation::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyLegMNLocation::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLegMNLocation::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyLegMNLocation::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyLegMNLocation::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyLegMNLocation::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
