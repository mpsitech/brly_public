/**
  * \file PnlBrlyLegList_blks.cpp
  * job handler for job PnlBrlyLegList (implementation of blocks)
...
void PnlBrlyLegList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfBrlyLegList";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfBrlyLegList";
...
void PnlBrlyLegList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrBrlyLegList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrBrlyLegList";
...
void PnlBrlyLegList::Tag::writeXML(
			const uint ixBrlyVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagBrlyLegList";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemBrlyLegList";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixBrlyVLocale == VecBrlyVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Legs");
			writeStringAttr(wr, itemtag, "sref", "TcoBlo", "Begin location");
			writeStringAttr(wr, itemtag, "sref", "TcoElo", "End location");
			writeStringAttr(wr, itemtag, "sref", "TcoTyp", "Type");
			writeStringAttr(wr, itemtag, "sref", "TcoAlt", "Altitude [m]");
		} else if (ixBrlyVLocale == VecBrlyVLocale::DECH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Strecken");
			writeStringAttr(wr, itemtag, "sref", "TcoBlo", "Beginn");
			writeStringAttr(wr, itemtag, "sref", "TcoElo", "Ende");
...
PnlBrlyLegList::DpchAppData::DpchAppData() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLEGLISTDATA) {
};
...
void PnlBrlyLegList::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyLegListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlyLegList::DpchAppDo::DpchAppDo() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYLEGLISTDO) {
	ixVDo = 0;
};
...
void PnlBrlyLegList::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyLegListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlyLegList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, const set<uint>& mask
		) : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYLEGLISTDATA, jref) {
...
void PnlBrlyLegList::DpchEngData::merge(
			DpchEngBrly* dpcheng
		) {
...
void PnlBrlyLegList::DpchEngData::writeXML(
			const uint ixBrlyVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngBrlyLegListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		...
		if (has(STATAPPQRY)) QryBrlyLegList::StatApp::writeXML(wr);
