/**
  * \file CrdBrlyPty_blks.cpp
  * job handler for job CrdBrlyPty (implementation of blocks)
...
void CrdBrlyPty::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdBrlyPty::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfBrlyPty";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfBrlyPty";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdPty", MtxCrdPty);
...
set<uint> CrdBrlyPty::ContInf::comm(
	...
	if (MtxCrdPty == comp->MtxCrdPty) insert(items, MTXCRDPTY);
...
void CrdBrlyPty::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppBrlyPty";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppBrlyPty";
...
void CrdBrlyPty::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrBrlyPty";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrBrlyPty";
...
void CrdBrlyPty::Tag::writeXML(
			const uint ixBrlyVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagBrlyPty";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemBrlyPty";
...
CrdBrlyPty::DpchAppDo::DpchAppDo() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYPTYDO) {
	ixVDo = 0;
};
...
void CrdBrlyPty::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyPtyDo");
	else
...
CrdBrlyPty::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFSge
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYPTYDATA, jref) {
...
void CrdBrlyPty::DpchEngData::merge(
			DpchEngBrly* dpcheng
		) {
...
void CrdBrlyPty::DpchEngData::writeXML(
			const uint ixBrlyVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngBrlyPtyData");
