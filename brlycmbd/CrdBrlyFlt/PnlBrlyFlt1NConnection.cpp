/**
  ...
  * job handler for job PnlBrlyFlt1NConnection (implementation)
...
#include "PnlBrlyFlt1NConnection_blks.cpp"
#include "PnlBrlyFlt1NConnection_evals.cpp"

/******************************************************************************
 class PnlBrlyFlt1NConnection
 ******************************************************************************/

PnlBrlyFlt1NConnection::PnlBrlyFlt1NConnection(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYFLT1NCONNECTION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyFlt1NConnection(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyFlt1NConnection::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyFlt1NConnection::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyFlt1NConnection::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyFlt1NConnection::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYFLT1NCONNECTIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYFLT1NCONNECTIONDO) {
...
void PnlBrlyFlt1NConnection::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyFlt1NConnection::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFlt1NConnection::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyFlt1NConnection::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQFlt1NConnection* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyFlt1NConnection::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyFlt1NConnection::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyFlt1NConnection::StgIac::JNUM)) {
...
void PnlBrlyFlt1NConnection::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyFlt1NConnection::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyFlt1NConnection::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyFlt1NConnection::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
