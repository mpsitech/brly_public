/**
  * \file PnlBrlyFltMNConnection_blks.cpp
  * job handler for job PnlBrlyFltMNConnection (implementation of blocks)
...
void PnlBrlyFltMNConnection::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfBrlyFltMNConnection";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfBrlyFltMNConnection";
...
void PnlBrlyFltMNConnection::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixBrlyVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppBrlyFltMNConnection";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppBrlyFltMNConnection";
...
void PnlBrlyFltMNConnection::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrBrlyFltMNConnection";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrBrlyFltMNConnection";
...
void PnlBrlyFltMNConnection::Tag::writeXML(
			const uint ixBrlyVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagBrlyFltMNConnection";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemBrlyFltMNConnection";
...
PnlBrlyFltMNConnection::DpchAppData::DpchAppData() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYFLTMNCONNECTIONDATA) {
};
...
void PnlBrlyFltMNConnection::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyFltMNConnectionData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlyFltMNConnection::DpchAppDo::DpchAppDo() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYFLTMNCONNECTIONDO) {
	ixVDo = 0;
};
...
void PnlBrlyFltMNConnection::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyFltMNConnectionDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlyFltMNConnection::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYFLTMNCONNECTIONDATA, jref) {
...
void PnlBrlyFltMNConnection::DpchEngData::merge(
			DpchEngBrly* dpcheng
		) {
...
void PnlBrlyFltMNConnection::DpchEngData::writeXML(
			const uint ixBrlyVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngBrlyFltMNConnectionData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		...
		if (has(STATAPPQRY)) QryBrlyFltMNConnection::StatApp::writeXML(wr);
