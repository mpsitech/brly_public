/**
  ...
  * job handler for job PnlBrlyFltMNConnection (implementation)
...
#include "PnlBrlyFltMNConnection_blks.cpp"
#include "PnlBrlyFltMNConnection_evals.cpp"

/******************************************************************************
 class PnlBrlyFltMNConnection
 ******************************************************************************/

PnlBrlyFltMNConnection::PnlBrlyFltMNConnection(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYFLTMNCONNECTION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyFltMNConnection(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyFltMNConnection::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyFltMNConnection::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyFltMNConnection::updatePreset(
			DbsBrly* dbsbrly
	...
	if (ixBrlyVPreset == VecBrlyVPreset::PREBRLYREFFLT) {
		recConRflt = BrlyRMConnectionMFlight();
...
void PnlBrlyFltMNConnection::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYFLTMNCONNECTIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYFLTMNCONNECTIONDO) {
...
void PnlBrlyFltMNConnection::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyFltMNConnection::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFltMNConnection::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyFltMNConnection::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQFltMNConnection* recSelNew = NULL;

	BrlyRMConnectionMFlight* _recConRflt = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyFltMNConnection::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyFltMNConnection::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyFltMNConnection::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbsbrly->tblbrlyrmconnectionmflight->loadRecByRef(recSelNew->ref, &_recConRflt)) {
					recConRflt = *_recConRflt;
					delete _recConRflt;
				};
...
void PnlBrlyFltMNConnection::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyFltMNConnection::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFltMNConnection::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFltMNConnection::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyFltMNConnection::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyFltMNConnection::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
