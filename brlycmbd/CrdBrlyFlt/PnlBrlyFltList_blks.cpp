/**
  * \file PnlBrlyFltList_blks.cpp
  * job handler for job PnlBrlyFltList (implementation of blocks)
...
void PnlBrlyFltList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfBrlyFltList";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfBrlyFltList";
...
void PnlBrlyFltList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrBrlyFltList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrBrlyFltList";
...
void PnlBrlyFltList::Tag::writeXML(
			const uint ixBrlyVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagBrlyFltList";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemBrlyFltList";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixBrlyVLocale == VecBrlyVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Flights");
			writeStringAttr(wr, itemtag, "sref", "TcoSrf", "Identifier");
			writeStringAttr(wr, itemtag, "sref", "TcoEqp", "Flight");
			writeStringAttr(wr, itemtag, "sref", "TcoLeg", "Leg");
			writeStringAttr(wr, itemtag, "sref", "TcoSta", "Departure");
			writeStringAttr(wr, itemtag, "sref", "TcoSto", "Arrival");
		} else if (ixBrlyVLocale == VecBrlyVLocale::DECH) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Fl\\u00fcge");
			writeStringAttr(wr, itemtag, "sref", "TcoSrf", "Bezeichner");
			writeStringAttr(wr, itemtag, "sref", "TcoEqp", "Flug");
			writeStringAttr(wr, itemtag, "sref", "TcoLeg", "Strecke");
			writeStringAttr(wr, itemtag, "sref", "TcoSta", "Abflug");
			writeStringAttr(wr, itemtag, "sref", "TcoSto", "Ankunft");
		};
...
PnlBrlyFltList::DpchAppData::DpchAppData() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYFLTLISTDATA) {
};
...
void PnlBrlyFltList::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyFltListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlyFltList::DpchAppDo::DpchAppDo() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYFLTLISTDO) {
	ixVDo = 0;
};
...
void PnlBrlyFltList::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyFltListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlBrlyFltList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, const set<uint>& mask
		) : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYFLTLISTDATA, jref) {
...
void PnlBrlyFltList::DpchEngData::merge(
			DpchEngBrly* dpcheng
		) {
...
void PnlBrlyFltList::DpchEngData::writeXML(
			const uint ixBrlyVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngBrlyFltListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/brly");
		...
		if (has(STATAPPQRY)) QryBrlyFltList::StatApp::writeXML(wr);
