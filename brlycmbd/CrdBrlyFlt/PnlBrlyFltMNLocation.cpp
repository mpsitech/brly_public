/**
  ...
  * job handler for job PnlBrlyFltMNLocation (implementation)
...
#include "PnlBrlyFltMNLocation_blks.cpp"
#include "PnlBrlyFltMNLocation_evals.cpp"

/******************************************************************************
 class PnlBrlyFltMNLocation
 ******************************************************************************/

PnlBrlyFltMNLocation::PnlBrlyFltMNLocation(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYFLTMNLOCATION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyFltMNLocation(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyFltMNLocation::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyFltMNLocation::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyFltMNLocation::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyFltMNLocation::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYFLTMNLOCATIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYFLTMNLOCATIONDO) {
...
void PnlBrlyFltMNLocation::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyFltMNLocation::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFltMNLocation::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyFltMNLocation::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQFltMNLocation* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyFltMNLocation::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyFltMNLocation::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyFltMNLocation::StgIac::JNUM)) {
...
void PnlBrlyFltMNLocation::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlBrlyFltMNLocation::handleDpchAppDoButAddClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFltMNLocation::handleDpchAppDoButSubClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyFltMNLocation::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyFltMNLocation::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyFltMNLocation::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
