/**
  * \file CrdBrlyFlt_blks.cpp
  * job handler for job CrdBrlyFlt (implementation of blocks)
...
/******************************************************************************
 class CrdBrlyFlt::VecVDo
 ******************************************************************************/

uint CrdBrlyFlt::VecVDo::getIx(
			const string& sref
		) {
...
string CrdBrlyFlt::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdBrlyFlt::VecVSge
 ******************************************************************************/

uint CrdBrlyFlt::VecVSge::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);
	...
	else if (s == "coldone") return COLDONE;
	else if (s == "plot") return PLOT;
	else if (s == "pltdone") return PLTDONE;

	return(0);
};

string CrdBrlyFlt::VecVSge::getSref(
			const uint ix
		) {
	...
	else if (ix == COLDONE) return("coldone");
	else if (ix == PLOT) return("plot");
	else if (ix == PLTDONE) return("pltdone");

	return("");
};

void CrdBrlyFlt::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
/******************************************************************************
 class CrdBrlyFlt::ContInf
 ******************************************************************************/

CrdBrlyFlt::ContInf::ContInf(
			const uint numFSge
			, const string& MrlAppHlp
			, const string& MtxCrdFlt
		) : Block() {
	this->numFSge = numFSge;
	this->MrlAppHlp = MrlAppHlp;
	this->MtxCrdFlt = MtxCrdFlt;
...
void CrdBrlyFlt::ContInf::writeXML(
	...
	else itemtag = "ContitemInfBrlyFlt";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdFlt", MtxCrdFlt);
	xmlTextWriterEndElement(wr);
};

set<uint> CrdBrlyFlt::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
	...
	if (MtxCrdFlt == comp->MtxCrdFlt) insert(items, MTXCRDFLT);

	return(items);
};

set<uint> CrdBrlyFlt::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {NUMFSGE, MRLAPPHLP, MTXCRDFLT};
...
void CrdBrlyFlt::StatApp::writeXML(
	...
	else itemtag = "StatitemAppBrlyFlt";
...
/******************************************************************************
 class CrdBrlyFlt::StatShr
 ******************************************************************************/

CrdBrlyFlt::StatShr::StatShr(
...
void CrdBrlyFlt::StatShr::writeXML(
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrBrlyFlt";
...
set<uint> CrdBrlyFlt::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> CrdBrlyFlt::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class CrdBrlyFlt::DpchAppDo
 ******************************************************************************/

CrdBrlyFlt::DpchAppDo::DpchAppDo() : DpchAppBrly(VecBrlyVDpch::DPCHAPPBRLYFLTDO) {
	ixVDo = 0;
};

string CrdBrlyFlt::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void CrdBrlyFlt::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppBrlyFltDo");
	else
...
/******************************************************************************
 class CrdBrlyFlt::DpchEngData
 ******************************************************************************/

CrdBrlyFlt::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFSge
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngBrly(VecBrlyVDpch::DPCHENGBRLYFLTDATA, jref) {
...
string CrdBrlyFlt::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void CrdBrlyFlt::DpchEngData::merge(
			DpchEngBrly* dpcheng
		) {
...
void CrdBrlyFlt::DpchEngData::writeXML(
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngBrlyFltData");
