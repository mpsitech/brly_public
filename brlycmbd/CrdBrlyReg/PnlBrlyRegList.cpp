/**
  * \file PnlBrlyRegList.cpp
  * job handler for job PnlBrlyRegList (implementation)
...
#include "PnlBrlyRegList_blks.cpp"
#include "PnlBrlyRegList_evals.cpp"

/******************************************************************************
 class PnlBrlyRegList
 ******************************************************************************/

PnlBrlyRegList::PnlBrlyRegList(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYREGLIST, jrefSup, ixBrlyVLocale) {
	...
	QryBrlyRegList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecBrlyVPreset::PREBRLYREFSEL, jref, xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFREG, jref));
	xchg->addIxPreset(VecBrlyVPreset::PREBRLYIXORD, jref, QryBrlyRegList::VecVOrd::SRF);

	qry = new QryBrlyRegList(xchg, dbsbrly, jref, ixBrlyVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixBrlyVExpstate = ((xchg->getRefPreset(VecBrlyVPreset::PREBRLYREFREG, jref)) ? VecBrlyVExpstate::MIND : VecBrlyVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyRegList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyRegList::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyRegList::regularize(
			DbsBrly* dbsbrly
			, const bool notif
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRegList::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYREGLISTDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYREGLISTDO) {
...
void PnlBrlyRegList::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyRegList::handleDpchAppDataContiac(
			DbsBrly* dbsbrly
			, ContIac* _contiac
			, DpchEngBrly** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryBrlyRegList::VecVOrd::SRF) && (_contiac->numFTos <= QryBrlyRegList::VecVOrd::SUP)) {
...
void PnlBrlyRegList::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRegList::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyRegList::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyRegList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyRegList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			refresh(dbsbrly, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyRegList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsbrly, VecBrlyVCall::CALLBRLYREFPRESET, jref, VecBrlyVPreset::PREBRLYREFREG, refSelNew);
		};
	};
...
void PnlBrlyRegList::handleDpchAppDoButMinimizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRegList::handleDpchAppDoButRegularizeClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRegList::handleDpchAppDoButNewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRegList::handleDpchAppDoButDeleteClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRegList::handleDpchAppDoButFilterClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRegList::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyRegList::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyRegList::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
