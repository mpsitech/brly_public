/**
  ...
  * job handler for job PnlBrlyRegSup1NRegion (implementation)
...
#include "PnlBrlyRegSup1NRegion_blks.cpp"
#include "PnlBrlyRegSup1NRegion_evals.cpp"

/******************************************************************************
 class PnlBrlyRegSup1NRegion
 ******************************************************************************/

PnlBrlyRegSup1NRegion::PnlBrlyRegSup1NRegion(
			XchgBrly* xchg
			, DbsBrly* dbsbrly
			, const ubigint jrefSup
			, const uint ixBrlyVLocale
		) : JobBrly(xchg, VecBrlyVJob::PNLBRLYREGSUP1NREGION, jrefSup, ixBrlyVLocale) {
	...
	qry = new QryBrlyRegSup1NRegion(xchg, dbsbrly, jref, ixBrlyVLocale);
...
// IP cust --- INSERT
DpchEngBrly* PnlBrlyRegSup1NRegion::getNewDpchEng(
			set<uint> items
		) {
...
void PnlBrlyRegSup1NRegion::refresh(
			DbsBrly* dbsbrly
			, set<uint>& moditems
		) {
...
void PnlBrlyRegSup1NRegion::updatePreset(
			DbsBrly* dbsbrly
...
void PnlBrlyRegSup1NRegion::handleRequest(
			DbsBrly* dbsbrly
			, ReqBrly* req
		) {
	if (req->ixVBasetype == ReqBrly::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYREGSUP1NREGIONDATA) {
		...
		} else if (req->dpchapp->ixBrlyVDpch == VecBrlyVDpch::DPCHAPPBRLYREGSUP1NREGIONDO) {
...
void PnlBrlyRegSup1NRegion::handleDpchAppBrlyInit(
			DbsBrly* dbsbrly
...
void PnlBrlyRegSup1NRegion::handleDpchAppDataStgiac(
			DbsBrly* dbsbrly
			, StgIac* _stgiac
			, DpchEngBrly** dpcheng
		) {
...
void PnlBrlyRegSup1NRegion::handleDpchAppDataStgiacqry(
			DbsBrly* dbsbrly
			, QryBrlyRegSup1NRegion::StgIac* _stgiacqry
			, DpchEngBrly** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	BrlyQRegSup1NRegion* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryBrlyRegSup1NRegion::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryBrlyRegSup1NRegion::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsbrly);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryBrlyRegSup1NRegion::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recReg = BrlyMRegion();

			if (recSelNew) {
				if (dbsbrly->tblbrlymregion->loadRecByRef(recSelNew->ref, &_recReg)) {
					recReg = *_recReg;
...
void PnlBrlyRegSup1NRegion::handleDpchAppDoButViewClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecBrlyVPreset::PREBRLYIXUACREG, jref)) {
			sref = "CrdBrlyReg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbsbrly, VecBrlyVCall::CALLBRLYCRDOPEN, jref, 0, 0, sref, recReg.ref, jrefNew);
		};
...
void PnlBrlyRegSup1NRegion::handleDpchAppDoButRefreshClick(
			DbsBrly* dbsbrly
			, DpchEngBrly** dpcheng
		) {
	set<uint> moditems;
...
void PnlBrlyRegSup1NRegion::handleCall(
			DbsBrly* dbsbrly
			, Call* call
		) {
...
bool PnlBrlyRegSup1NRegion::handleCallBrlyStatChg(
			DbsBrly* dbsbrly
			, const ubigint jrefTrig
		) {
	bool retval = false;
